...
def _final_live_scan():
    try:
        ...
        if not matches:
            print("LIVE FINAL | matches=0 | candidates=0 | qualified=0 | sent=0")
            return 0
        ...
        for m in matches:
            try:
                ...
                status = fixture.get('status', {}) or {}
                minute = int(_v10_num(status.get('elapsed'), 0))
                ...
                # Use the newest candidate engine available in the file.
                c = None
                try:
                    c = get_best_live_signal(m)
                except Exception:
                    try:
                        c = _v10_live_candidates(m)
                    except Exception:
                        c = None
        ...
        sent = 0
        for s in ranked[:MAX_LIVE_SIGNALS_PER_SCAN]:
            try:
        ...
        print(
            f"LIVE FINAL | matches={len(matches)} | eligible={eligible} | "
            f"candidates={len(candidates)} | qualified={len(ranked)} | sent={sent}"
        )
        return sent
...
def print_system_status():
    print('=' * 64)
    print('🤖 AI FOOTBALL BOT — FINAL WORKING ENGINE')
    print('STATUS: ONLINE')
    print('LIVE:', LIVE_SCAN_INTERVAL, 'sec')
    print('PREMATCH:', PREMATCH_SCAN_INTERVAL, 'sec')
...
BET_BUILDER_MAX_LEGS = 5
BUILDER_MIN_LEG_PROB = 70.0
BUILDER_MIN_LEG_CONF = 72.0
BUILDER_MAX_RISK = 32.0

def _final_builder_market_allowed(label):
    s = str(label or '').upper()
    # Explicitly reject markets that were causing the bad signal.
    if 'TOUCH' in s or 'THROW' in s or 'UNDER 4.5' in s:
        return False
    return any(x in s for x in (
        'OVER 1.5 GOAL', 'OVER 2.5 GOAL', 'UNDER 2.5 GOAL',
        'OVER 3.5 GOAL', 'UNDER 3.5 GOAL',
        'HOME OVER 1.5 GOAL', 'AWAY OVER 1.5 GOAL',
        'BTTS', 'CORNER ', 'CARD ',
        'HOME WIN', 'AWAY WIN'
    ))
...
def _final_builder_candidates(match):
    try:
        rows = _v7_prematch_candidates(match, detailed=True) or []
    except Exception as e:
        logging.warning("FINAL BUILDER MODEL ERROR: %s", repr(e))
        return []
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        label = row.get('market') or row.get('label') or ''
        odd = safe_float(row.get('odd'))
        prob = safe_float(row.get('probability'))
        conf = safe_float(row.get('confidence'))
        risk = safe_float(row.get('risk'), 100)

        # Model probability MUST exist. Never derive it from the odd.
        if not _final_builder_market_allowed(label):
            continue
        if odd is None or odd < 1.05 or odd > 3.00:
            continue
        if prob is None or prob < BUILDER_MIN_LEG_PROB or prob > 99:
            continue
        if conf is None or conf < BUILDER_MIN_LEG_CONF:
            continue
        if risk is None or risk > BUILDER_MAX_RISK:
            continue
        ...
        if not family:
            u = str(label).upper()
            family = (
                'goals' if 'GOAL' in u or 'BTTS' in u else
                'corners' if 'CORNER' in u else
                'cards' if 'CARD' in u else
                'result'
            )

        key = (str(label).strip().upper(), round(float(odd), 3))
        if key in seen:
            continue
        seen.add(key)
...
def build_best_bet_builder(match, detailed=True):
    ...
    for n in range(BET_BUILDER_MIN_LEGS, max_n + 1):
        for combo in combinations(candidates, n):
            ...
            # REAL model probabilities only.
            joint = 1.0
            for leg in combo:
                joint *= leg['probability'] / 100.0

            # Conservative correlation penalties.
            if 'goals' in families and any(f == 'result' for f in families):
                joint *= 0.97
            if 'goals' in families and len(families) > 1:
                joint *= 0.98
            if 'corners' in families and 'cards' in families:
                joint *= 0.96
            ...
            confidence = min(x['confidence'] for x in combo)
            risk = max(x['risk'] for x in combo)
            risk += max(0.0, 75.0 - joint_pct) * 0.20
            risk = min(38.0, risk)
            ...
            if best is None or score > best[0]:
                best = (score, combo, odd, joint_pct, confidence, risk, edge)

    if best is None:
        return None

    score, combo, odd, joint_pct, confidence, risk, edge = best
    fixture = match.get('fixture', {}) or {}
        ...
        'builder_legs': [
            {
                'market': leg['market'],
                'odd': round(leg['odd'], 2),
                'probability': round(leg['probability'], 1)
            }
            for leg in combo
        ]
    }

def _final_builder_sanity(signal):
    """Last gate before Telegram: reject impossible/old Builder output."""
    if not isinstance(signal, dict):
        return False
    ...
    if not (2 <= len(legs) <= 5):
        return False
    for leg in legs:
        label = str(leg.get('market', '')).upper()
        if 'UNDER 4.5' in label or 'TOUCH' in label or 'THROW' in label:
            return False
        p = safe_float(leg.get('probability'))
        o = safe_float(leg.get('odd'))
        if p is None or o is None or p <= 0 or o <= 1:
            return False
    return True
...
# ============================================================
# Goals:
# 1) LIVE must use the real live Betano market feed, not only NEXT GOAL.
# 2) LIVE may use NEXT GOAL / remaining goals / corners / cards when the
#    corresponding real Betano live market exists.
# 3) PREMATCH keeps the existing statistical model, but normal signals and
#    Builder are selected independently.
# 4) Builder uses REAL model probabilities, different market families and
#    conservative correlation penalties. No probability is invented from
#    bookmaker odds.
# 5) PREMATCH only accepts fixtures whose kickoff is genuinely in the future.
# 6) One signal per fixture/market is sent; new fixtures can appear later.
# ============================================================

from math import exp, sqrt
from itertools import combinations as _v8_combinations

V8_NAME = "AI FOOTBALL BOT V8 — LIVE + PREMATCH FIXED"
...
PREMATCH_MAX_RISK = 32.0
PREMATCH_MIN_ODD = 1.35
PREMATCH_MAX_ODD = 4.50
...
BUILDER_MIN_LEG_CONF = 74.0
BUILDER_MAX_RISK = 30.0

V8_LIVE_MARKET_CACHE = {}
V8_PREMATCH_SENT = set()
V8_BUILDER_SENT = set()
...
def _v8_clamp(v, lo, hi):
    return max(lo, min(hi, _v8_num(v, lo)))


def _v8_total(stats, *keys):
    total = 0.0
    for key in keys:
        total += _v8_num((stats or {}).get("home", {}).get(key), 0)
        total += _v8_num((stats or {}).get("away", {}).get(key), 0)
    return total


def _v8_betano(bookmakers):
...
def get_live_betano_markets(fixture_id):
    ...
    now = time.time()
    cached = V8_LIVE_MARKET_CACHE.get(fixture_id)
    ...
    try:
        ...
        # response -> bookmaker(s) -> bets -> values
        for item in response:
            if not isinstance(item, dict):
                continue
            b = _v8_betano(item.get("bookmakers"))
            if b:
                bookmaker = b
                break
            if clean_text(item.get("name")) == "betano" and item.get("bets"):
                bookmaker = item
                break

        if not bookmaker:
            V8_LIVE_MARKET_CACHE[fixture_id] = (now, [])
            return []
        ...
        V8_LIVE_MARKET_CACHE[fixture_id] = (now, result)
        return result

    except Exception as e:
        logging.warning("V8 LIVE ODDS ERROR | fixture=%s | %s",
                        fixture_id, repr(e))
        V8_LIVE_MARKET_CACHE[fixture_id] = (now, [])
        return []


def _v8_find_live(markets, families, targets=(), side=None):
    """
    Flexible live-market lookup. It deliberately does not depend on one
    exact API-Football market name.
    """
    families = tuple(clean_text(x) for x in families)
    targets = tuple(clean_text(x) for x in targets)

    for market in markets or []:
        name = clean_text(market.get("name"))
        if not any(f in name for f in families):
            continue

        for value in market.get("values") or []:
            vn = clean_text(value.get("value"))
            if targets and not any(vn.startswith(t) for t in targets):
                continue

            if side:
                side_n = clean_text(side)
                if side_n not in vn:
                    continue
...
# LIVE MODEL
# ------------------------------------------------------------
def _v8_live_stats(match):
    return get_live_market_stats(match) or {}


def _v8_live_activity(stats):
    h = stats.get("home", {}) or {}
    a = stats.get("away", {}) or {}

    def activity(x):
        return (
            _v8_num(x.get("shots on goal"), 0) * 8.0 +
            _v8_num(x.get("total shots"), 0) * 1.5 +
            _v8_num(x.get("dangerous attacks"), 0) * 0.18 +
            _v8_num(x.get("expected goals"), 0) * 14.0 +
            _v8_num(x.get("corner kicks"), 0) * 1.8
        )

    return activity(h), activity(a)


def _v8_next_goal_probability(stats, side):
    ha, aa = _v8_live_activity(stats)
    total = ha + aa

    if total <= 0:
        return None

    selected = ha if side == "home" else aa
    dominance = selected / total

    shots_on = _v8_total(stats, "shots on goal")
    xg = _v8_total(stats, "expected goals")
    dangerous = _v8_total(stats, "dangerous attacks")

    p = (
        52.0
        + (dominance - 0.50) * 70.0
        + min(10.0, shots_on * 1.4)
        + min(8.0, xg * 2.5)
        + min(6.0, dangerous * 0.04)
    )

    return round(_v8_clamp(p, 55, 90), 1)


def _v8_remaining_goal_probability(stats, minute, current_goals):
    """
    Conservative Poisson-style estimate for additional goals.
    It is based on observed live activity and does not use the bookmaker
    price as a model probability.
    """
    shots_on = _v8_total(stats, "shots on goal")
    shots = _v8_total(stats, "total shots")
    xg = _v8_total(stats, "expected goals")
    dangerous = _v8_total(stats, "dangerous attacks")
    ...
    elapsed = max(1.0, float(minute))
    remaining = max(2.0, 90.0 - elapsed)

    # Activity-derived expected goals for the remaining time.
    lam = activity * (remaining / max(25.0, elapsed)) * 1.8
    lam = _v8_clamp(lam, 0.08, 2.20)

    # Under/over 1.5 remaining goals depends on the current total.
    needed = max(1, 2 - int(current_goals))

    if needed == 1:
        p = 1.0 - exp(-lam)
    else:
        p = 1.0 - exp(-lam) * (1.0 + lam)

    return round(_v8_clamp(p * 100.0, 45, 92), 1)


def _v8_remaining_corners_probability(stats, minute):
    corners = _v8_total(stats, "corner kicks")
    shots = _v8_total(stats, "total shots")
    on = _v8_total(stats, "shots on goal")

    remaining = max(2.0, 90.0 - minute)
    rate = corners / max(1.0, minute)

    expected_remaining = rate * remaining + on * 0.10 + shots * 0.025

    p = 1.0 - exp(-max(0.02, expected_remaining))
    return round(_v8_clamp(p * 100.0, 45, 92), 1)


def _v8_remaining_cards_probability(stats, minute):
    cards = _v8_total(stats, "yellow cards", "yellow card")
    fouls = _v8_total(stats, "fouls")

    remaining = max(2.0, 90.0 - minute)
    rate = cards / max(1.0, minute)

    expected_remaining = rate * remaining + fouls * 0.008

    p = 1.0 - exp(-max(0.02, expected_remaining))
    return round(_v8_clamp(p * 100.0, 45, 92), 1)


def _v8_live_confidence(prob, stats, minute, market):
    shots_on = _v8_total(stats, "shots on goal")
    shots = _v8_total(stats, "total shots")
    xg = _v8_total(stats, "expected goals")

    c = 62.0
    c += max(0.0, prob - 60.0) * 0.38
    c += min(10.0, shots_on * 1.5)
    c += min(7.0, shots * 0.25)
    c += min(7.0, xg * 3.0)
    ...
    if "CORNER" in market:
        c += min(5.0, _v8_total(stats, "corner kicks") * 0.7)

    if "CARD" in market:
        c += min(
            5.0,
            _v8_total(stats, "yellow cards", "yellow card") * 1.5
        )

    return round(_v8_clamp(c, 62, 92), 1)


def _v8_live_risk(prob, confidence, minute, market, stats):
    risk = 26.0
    ...
    if "NEXT GOAL" in market:
        if _v8_total(stats, "shots on goal") < 2:
            risk += 7

    if "CORNER" in market:
        if _v8_total(stats, "corner kicks") < 2:
            risk += 6

    if "CARD" in market:
        if _v8_total(stats, "yellow cards", "yellow card") < 1:
            risk += 6

    return int(_v8_clamp(risk, 8, 45))


def _v8_make_live_candidate(match, market, odd, probability, stats, minute):
    if odd is None:
        return None
    ...
    confidence = _v8_live_confidence(
        probability, stats, minute, market
    )
    risk = _v8_live_risk(
        probability, confidence, minute, market, stats
    )
    edge = value_edge(probability, odd)
    ...
    return {
        ...
        "score": round(score, 2),
        "minute": minute,
        "home_goals": int(_v8_num(goals.get("home"), 0)),
        "away_goals": int(_v8_num(goals.get("away"), 0)),
        "xg": round(_v8_total(stats, "expected goals"), 2),
        "pressure": round(
            _v8_total(stats, "shots on goal") * 8 +
            _v8_total(stats, "corner kicks") * 4 +
            _v8_total(stats, "dangerous attacks") * 0.12,
            1
        ),
    }


def _v8_live_candidates(match):
    fixture = match.get("fixture", {}) or {}
    fid = fixture.get("id")
    minute = int(_v8_num(
        (fixture.get("status") or {}).get("elapsed"), 0
    ))

    if not fid or minute < LIVE_MINUTE or minute > LIVE_MAX_MINUTE:
        return []

    stats = _v8_live_stats(match)
    markets = get_live_betano_markets(fid) or []
    ...
    # 1. NEXT GOAL — either side, only if the real market exists.
    for side, label in (
        ("home", "🎯 NEXT GOAL HOME"),
        ("away", "🎯 NEXT GOAL AWAY"),
    ):
        ...
        if odd:
            p = _v8_next_goal_probability(stats, side)
            if p is not None:
                c = _v8_make_live_candidate(
                    match, label, odd, p, stats, minute
                )
                if c:
                    result.append(c)

    # 2. OVER 1.5 GOALS — actual live line.
    odd = _v8_find_live(
        markets,
        ("goals", "total goals"),
        targets=("over 1.5", "over 1.5 goals")
    )
    if odd:
        current = (
            int(_v8_num((match.get("goals") or {}).get("home"), 0)) +
            int(_v8_num((match.get("goals") or {}).get("away"), 0))
        )
        p = _v8_remaining_goal_probability(
            stats, minute, current
        )
        c = _v8_make_live_candidate(
            match, "⚽ OVER 1.5 GOALS", odd, p, stats, minute
        )
        if c:
            result.append(c)

    # 3. NEXT CORNER / OVER 1.5 CORNERS — actual live line.
    odd = _v8_find_live(
        markets,
        ("corner", "corners"),
        targets=("over 1.5", "over 1.5 corners")
    )
    if odd:
        p = _v8_remaining_corners_probability(stats, minute)
        c = _v8_make_live_candidate(
            match, "🚩 OVER 1.5 CORNERS", odd, p, stats, minute
        )
        if c:
            result.append(c)

    # 4. CARDS — actual live line.
    odd = _v8_find_live(
        markets,
        ("card", "cards", "booking"),
        targets=("over 1.5", "over 1.5 cards")
    )
    if odd:
        p = _v8_remaining_cards_probability(stats, minute)
        c = _v8_make_live_candidate(
            match, "🟨 OVER 1.5 CARDS", odd, p, stats, minute
        )
        if c:
            result.append(c)
...
def get_best_live_signal(match):
    candidates = _v8_live_candidates(match)
    if not candidates:
        return None
...
def rank_live_signals(signals):
    ...
    for s in signals or []:
        ...
        if s.get("fixture_id") in used_fixtures:
            continue

        if (
            _v8_num(s.get("probability"), 0) >= LIVE_MIN_PROBABILITY and
            _v8_num(s.get("confidence"), 0) >= LIVE_MIN_CONFIDENCE and
            _v8_num(s.get("risk"), 100) <= LIVE_MAX_RISK and
            LIVE_MIN_ODD <= _v8_num(s.get("odd"), 0) <= LIVE_MAX_ODD and
            _v8_num(s.get("edge"), -999) >= 0
        ):
            valid.append(s)
            used_fixtures.add(s.get("fixture_id"))
...
def send_live_signal(signal):
    ...
    market = signal.get("market")
    minute = int(_v8_num(signal.get("minute"), 0))
    ...
    # Do not spam the same fixture/market every minute.
    if previous:
        old_minute = int(_v8_num(previous.get("minute"), 0))
        if now - previous.get("time", 0) < LIVE_COOLDOWN:
            return False
...
def scan_live():
    try:
        ...
        for match in matches:
            try:
                candidates.extend(_v8_live_candidates(match))
            except Exception as e:
                logging.warning("V8 LIVE MATCH ERROR: %s", repr(e))
        ...
        print(
            f"V8 LIVE DEBUG | matches={len(matches)} | "
            f"candidates={len(candidates)} | qualified={len(ranked)} | "
            f"sent={sent}"
        )
        ...
        logging.exception("V8 LIVE SCAN ERROR: %s", repr(e))
        return 0
...
def format_live_signal(signal):
    return (
        "🔥 LIVE V8\n\n"
        f"⚽ {signal.get('home_team','HOME')} - "
        f"{signal.get('away_team','AWAY')}\n"
        f"🌍 {signal.get('country','')}\n"
        f"🏆 {signal.get('league','')}\n"
        f"⏱ {signal.get('minute',0)}' | "
        f"{signal.get('home_goals',0)}-{signal.get('away_goals',0)}\n\n"
        f"🎯 MARKET: {signal.get('market','UNKNOWN')}\n"
        f"📈 Probability: {signal.get('probability',0):.1f}%\n"
        f"💰 Betano Odds: {signal.get('odd',0):.2f}\n"
        f"📊 Edge: {signal.get('edge',0):+.1f}%\n"
...
# ------------------------------------------------------------
# PREMATCH: KEEP THE EXISTING MODEL, FIX SELECTION
# ------------------------------------------------------------
def _v8_future_matches(matches):
    now = datetime.now(TIMEZONE)
    result = []
...
def _v8_select_normal(matches):
    """
    Existing V7 statistical candidate engine is retained.
    Selection is now one strong individual signal per fixture.
    """
    candidates = []

    for match in matches:
        try:
            rows = _v7_prematch_candidates(match, detailed=False) or []
            for row in rows:
                if not isinstance(row, dict):
                    continue

                p = _v8_num(row.get("probability"), 0)
                c = _v8_num(row.get("confidence"), 0)
                r = _v8_num(row.get("risk"), 100)
                odd = _v8_num(row.get("odd"), 0)

                if not (
                    p >= PREMATCH_MIN_PROBABILITY and
                    c >= PREMATCH_MIN_CONFIDENCE and
                    r <= PREMATCH_MAX_RISK and
                    PREMATCH_MIN_ODD <= odd <= PREMATCH_MAX_ODD
                ):
                    continue
                ...
                row["score"] = (
                    p * 0.55 +
                    c * 0.30 +
                    max(0, _v8_num(row.get("edge"), 0)) * 0.15 -
                    r * 0.10
                )
                candidates.append(row)

        except Exception as e:
            logging.warning("V8 PREMATCH MODEL ERROR: %s", repr(e))

    # Best individual signal per fixture.
    candidates.sort(
        key=lambda x: (
            x.get("score", 0),
            x.get("probability", 0),
    ...
    for row in candidates:
        ...
        # Do not repeat the same market family five times.
        family = row.get("family", "")
        if sum(
            1 for x in selected
...
# ------------------------------------------------------------
# BUILDER: MODEL PROBABILITIES ONLY + DIFFERENT FAMILIES
# ------------------------------------------------------------
def _v8_builder_candidates(match):
    rows = _v7_prematch_candidates(match, detailed=True) or []
    result = []
    ...
    for row in rows:
        ...
        label = str(row.get("market", "")).strip()
        p = _v8_num(row.get("probability"), 0)
        c = _v8_num(row.get("confidence"), 0)
        r = _v8_num(row.get("risk"), 100)
        odd = _v8_num(row.get("odd"), 0)
        family = row.get("family") or _v8_family(label)

        # Never use artificial 95/99% probabilities as builder legs.
        # The model itself is capped here at 90.
        if p > 90:
            p = 90.0

        if not (
            p >= BUILDER_MIN_LEG_PROB and
            c >= BUILDER_MIN_LEG_CONF and
            r <= BUILDER_MAX_RISK and
            1.05 <= odd <= 2.20
        ):
            continue
    ...
    # Exact market duplicate removal.
    seen = set()
    unique = []
    ...
    for row in sorted(
        ...
        key = (row["market"], round(row["odd"], 3))
        if key in seen:
            continue
        seen.add(key)
        unique.append(row)
...
def build_best_bet_builder(match, detailed=True):
    candidates = _v8_builder_candidates(match)
    ...
    for n in range(
        ...
        min(BET_BUILDER_MAX_LEGS, len(candidates)) + 1
    ):
        for combo in _v8_combinations(candidates, n):
            families = [x["family"] for x in combo]

            # No two legs from the same family.
            if len(set(families)) != len(families):
                continue
            ...
            if not (BET_BUILDER_MIN_ODD <= odd <= BET_BUILDER_MAX_ODD):
                continue
            ...
            if "goals" in families and "btts" in families:
                joint *= 0.88

            if "goals" in families and "result" in families:
                joint *= 0.94
            ...
            joint_pct = min(90.0, joint * 100.0)
            implied = 100.0 / odd
            edge = joint_pct - implied

            # A Builder must itself have value.
            if edge < 1.0:
                continue

            confidence = min(
                x["confidence"] for x in combo
            )

            risk = max(
                x["risk"] for x in combo
            )

            risk += max(0.0, 72.0 - joint_pct) * 0.20
            risk += (n - 2) * 2.0
    ...
    score, combo, odd, joint, confidence, risk, edge = best
...
def _v8_send_prematch(signal):
    ...
    # Keep individual and Builder deduplication separate.
    if market == "🧩 BET BUILDER":
        if key in V8_BUILDER_SENT:
            return False
    else:
        if key in V8_PREMATCH_SENT:
            return False
    ...
    if market == "🧩 BET BUILDER":
        V8_BUILDER_SENT.add(key)
    else:
        V8_PREMATCH_SENT.add(key)
...
def scan_prematch():
    try:
        raw = get_prematch_matches() or []
        matches = _v8_future_matches(raw)

        print(
            f"V8 PREMATCH DEBUG | api={len(raw)} | "
            f"future={len(matches)}"
        )
        ...
        # Normal signals are the main portfolio.
        for signal in _v8_select_normal(matches):
            if _v8_send_prematch(signal):
                sent += 1

        # Builder is optional. It can never replace the normal portfolio.
        builders = []

        for match in matches:
            try:
                b = build_best_bet_builder(match, True)
                if b:
                    builders.append(b)
            except Exception as e:
                logging.warning(
                    "V8 BUILDER MODEL ERROR: %s", repr(e)
                )
        ...
        for b in builders[:BET_BUILDER_DAILY_TOP3]:
            if _v8_send_prematch(b):
                builder_sent += 1

        print(
            f"V8 PREMATCH | normal_sent={sent} | "
            f"builder_sent={builder_sent}"
        )

        return sent + builder_sent

    except Exception as e:
        logging.exception("V8 PREMATCH ERROR: %s", repr(e))
        return 0


# ------------------------------------------------------------
# STATUS + SINGLE CLEAN MAIN LOOP
# ------------------------------------------------------------
def print_system_status():
    print("=" * 68)
    print(V8_NAME)
    ...
    print("BUILDER LEGS:", BET_BUILDER_MIN_LEGS, "-", BET_BUILDER_MAX_LEGS)
    print("BUILDER TOP:", BET_BUILDER_DAILY_TOP3)
    print("PREMATCH: FUTURE FIXTURE FILTER ON")
    print("BUILDER: CORRELATION FILTER ON")
    print("=" * 68)
...
# ============================================================
# FINAL V8 MAIN — PROVEN V3 LIVE + V8 PREMATCH
# ============================================================

LIVE_LOOP_INTERVAL = 300
PREMATCH_LOOP_INTERVAL = 900
...
def main_loop():
    ...
    print("=" * 68)
    print("AI FOOTBALL BOT V8")
    print("LIVE ENGINE: V3 PROVEN")
    print("PREMATCH ENGINE: V8")
    print("BUILDER: V8 OPTIONAL / SEPARATE")
    print("LIVE INTERVAL:", LIVE_LOOP_INTERVAL, "sec")
    print("PREMATCH INTERVAL:", PREMATCH_LOOP_INTERVAL, "sec")
    print("=" * 68)
    ...
    while True:
        ...
        if now - last_prematch_scan >= PREMATCH_LOOP_INTERVAL:
            try:
            ...
            try:
                ...
                logging.exception("V8 PREMATCH SCAN ERROR: %s", repr(e))
        ...
        try:
            ...
            logging.exception("V3 LIVE LOOP ERROR: %s", repr(e))
...
if __name__ == "__main__":
    try:
        ...
        logging.exception("V8 FATAL MAIN ERROR: %s", repr(e))
