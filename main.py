...
def get_match_odds(fixture_id):
    """Return current Betano pre-match prices only.

    No bookmaker fallback and no prematch cache: the response is fetched
    immediately when the signal is built. Betano is located across every
    response item, not only odds[0].
    """
    if not fixture_id:
        return None
    ...
    for item in odds:
        ...
        for bookmaker in item.get('bookmakers', []) or []:
            if not isinstance(bookmaker, dict):
                continue
            if bookmaker.get('id') == 32 or clean_text(bookmaker.get('name')) == 'betano':
                betano = bookmaker
                break
        if betano is not None:
            break
    ...
    home = draw = away = None
    over25 = under25 = over35 = under35 = None
    btts = home_over15 = away_over15 = None
    ...
    for bet in betano.get('bets', []) or []:
        ...
        for value in values:
            if not isinstance(value, dict) or bool(value.get('stopped', False)):
                continue
            ...
            if label == 'home' and name in ('match winner', 'winner', '1x2'):
                home = odd
            elif label == 'draw' and name in ('match winner', 'winner', '1x2'):
                draw = odd
            elif label == 'away' and name in ('match winner', 'winner', '1x2'):
                away = odd
            elif ('both teams to score' in name or name == 'btts') and label == 'yes':
                btts = odd
            elif 'goal' in name and 'home' not in name and 'away' not in name:
                if label == 'over 2.5': over25 = odd
                elif label == 'under 2.5': under25 = odd
                elif label == 'over 3.5': over35 = odd
                elif label == 'under 3.5': under35 = odd
            elif 'home' in name and 'goal' in name and label == 'over 1.5':
                home_over15 = odd
            elif 'away' in name and 'goal' in name and label == 'over 1.5':
                away_over15 = odd

    # 1X2 is required by the legacy normal engine.
    if home is None or draw is None or away is None:
        return None

    return (home, draw, away, over25, under25, over35, under35, btts, home_over15, away_over15)
...
LIVE_MIN_PROBABILITY = 60
LIVE_MIN_CONFIDENCE = 65

LIVE_MIN_PRESSURE = 55
LIVE_MIN_ATTACK = 55
...
def save_signal(signal):
    """Persist signal and entry state required for reliable result tracking."""
    try:
        conn=sqlite3.connect(DB_NAME); cur=conn.cursor()
        cur.execute("""INSERT INTO signals(
            fixture_id,country,league,home_team,away_team,market,probability,odd,confidence,result,created_at,status,
            minute,home_id,away_id,home_goals,away_goals,fixture_status
        ) VALUES (?,?,?,?,?,?,?,?,?,NULL,?,?,?,?,?,?,?,?,?)""", (
            signal.get('fixture_id'),signal.get('country',''),signal.get('league',''),signal.get('home_team','HOME'),signal.get('away_team','AWAY'),
            signal.get('market','UNKNOWN'),_v7_num(signal.get('probability'),0),_v7_num(signal.get('odd'),0),_v7_num(signal.get('confidence'),0),
            datetime.now(TIMEZONE).isoformat(),'PENDING',int(_v7_num(signal.get('minute'),0)),signal.get('home_id'),signal.get('away_id'),
            int(_v7_num(signal.get('home_goals'),0)),int(_v7_num(signal.get('away_goals'),0)),'LIVE' if signal.get('minute') is not None else 'PREMATCH'
        ))
        conn.commit(); conn.close(); return True
    except Exception as e:
        logging.warning('DB SAVE ERROR: %s',repr(e)); return False
...
def evaluate_extended_live_market(signal):
    """Evaluate a live market against the state at which the signal was sent."""
    ...
    hg = int(result.get('home', 0) or 0)
    ag = int(result.get('away', 0) or 0)
    entry_hg = int(signal.get('home_goals', 0) or 0)
    entry_ag = int(signal.get('away_goals', 0) or 0)
    entry_total = entry_hg + entry_ag
    total = hg + ag
    entry_minute = int(signal.get('minute', 0) or 0)

    # Next-goal markets: only goals AFTER the signal entry minute count.
    if market in ('🎯 NEXT GOAL HOME', '🎯 NEXT GOAL AWAY', '⚡ FAST NEXT GOAL'):
        ...
        wanted = None
        for ev in events:
            if clean_text(ev.get('type')) != 'goal':
                continue
            em = _safe_num(ev.get('time', {}).get('elapsed'), None)
            if em is None or em <= entry_minute:
                continue
            tid = ev.get('team', {}).get('id')
            if tid == signal.get('home_id'):
                wanted = 'HOME'
            elif tid == signal.get('away_id'):
                wanted = 'AWAY'
            if wanted:
                break
        if not wanted:
            return 'LOSS'
        if market == '🎯 NEXT GOAL HOME':
            return 'WIN' if wanted == 'HOME' else 'LOSS'
        if market == '🎯 NEXT GOAL AWAY':
            return 'WIN' if wanted == 'AWAY' else 'LOSS'
        return 'WIN'

    # Goal-total market: require two or more goals AND at least one goal
    # beyond the entry state, preventing an instant false WIN.
    if market == '⚽ OVER 1.5 GOALS':
        return 'WIN' if total >= 2 and total > entry_total else 'LOSS'
    if market == '🚀 OVER 1.5 REMAINING GOALS':
        return 'WIN' if total >= entry_total + 2 else 'LOSS'
    ...
    if market == '⚽ LATE GOAL':
        ...
        for ev in data.get('response', []) or []:
            if clean_text(ev.get('type')) != 'goal':
                continue
            em = _safe_num(ev.get('time', {}).get('elapsed'), 0)
            if em >= max(70, entry_minute + 1):
                return 'WIN'
        return 'LOSS'

    # Final match statistics for full-match corners/cards.
    fixture_data = api_get('fixtures', {'id': fid}) or {}
    fixture_rows = fixture_data.get('response', []) or []
    ...
    if market == '🚩 OVER 1.5 CORNERS':
        return 'WIN' if corners >= 2 else 'LOSS'
    if market == '🟨 OVER 1.5 CARDS':
        return 'WIN' if cards >= 2 else 'LOSS'

    if market == '🚩 FIRST HALF OVER 1.5 CORNERS':
        data = api_get('fixtures/events', {'fixture': fid}) or {}
        first_half_corners = 0
        for ev in data.get('response', []) or []:
            if clean_text(ev.get('type')) not in ('corner', 'corner kick', 'corner kicks'):
                continue
            em = _safe_num(ev.get('time', {}).get('elapsed'), 0)
            if em <= 45:
                first_half_corners += 1
        return 'WIN' if first_half_corners >= 2 else 'LOSS'
...
# FINAL AUTHORITATIVE THRESHOLDS
MIN_LIVE_PROBABILITY = LIVE_MIN_PROBABILITY
MIN_LIVE_CONFIDENCE = LIVE_MIN_CONFIDENCE
MAX_LIVE_RISK = LIVE_MAX_RISK
MIN_PREMATCH_PROBABILITY = PREMATCH_MIN_PROBABILITY
MIN_PREMATCH_CONFIDENCE = PREMATCH_MIN_CONFIDENCE
MAX_PREMATCH_RISK = PREMATCH_MAX_RISK
RESULT_SCAN_INTERVAL = 120
...
# BLOCK: CHECK_PENDING_SIGNALS
def check_pending_signals():
    """Resolve completed live signals from the exact stored entry state."""
    try:
        conn=sqlite3.connect(DB_NAME); cur=conn.cursor()
        cur.execute("""SELECT id,fixture_id,home_team,away_team,market,probability,odd,confidence,created_at,
                              minute,home_id,away_id,home_goals,away_goals
                       FROM signals WHERE result IS NULL ORDER BY id ASC LIMIT 100""")
        rows=cur.fetchall(); conn.close(); checked=0
        prefixes=('🎯','⚡','🚩','🟨','⚽')
        for row in rows:
            sid,fid,home,away,market,p,odd,conf,created,minute,home_id,away_id,hg,ag=row
            if not str(market).startswith(prefixes): continue
            signal={'fixture_id':fid,'home_team':home,'away_team':away,'market':market,'probability':p,'odd':odd,'confidence':conf,
                    'created_at':created,'minute':int(minute or 0),'home_id':home_id,'away_id':away_id,'home_goals':int(hg or 0),'away_goals':int(ag or 0)}
            result=evaluate_extended_live_market(signal)
            if result and update_signal_result(sid,result):
                checked+=1; print('RESULT:',home,'-',away,market,result)
        return checked
    except Exception as e:
        logging.warning('EXTENDED RESULT CHECK ERROR: %s',repr(e)); return 0
...
def get_v7_raw_odds(fixture_id):
    """Return a FRESH pre-match odds response.

    PREMATCH prices are never read from a local cache.  The only accepted
    price source is the Betano bookmaker object returned by the odds feed.
    """
    if not fixture_id:
        return []
    try:
        return get_odds(fixture_id) or []
    except Exception as e:
        logging.warning("FRESH BETANO ODDS ERROR | fixture=%s | %s", fixture_id, repr(e))
        return []


def _v7_betano_from_odds(data):
    """Find Betano across the complete odds response. Never fall back to another book."""
    for item in data or []:
        if not isinstance(item, dict):
            continue
        for bookmaker in item.get('bookmakers', []) or []:
...
def get_v7_prematch_markets(fixture_id):
    """Return only real, current full-match Betano markets and exact odds."""
    ...
    for bet in bookmaker.get('bets', []) or []:
        ...
        # Prematch Builder/normal markets are full-match only.
        if any(x in name for x in ('1st half', 'first half', '1h', 'half time')):
            continue

        vals = []
        for v in bet.get('values', []) or []:
            if not isinstance(v, dict) or bool(v.get('stopped', False)):
                continue
            odd = _v7_num(v.get('odd'), 0)
            value = clean_text(v.get('value'))
            if odd <= 1.01 or not value:
                continue
            # Preserve the exact value/odd supplied by the Betano bookmaker feed.
...
def _v7_prematch_candidates(match, detailed=False):
    ...
    if True:
        ...
        if total_y is not None:
            for target in (
                # Deliberately do not offer CARD OVER 2.5.
                # It is too low for this portfolio and was explicitly causing
                # the unwanted prematch card signals.
                'over 3.5',
                'over 4.5',
                'over 5.5',
                'under 5.5',
...
def send_prematch_signal(signal):
    """Final gate: refresh Betano and verify every displayed prematch price."""
    if not isinstance(signal, dict):
        return False
    ...
    if not fresh_markets:
        logging.info('PREMATCH BLOCKED | no fresh Betano feed | fixture=%s', fid)
        return False
    ...
    def exact_odd(label):
        ...
        # 1X2
        if text in ('home win', 'away win', 'draw'):
            want = {'home win': 'home', 'away win': 'away', 'draw': 'draw'}[text]
            for name, vals in fresh_markets:
                if name not in ('match winner', 'winner', '1x2'):
                    continue
                for value, odd in vals:
                    if clean_text(value) == want:
                        return float(odd)
            return None

        # BTTS
        if 'btts' in text or 'both teams' in text:
            value, odd = _v7_find_market(fresh_markets, 'btts', 'yes', half=False)
            return float(odd) if value is not None and odd is not None else None

        # Team-specific goals must match the HOME/AWAY goals market, not the
        # generic total-goals market.
        team_side = None
        if 'home over 1.5' in text:
            team_side = 'home'
        elif 'away over 1.5' in text:
            team_side = 'away'
        if team_side:
            for name, vals in fresh_markets:
                if team_side in clean_text(name) and 'goal' in clean_text(name):
                    for value, odd in vals:
        ...
        m = re.search(r'\b(over|under)\s+([0-9]+(?:\.[05])?)', text)
        if not m:
            return None
        ...
        value, odd = _v7_find_market(fresh_markets, family, target, half=False)
        if value is None or odd is None or clean_text(value) != clean_text(target):
            return None
        return float(odd)


    # Normal signal: replace the displayed price with the freshly verified Betano price.
    if market != '🧩 BET BUILDER':
        odd = exact_odd(market)
        if odd is None:
            logging.info('PREMATCH BLOCKED | exact fresh Betano line missing | fixture=%s | market=%s', fid, market)
            return False
        signal['odd'] = odd
    else:
        # Builder: every leg must be present at the exact current Betano line.
        fresh_legs = []
        for leg in signal.get('builder_legs', []) or []:
            label = str(leg.get('market', ''))
            # Explicitly block the unwanted card over 2.5 line.
            if clean_text(label) == 'card over 2.5':
                logging.info('BUILDER BLOCKED | CARD OVER 2.5')
                return False
            odd = exact_odd(label)
            if odd is None:
                logging.info('BUILDER BLOCKED | exact fresh Betano leg missing | fixture=%s | leg=%s', fid, label)
                return False
            item = dict(leg)
            item['odd'] = round(odd, 2)
            fresh_legs.append(item)

        if len(fresh_legs) < BET_BUILDER_MIN_LEGS:
            return False

        combined = 1.0
        for leg in fresh_legs:
            combined *= float(leg['odd'])
        if combined < BUILDER_MIN_ODD or combined > BUILDER_MAX_ODD:
            return False
        signal['builder_legs'] = fresh_legs
        signal['odd'] = round(combined, 2)
...
def main_loop():
    ...
    except Exception as e: logging.warning('DATABASE INIT ERROR: %s',repr(e)); return
    print_system_status()
    if not api_health_check(): print('❌ API NOT AVAILABLE'); return
    print('✅ API CONNECTION OK')
    LAST_LIVE_SCAN=LAST_PREMATCH_SCAN=LAST_RESULT_SCAN=0
    while True:
        now=time.time(); cleanup_signal_memory()
        if now-LAST_LIVE_SCAN>=LIVE_SCAN_INTERVAL:
            print(datetime.now(TIMEZONE).strftime('%H:%M:%S'),'LIVE SCAN'); sent=_final_live_scan(); print('LIVE SIGNALS SENT:',sent); LAST_LIVE_SCAN=now
        if now-LAST_PREMATCH_SCAN>=PREMATCH_SCAN_INTERVAL:
            print(datetime.now(TIMEZONE).strftime('%H:%M:%S'),'PREMATCH SCAN'); sent=_final_prematch_scan(); print('PREMATCH SIGNALS SENT:',sent); LAST_PREMATCH_SCAN=now
        if now-LAST_RESULT_SCAN>=RESULT_SCAN_INTERVAL:
            checked=check_pending_signals()
            if checked: print('RESULTS UPDATED:',checked)
            LAST_RESULT_SCAN=now
        time.sleep(5)


# LAST-WORD CONFIG: these aliases intentionally come after every historical
# V4/V5/V7 block so runtime filters use the final hotfix thresholds.
MIN_LIVE_PROBABILITY = 68.0
MIN_LIVE_CONFIDENCE = 74.0
MAX_LIVE_RISK = 38.0
MIN_PREMATCH_PROBABILITY = 66.0
MIN_PREMATCH_CONFIDENCE = 70.0
MAX_PREMATCH_RISK = 36.0
RESULT_SCAN_INTERVAL = 120
