...
# ============================================================
# OLD MAIN V3 LIVE ENGINE — CLEAN ISOLATED INTEGRATION
# Prematch + Bet Builder above remain the original base.
# ============================================================


def _oldv3_live_send_telegram(message):
    try:
...
def _oldv3_live_get_live_matches():
    try:
...
def _oldv3_live_get_statistics(fixture_id):
    try:
...
def _oldv3_live_get_odds(fixture_id):
    try:
...
def _oldv3_live_get_match_odds(fixture_id):
    if fixture_id in odds_cache:
        cache_time, data = odds_cache[fixture_id]
    ...
    try:
        ...
        odds = _oldv3_live_get_odds(fixture_id)
        if not odds:
            return None
...
def _oldv3_live_market_available(fixture_id, market):
    try:
        stats = _oldv3_live_get_statistics(fixture_id)
        if len(stats) < 2:
            return False
...
def _oldv3_live_extract(team, stat_name):
    try:
...
def _oldv3_live_calculate_pressure(team):
    pressure = 0
    possession = _oldv3_live_extract(team, 'Ball Possession')
    shots_on = _oldv3_live_extract(team, 'Shots on Goal')
    total_shots = _oldv3_live_extract(team, 'Total Shots')
    corners = _oldv3_live_extract(team, 'Corner Kicks')
    attacks = _oldv3_live_extract(team, 'Dangerous Attacks')
...
def _oldv3_live_calculate_card_pressure(minute, home_fouls, away_fouls, home_yellow, away_yellow, home_red, away_red, home_danger, away_danger):
    pressure = 50
...
def _oldv3_live_analyze_live_match(fixture):
    try:
        ...
        home_stats = stats[0]
        away_stats = stats[1]
        home_red = _oldv3_live_extract(home_stats, 'Red Cards')
        away_red = _oldv3_live_extract(away_stats, 'Red Cards')
        home_pressure = _oldv3_live_calculate_pressure(home_stats)
        away_pressure = _oldv3_live_calculate_pressure(away_stats)
        home_form = _oldv3_live_get_team_form(fixture['teams']['home']['id'], venue='home')
        away_form = _oldv3_live_get_team_form(fixture['teams']['away']['id'], venue='away')
        if home_form:
        ...
        if home_red > away_red:
            home_pressure -= 35
            away_pressure += 20
        home_xg = _oldv3_live_extract(home_stats, 'Expected Goals')
        away_xg = _oldv3_live_extract(away_stats, 'Expected Goals')
        if home_xg >= 1.2:
            home_pressure += 10
        elif home_xg >= 0.8:
            home_pressure += 5
        ...
        if away_red > home_red:
            away_pressure -= 35
            home_pressure += 20
        home_shots_on = _oldv3_live_extract(home_stats, 'Shots on Goal')
        away_shots_on = _oldv3_live_extract(away_stats, 'Shots on Goal')
        ...
        if away_shots_on == 0:
            home_pressure += 5
        home_total_shots = _oldv3_live_extract(home_stats, 'Total Shots')
        away_total_shots = _oldv3_live_extract(away_stats, 'Total Shots')
        if home_shots_on >= 6:
            home_pressure += 5
        if away_shots_on >= 6:
            away_pressure += 5
        home_corners = _oldv3_live_extract(home_stats, 'Corner Kicks')
        away_corners = _oldv3_live_extract(away_stats, 'Corner Kicks')
        home_fouls = _oldv3_live_extract(home_stats, 'Fouls')
        away_fouls = _oldv3_live_extract(away_stats, 'Fouls')
        home_yellow = _oldv3_live_extract(home_stats, 'Yellow Cards')
        away_yellow = _oldv3_live_extract(away_stats, 'Yellow Cards')
        ...
        if minute > 90:
            return None
        card_probability = _oldv3_live_calculate_card_pressure(minute, home_fouls, away_fouls, home_yellow, away_yellow, home_red, away_red, home_pressure, away_pressure)
        ...
        if minute >= 55 and card_probability >= 82 and (home_yellow + away_yellow >= 3) and (home_fouls + away_fouls >= 20):
            if not _oldv3_live_market_available(fixture_id, 'Cards'):
                return None
        ...
        if minute <= 40 and total >= 2 and (goal_diff >= 2) and (max(home_pressure, away_pressure) >= 80):
            if home > away:
                if not _oldv3_live_market_available(fixture_id, 'Next Goal'):
                    return None
        ...
        if minute >= 60 and minute <= 88 and (home_corners + away_corners >= 6) and (home_total_shots + away_total_shots >= 10) and (max(home_pressure, away_pressure) >= 70) and (corner_probability >= 70):
            if not _oldv3_live_market_available(fixture_id, 'Corners'):
                return None
...
def _oldv3_live_get_team_form(team_id, venue=None):
    cache_key = f'{team_id}_{venue}'
    if cache_key in team_form_cache:
        cache_time, data = team_form_cache[cache_key]
...
def _oldv3_live_live_loop():
    ...
    for match in matches:
        ...
        away_xg = 0
        if len(stats) >= 2:
            home_pressure = _oldv3_live_calculate_pressure(stats[0])
            away_pressure = _oldv3_live_calculate_pressure(stats[1])
            home_form = _oldv3_live_get_team_form(match['teams']['home']['id'], venue='home')
            away_form = _oldv3_live_get_team_form(match['teams']['away']['id'], venue='away')
            home_shots = _oldv3_live_extract(stats[0], 'Shots on Goal')
            away_shots = _oldv3_live_extract(stats[1], 'Shots on Goal')
            home_corners = _oldv3_live_extract(stats[0], 'Corner Kicks')
            away_corners = _oldv3_live_extract(stats[1], 'Corner Kicks')
        country = match['league']['country']
        league = match['league']['name']
        odds_text = '-'
        match_odds = _oldv3_live_get_match_odds(fixture_id)
        home_odd = None
        away_odd = None
        if match_odds:
        ...
        _oldv3_live_send_telegram(f"                                \n🔥 LIVE SIGNAL\n\n🏆 {home} vs {away}\n\n🌍 {country}\n🏟 {league}\n\n📊 Score:\n{match['goals']['home'] or 0} - {match['goals']['away'] or 0}\n\n⏱ Minute: {minute}\n\n🔥{signal[0]}🔥\n\n💰 Odds:\n{odds_text}\n\n💎 Confidence: {signal[1]}%\n\n🎯 Goal Probability:\n{goal_probability}%\n")

# Only the Live entry point is replaced.
live_loop = _oldv3_live_live_loop
