...
CACHE_TIME_FORM = 20 * 3600       # one morning-to-morning cycle
CACHE_TIME_ODDS = 600            # 10 minutes
...
def api_get(endpoint, params=None):
    ...
    _quota_add(provider)
    _HIGHLIGHTLY_ENDPOINT_COUNTS[hl_path] = _HIGHLIGHTLY_ENDPOINT_COUNTS.get(hl_path, 0) + 1
    try:
...
def get_match_odds(fixture_id):
    ...
    # -----------------------------------------------------
    # SHARED MORNING CACHE (scanner.py)
    # -----------------------------------------------------
    odds = None
    try:
        import scanner as daily_scanner
        odds = daily_scanner.get_cached_betano_odds(fixture_id)
    except Exception:
        odds = None
    if odds is None:
...
# BLOCK: GET_PREMATCH_MATCHES
def get_prematch_matches():
    """Load the exact daily 12:00 BG -> next-day 12:00 BG window.
    Reuse scanner.py's morning fixture cache before calling Highlightly.
    """
    try:
        import scanner as daily_scanner
        cached = daily_scanner.get_cached_upcoming_matches()
        if cached:
            return remove_started_matches(cached)
    except Exception as exc:
        logging.warning('PREMATCH SHARED CACHE ERROR: %s', repr(exc))
    try:
        ...
        all_matches=[]
        for date_string in sorted({start_dt.strftime('%Y-%m-%d'), end_dt.strftime('%Y-%m-%d')}):
        ...
        for mm in all_matches:
            fid=(mm.get('fixture') or {}).get('id')
            if not fid or fid in seen: continue
            try:
                dt=datetime.fromisoformat(str((mm.get('fixture') or {}).get('date')).replace('Z','+00:00')).astimezone(TIMEZONE)
            except Exception:
                continue
            if start_dt <= dt < end_dt:
                seen.add(fid); out.append(mm)
        return remove_started_matches(out)
    except Exception as e:
        ...
        try:
            ...
            if minutes >= 630 and last_day_football != day:
                try:
                    ...
                    # Reuse the same morning fixture/odds cache. Never start a
                    # second football request chain after a quota lock.
                    if not daily_scanner._quota_locked('football'):
                        scan_prematch()
                        logging.info('FOOTBALL API USAGE TODAY=%s BY_ENDPOINT=%s', _quota_used('football'), dict(sorted(_HIGHLIGHTLY_ENDPOINT_COUNTS.items())))
                    else:
                        logging.warning('PREMATCH SKIPPED: Football API quota is locked')
                except daily_scanner.APIQuotaExceeded as exc:
