...
def _get_cached_scanner_team_matches(team_id):
    ...
    try:
        ...
        games = obj.get('matches') if isinstance(obj, dict) else None
...
# Main-owned morning fixture cache.  This deliberately lives in main.py so
# PREMATCH/Builder can reuse the exact fixture list loaded for the daily
# statistical scan without requiring any change to scanner.py.
_MAIN_MORNING_FIXTURES_CACHE = {}


def _main_morning_fixture_window():
...
def _main_set_morning_fixtures(matches, start, end):
    key = (start.isoformat(), end.isoformat())
    _MAIN_MORNING_FIXTURES_CACHE[key] = list(matches or [])
    logging.info('MAIN MORNING FIXTURE CACHE SET | %s fixtures | %s -> %s', len(matches or []), start, end)


def _main_get_morning_fixtures(start, end):
    key = (start.isoformat(), end.isoformat())
    cached = _MAIN_MORNING_FIXTURES_CACHE.get(key)
    return list(cached) if cached else []


def get_prematch_matches():
    """Reuse the morning fixture list owned by main.py.

    The active daily scanner currently has no cache getter.  Main therefore
    keeps the single morning fixture list and PREMATCH/Builder consume it
    without issuing another fixture-list request.
    """
    try:
        start, end = _main_morning_fixture_window()
        cached = _main_get_morning_fixtures(start, end)
        if cached:
            return cached

        # Compatibility with older scanner versions that expose the cache.
        try:
            import scanner as daily_scanner
            getter = getattr(daily_scanner, 'get_cached_upcoming_matches', None)
            if callable(getter):
                cached = getter(start, end) or []
                if cached:
                    _main_set_morning_fixtures(cached, start, end)
                    return list(cached)
        except Exception as cache_exc:
            logging.warning('PREMATCH SCANNER CACHE ERROR: %s', repr(cache_exc))

        logging.warning('PREMATCH: morning fixture cache is empty; no second fixture-list request')
        return []
    except Exception as e:
...
def scan_live():
    ...
    for match in matches:
        ...
        if key in LEGACY_LIVE_SENT:
            continue

        LEGACY_LIVE_SENT[key] = time.time()
        ...
        if "HOME" in signal[0] and (home_odd is None or home_odd < 1.50):
            continue
        if "AWAY" in signal[0] and (away_odd is None or away_odd < 1.50):
            continue
...
def send_prematch_signal(signal):
    ...
    if market != '🧩 BET BUILDER':
        ...
        if len(legs) < 2 or len(legs) > 4:
            return False
        families = [x.get('family') for x in legs]
        if any(not f for f in families) or len(set(families)) != len(families):
            return False
        if any(float(x.get('edge', 0) or 0) < 2.0 for x in legs):
            return False
        if float(signal.get('edge', -999) or -999) < 3.0:
            return False
...
def scan_prematch():
    try:
        ...
        # Russia/Belarus and blocked league types are excluded before any
        # prematch/builder analysis or odds calls.
        matches=[m for m in (matches or []) if not _main_fixture_blocked(m)]
...
def main_loop():
    ...
    while True:
        ...
        try:
            # ============================================================
            # 1) FOOTBALL PREMATCH + BET BUILDER FIRST
            # ============================================================
            # At/after 10:00 BG, and once per Bulgaria calendar day.
            # This MUST run before Sport daily/statistical work because that
            # work can take a long time and must never delay football signals.
            if minutes >= 600 and last_day_football != day:
                try:
                    logging.info("PREMATCH MORNING FLOW START | %s", day)
                    # Load the fixture window once in main.  This is the only
                    # morning fixture-list request; PREMATCH/Builder reuse it.
                    start_bg, end_bg = _main_morning_fixture_window()
                    cached_for_today = _main_get_morning_fixtures(start_bg, end_bg)
                    if not cached_for_today:
                        getter = getattr(daily_scanner, 'get_cached_upcoming_matches', None)
                        if callable(getter):
                            cached_for_today = getter(start_bg, end_bg) or []
                        else:
                            fetcher = getattr(daily_scanner, 'get_fixtures_for_window', None)
                            if callable(fetcher):
                                cached_for_today = fetcher(start_bg, end_bg) or []
                        _main_set_morning_fixtures(cached_for_today, start_bg, end_bg)

                    prematch_sent = scan_prematch()
                    logging.info("PREMATCH MORNING FLOW END | sent=%s", prematch_sent)
                except Exception as exc:
                    logging.exception("PREMATCH MORNING FLOW ERROR: %s", exc)

                # Mark the football morning flow consumed after PREMATCH.
                # The statistical daily scanner is deliberately executed
                # AFTER LIVE below, so a restart during 17:00-00:00 can never
                # block the first LIVE pass with the long daily scan.
                last_day_football = day

            # ============================================================
            # 2) LIVE — 17:00-00:00 BG, every 5 minutes
            # ============================================================
            # Keep LIVE ahead of the long Sport daily job on every scheduler
            # pass so a restart during the live window can start LIVE first.
            ...
            # ============================================================
            # 3) FOOTBALL STATISTICAL DAILY — once at/after 10:00 BG
            # ============================================================
            # Run after PREMATCH and LIVE. This preserves the existing
            # statistical scanner while preventing it from blocking LIVE
            # when Railway restarts during the 17:00-00:00 window.
            if minutes >= 600 and last_day_football == day:
                # A separate flag prevents this block from running again.
                # Use the same daily DB/run protection inside scanner.py.
                football_daily_key = f"_main_football_daily_done_{day}"
                if globals().get(football_daily_key) is not True:
                    try:
                        if hasattr(daily_scanner, "_quota_locked") and daily_scanner._quota_locked("football"):
                            ...
                            start_bg, end_bg = _main_morning_fixture_window()
                            shared_fixtures = _main_get_morning_fixtures(start_bg, end_bg)
                            original_fixture_loader = getattr(daily_scanner, 'get_fixtures_for_window', None)
                            if shared_fixtures and callable(original_fixture_loader):
                                daily_scanner.get_fixtures_for_window = lambda _start, _end, _fixtures=shared_fixtures: list(_fixtures)
                                try:
                                    daily_scanner.run_daily_scanner('day', send_func=send_telegram)
                                finally:
                                    daily_scanner.get_fixtures_for_window = original_fixture_loader
                            else:
                                daily_scanner.run_daily_scanner('day', send_func=send_telegram)
                        ...
                        globals()[football_daily_key] = True

            # ============================================================
            # 4) SPORT DAILY — once at/after 10:00 BG
            # ============================================================
            # Run after football PREMATCH/LIVE/statistical flow. This
            # preserves the existing Sport scanner and its quota protection.
