...
def process_prematch_matches(
    ...
    """Daily delivery: best 3 singles + best 2 independent Builders."""
    today = datetime.now(TIMEZONE).strftime("%Y-%m-%d")
    normal_sent = 0
    builder_sent = 0
...
def _builder_statistical_candidates(match):
    ...
    if home_stats and away_stats:
        ...
        corner_parts = [
            home_stats.get("corners_for"),
            away_stats.get("corners_against"),
            away_stats.get("corners_for"),
            home_stats.get("corners_against"),
        ]
        corner_parts = [float(x) for x in corner_parts if x is not None]
        if len(corner_parts) >= 2:
            corner_lambda = sum(corner_parts) / len(corner_parts)

        card_parts = [
            home_stats.get("cards_for"),
            away_stats.get("cards_against"),
            away_stats.get("cards_for"),
            home_stats.get("cards_against"),
        ]
        card_parts = [float(x) for x in card_parts if x is not None]
        if len(card_parts) >= 2:
            card_lambda = sum(card_parts) / len(card_parts)
...
def _prematch_consistency_ok(signal, match):
    """Reject obvious contradictions, but never reject merely because a stat is missing."""
    market = clean_text(signal.get("market"))
    teams = match.get("teams", {}) or {}
    h = teams.get("home", {}).get("id")
    a = teams.get("away", {}).get("id")
    hf = get_team_form(h, "home") if h else None
    af = get_team_form(a, "away") if a else None
    if not hf or not af:
        return True
    ...
    if "btts yes" in market or market.endswith("btts"):
        if hb < 45 or ab < 45 or (hb + ab) / 2 < 55 or (hcs + acs) / 2 > 60:
            return False
...
def _prematch_card_ok(signal, match):
    """Cards only when both sides show strong historical card evidence."""
    market = clean_text(signal.get("market"))
    if "card" not in market and "booking" not in market:
        return True
    # analyze_prematch may already have a card model; require explicit strong evidence
    p = _safe_num(signal.get("probability"), 0)
    if p < 70:
        return False
    return True
...
def get_best_prematch_signals(matches):
    """Return only the best 3 coherent singles for the day."""
    candidates = []
    seen = set()
    for match in matches:
        try:
            ...
            for raw in signals:
                ...
                if "BUILDER" in market.upper():
                    continue
                odd = _safe_num(signal.get("odd"), 0)
                prob = _safe_num(signal.get("probability"), 0)
                conf = _safe_num(signal.get("confidence"), 0)
                edge = _safe_num(signal.get("edge"), -999)
                risk = _safe_num(signal.get("risk"), 100)
                if not (PREMATCH_MIN_SINGLE_ODD_FINAL <= odd <= PREMATCH_MAX_SINGLE_ODD_FINAL):
                    continue
                if prob < PREMATCH_MIN_SINGLE_PROB_FINAL or conf < PREMATCH_MIN_SINGLE_CONFIDENCE_FINAL:
                    continue
                if edge < PREMATCH_MIN_SINGLE_EDGE_FINAL or risk > PREMATCH_MAX_SINGLE_RISK_FINAL:
                    continue
                if not _prematch_consistency_ok(signal, match):
                    continue
                if not _prematch_card_ok(signal, match):
                    continue
                signal = dict(signal)
    ...
    return sorted(best_by_fixture.values(), key=lambda x: (_safe_num(x.get("edge"), -999), _safe_num(x.get("probability"), 0), _safe_num(x.get("confidence"), 0), -_safe_num(x.get("risk"), 100)), reverse=True)[:3]


def get_best_bet_builder_signals(matches):
    """Independent Builder Top-2; Builder never consumes single slots."""
    builders = []
    for match in matches:
        try:
            ...
            if not builder:
                continue
            if (_safe_num(builder.get("probability"), 0) < BUILDER_MIN_COMBINED_PROB_FINAL
    ...
    for b in builders:
        ...
        if fid is None:
            continue
        old = best_by_fixture.get(fid)
        if old is None or (_safe_num(b.get("score"),0),_safe_num(b.get("probability"),0),_safe_num(b.get("edge"),-999)) > (_safe_num(old.get("score"),0),_safe_num(old.get("probability"),0),_safe_num(old.get("edge"),-999)):
            best_by_fixture[fid] = b
    selected = sorted(best_by_fixture.values(), key=lambda x: (_safe_num(x.get("score"),0),_safe_num(x.get("probability"),0),_safe_num(x.get("edge"),-999),-_safe_num(x.get("risk"),100)), reverse=True)[:2]
    print(f"BET BUILDER V2 | candidates={len(builders)} | fixtures={len(best_by_fixture)} | selected={len(selected)}")
    return selected
...
MAX_PREMATCH_SIGNALS_PER_SCAN=5
MAX_LIVE_SIGNALS_PER_SCAN=5
# PREMATCH: maximum 3 singles per Bulgarian calendar day
PREMATCH_DAILY_TOP5 = 3

# FINAL QUALITY POLICY — few signals, strong evidence.
PREMATCH_MIN_SINGLE_ODD_FINAL = 1.40
PREMATCH_MAX_SINGLE_ODD_FINAL = 3.50
PREMATCH_MIN_SINGLE_PROB_FINAL = 64.0
PREMATCH_MIN_SINGLE_CONFIDENCE_FINAL = 74.0
PREMATCH_MIN_SINGLE_EDGE_FINAL = 2.0
PREMATCH_MAX_SINGLE_RISK_FINAL = 32.0
BUILDER_MIN_LEG_PROB_FINAL = 66.0
BUILDER_MIN_COMBINED_PROB_FINAL = 48.0
BUILDER_MIN_CONFIDENCE_FINAL = 68.0
BUILDER_MIN_EDGE_FINAL = 1.0
BUILDER_MAX_RISK_FINAL = 34.0
BUILDER_CARD_MIN_LAMBDA_FINAL = 4.8
