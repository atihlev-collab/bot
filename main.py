...
def get_live_market_stats(match):
    ...
    for item in statistics:
        ...
        for st in item.get('statistics',[]) or []:
            ...
            num=_safe_num(val,0)
            target[n]=num
            # API-Football has used slightly different labels across
            # competitions/versions. Keep canonical aliases so Live does
            # not silently see zero corners/cards.
            if n in ('corner kicks','corner','corners'):
                target['corner kicks']=num
            elif n in ('yellow cards','yellow card','yellowcards'):
                target['yellow cards']=num
                target['yellow card']=num
            elif n in ('shots on goal','shots on target'):
                target['shots on goal']=num
            elif n in ('total shots','shots'):
                target['total shots']=num
            elif n in ('dangerous attacks','dangerous attack'):
                target['dangerous attacks']=num
            elif n in ('expected goals','xg'):
                target['expected goals']=num
...
def build_live_market_candidates(match):
    ...
    if not odds:
        logging.debug('LIVE NO BETANO MARKETS fixture=%s', fid)
        return []
