...
def get_v7_raw_odds(fixture_id):
    ...
    if cached:
        ...
        if now - cache_time < RAW_ODDS_CACHE_TTL:
            return data

    data = get_odds(fixture_id) or []
    RAW_ODDS_CACHE[fixture_id] = (now, data)
    return data
    ...
    if detailed:
        ...
        if total_c is not None:
            ...
            for market_name, values in markets:
                ...
                # IMPORTANT: only the REAL full-match total-corners
                # market from Betano. Never use team corners, Asian
                # handicap, 1st-half, or other corner markets.
                ...
                for value, odd in values:
                    ...
                    home_history = get_v7_market_history(home['id'], 'home').get('corners', [])
                    away_history = get_v7_market_history(away['id'], 'away').get('corners', [])
                    history = home_history + away_history

                    if home_history and away_history:

                        home_mean = sum(home_history) / len(home_history)
                        away_mean = sum(away_history) / len(away_history)
                        ...
                        variance_total = (
                            _sample_var(home_history)
                            + _sample_var(away_history)
                        )

                        sd = max(
                            1.50,
                            math.sqrt(max(0.01, variance_total))
                        )
        ...
        if total_y is not None:
            ...
            for market_name, values in markets:
                ...
                # IMPORTANT: only the REAL full-match total-cards
                # market from Betano. Never use team cards, Asian
                # handicap, red-card, or first-half card markets.
                if market_name not in (
                    'cards over under',
                    'cards over/under',
                ...
                for value, odd in values:
                    ...
                    home_history = get_v7_market_history(home['id'], 'home').get('cards', [])
                    away_history = get_v7_market_history(away['id'], 'away').get('cards', [])
                    history = home_history + away_history
...
def _final_prematch_scan():
    try:
        ...
        # V7 is the ONLY prematch selector.
        # It reads the actual Betano catalog first and evaluates only
        # markets/lines that Betano really publishes for this fixture.
        selected = _v7_select_normal(matches) or []
        ...
        # Final builder sanity gate.
        builders = [
            b for b in builders
            if _final_builder_sanity(b)
            and b.get('risk', 100) <= BUILDER_MAX_RISK
            and b.get('confidence', 0) >= BUILDER_MIN_CONFIDENCE
        ]

        builder_sent = 0
        for builder in builders[:BET_BUILDER_DAILY_TOP3]:
            try:
...
def get_match_odds(fixture_id):
    ...
    try:
        ...
        # Use the global helpers. Do NOT define clean_text/safe_float
        # locally here: local shadowing caused UnboundLocalError in
        # deployed versions after partial block edits.
        odds = get_odds(fixture_id)
        if not odds:
            return None
...
# ============================================================
# FINAL LIVE WRAPPER — V7 LIVE ENGINE
# ============================================================
    ...
    try:
        ...
        for match in matches:
            try:
                ...
                if not signal:
                    continue
                if live_signal_quality_filter(signal):
        ...
        sent = 0
        for signal in candidates[:MAX_LIVE_SIGNALS_PER_SCAN]:
            try:
        ...
        print(
            f"LIVE FINAL | matches={len(matches)} | "
...
def main_loop():
    ...
    print("=" * 64)
    print("AI FOOTBALL BOT V9 — CLEAN PREMATCH + PROVEN LIVE")
    print("STATUS: ONLINE")
    print("LIVE ENGINE: FINAL PROVEN ENGINE")
    print("PREMATCH ENGINE: REAL BETANO MARKETS + STATISTICS")
    print("BUILDER: STATISTICS FIRST / 2-5 LEGS")
    print("LIVE:", LIVE_INTERVAL, "sec")
    print("PREMATCH:", PREMATCH_INTERVAL, "sec")
    print("=" * 64)
    ...
    while True:
        ...
        if now - last_prematch >= PREMATCH_INTERVAL:
            try:
                ...
                logging.exception("PREMATCH FINAL LOOP ERROR: %s", repr(e))
            last_prematch = now
