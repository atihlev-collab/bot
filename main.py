...
def get_match_odds(fixture_id):
    ...
    # -----------------------------------------------------
    # PREMATCH ODDS: ALWAYS FRESH
    # -----------------------------------------------------
    # Never reuse an old bookmaker price for a prematch signal.
    # -----------------------------------------------------
    ...
    # Keep the cache only as a diagnostic snapshot. PREMATCH callers above
    # intentionally bypass it and request fresh odds.
...
def get_v7_raw_odds(fixture_id):
    ...
    RAW_ODDS_CACHE[fixture_id] = (time.time(), data)
    return data
