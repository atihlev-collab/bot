...
# LIVE-ONLY statistics cache. PREMATCH code remains untouched.
LIVE_STATS_CACHE = {}
LIVE_STATS_CACHE_TTL = 45

def _live_cached_statistics(fixture_id):
    now = time.time()
    cached = LIVE_STATS_CACHE.get(fixture_id)
    if cached and now - cached[0] < LIVE_STATS_CACHE_TTL:
        return cached[1]
    try:
        stats = _live_cached_statistics(fixture_id)
    except Exception as e:
        print("LIVE STATS CACHE ERROR:", repr(e))
        return []
    if stats:
        LIVE_STATS_CACHE[fixture_id] = (now, stats)
    return stats
...
def analyze_prematch_match(match):
    ...
    try:
        ...
        if (
            away_score >= 78
            and away_over15_probability >= 68
...
"""
        ...
        # Only mark as sent after Telegram call completed.
