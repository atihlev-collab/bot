...
def get_match_odds(fixture_id):
    ...
    # Search every API response item for Betano.
    betano = None
    for item in odds:
...
def get_prematch_betano_builder_markets(fixture_id):
    """Return all usable Betano prematch markets for a fixture.

    API-Football can return bookmaker data in more than one response item,
    so never assume odds[0] is the item containing Betano.
    """
    odds = get_odds(fixture_id)
    if not odds:
        return []
    ...
    for item in odds:
        ...
        if not isinstance(bks, list):
            continue
        bookmaker = _betano_bookmaker(bks)
        if bookmaker:
            break
    ...
    for bet in bookmaker.get("bets", []) or []:
        ...
        for v in bet.get("values", []) or []:
            ...
            if odd is None or odd <= 1.01:
                continue
            # Exact Betano value/odd only. Never synthesize a price.
...
def _builder_statistical_candidates(match):
    """Build candidates ONLY from markets/lines actually returned by Betano.

    The model evaluates a real Betano selection; it never creates a market,
    line or odd. First-half/half-time markets are excluded for Builder.
    """
    ...
    goal_lambdas = _builder_goal_lambdas(match) or (1.20, 1.10)
    home_lam, away_lam = goal_lambdas
    corner_lambda = 9.5
    card_lambda = 4.0
    ...
    home_id = teams.get("home", {}).get("id")
    away_id = teams.get("away", {}).get("id")
    hs = _builder_recent_market_stats(home_id, "home") if home_id else None
    aws = _builder_recent_market_stats(away_id, "away") if away_id else None
    if hs and aws:
        corner_lambda = sum([
            _safe_num(hs.get("corners_for"), 0),
            _safe_num(hs.get("corners_against"), 0),
            _safe_num(aws.get("corners_for"), 0),
            _safe_num(aws.get("corners_against"), 0),
        ]) / 4.0
        card_lambda = sum([
            _safe_num(hs.get("cards_for"), 0),
            _safe_num(hs.get("cards_against"), 0),
            _safe_num(aws.get("cards_for"), 0),
            _safe_num(aws.get("cards_against"), 0),
        ]) / 4.0

    raw = []

    def add(label, odd, prob, family):
        odd = safe_float(odd)
        prob = float(prob)
        if odd is None or odd <= 1.01 or odd > 8.0:
            return
        if prob < BUILDER_MIN_LEG_PROB:
            return
        raw.append({
            "label": label,
            "odd": odd,
            "model_probability": max(0.0, min(99.0, prob)),
            "implied": 100.0 / odd,
            "family": family,
        })

    def parse_line(value):
        m = re.match(r"^(over|under)\\s+([0-9]+(?:\\.[05])?)$", clean_text(value), re.I)
        return (m.group(1).lower(), float(m.group(2))) if m else (None, None)

    for market in markets:
        market_name = clean_text(market.get("name"))
        low = market_name.lower()
        if any(x in low for x in ("1st half", "first half", "1h", "half time", "halftime")):
            continue

        for v in market.get("values", []) or []:
            value = clean_text(v.get("value"))
            odd = safe_float(v.get("odd"))
            if not value or odd is None:
                continue

            # The selection below is ALWAYS the exact Betano value.
            # Only its probability is estimated by our model.
            selection = value.lower()
            family = None
            prob = None
            label_prefix = None

            if "both teams" in low or "btts" in low or "gg/ng" in low:
                if selection in ("yes", "gg", "both teams to score - yes"):
                    prob = (1-math.exp(-home_lam))*(1-math.exp(-away_lam))*100
                    family, label_prefix = "btts", "BTTS"
                elif selection in ("no", "ng", "both teams to score - no"):
                    p_yes = (1-math.exp(-home_lam))*(1-math.exp(-away_lam))
                    prob = (1-p_yes)*100
                    family, label_prefix = "btts", "BTTS"

            elif "total goals" in low or low == "goals" or ("goal" in low and "home" not in low and "away" not in low):
                side, line = parse_line(value)
                if line is not None:
                    lam = home_lam + away_lam
                    prob = _builder_line_probability(side, line, lam)*100
                    family, label_prefix = "goals", "GOAL"

            elif "home team goals" in low or "home goals" in low:
                side, line = parse_line(value)
                if line is not None:
                    prob = _builder_line_probability(side, line, home_lam)*100
                    family, label_prefix = "home_goals", "HOME GOAL"

            elif "away team goals" in low or "away goals" in low:
                side, line = parse_line(value)
                if line is not None:
                    prob = _builder_line_probability(side, line, away_lam)*100
                    family, label_prefix = "away_goals", "AWAY GOAL"

            elif "corner" in low:
                side, line = parse_line(value)
                if line is not None:
                    prob = _builder_line_probability(side, line, corner_lambda)*100
                    family, label_prefix = "corner", "CORNER"

            elif "card" in low or "booking" in low:
                side, line = parse_line(value)
                if line is not None:
                    prob = _builder_line_probability(side, line, card_lambda)*100
                    family, label_prefix = "card", "CARD"

            if family and prob is not None:
                add(f"{label_prefix} {value.upper()}", odd, prob, family)

    # Keep the best real Betano price/selection per exact label.
    best = {}
    for c in raw:
