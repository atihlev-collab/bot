...
# ============================================================
# V3 LIVE SUPPORT — exact constants required by the proven engine
# ============================================================
BASE_URL = "https://v3.football.api-sports.io"
...
def main_loop():
    ...
    print("=" * 60)
    print("AI FOOTBALL BOT V8 — V3 LIVE / EXISTING PREMATCH")
    print("STATUS: ONLINE")
    print("LIVE ENGINE: V3 PROVEN")
    print("PREMATCH ENGINE: EXISTING BASE")
    print("BUILDER: EXISTING BASE")
    print("=" * 60)
...
# ============================================================
# FINAL LIVE OVERRIDE — PROVEN LIVE ENGINE FROM MAIN_V4_FIXED
# ============================================================
# Only LIVE routing/market logic is imported from the proven old main.
# PREMATCH/BUILDER remain from this V8 file.
...
def get_best_live_signal(match):
    try:
        ...
        home_team = teams.get('home', {}).get('name', 'HOME')
        away_team = teams.get('away', {}).get('name', 'AWAY')
        base_signals = analyze_live_match(match)
        if not base_signals:
            return None
        base = max(base_signals, key=lambda x: (x.get('confidence', 0), x.get('probability', 0)))
        market = base['market']
        ...
        xg = base['xg']
        live_odds = parse_live_next_goal_odds(fixture_id, home_team, away_team)
        if not live_odds:
            print('NO LIVE NEXT GOAL ODDS:', home_team, '-', away_team)
            return None
        if market == '🎯 NEXT GOAL HOME':
            odd = live_odds.get('home')
        else:
            odd = live_odds.get('away')
        if odd is None:
            return None
        signal = build_final_live_signal(match, market, probability, confidence, risk, odd, minute, attack, pressure, xg)
        if signal is None:
            return None
        print('LIVE SIGNAL:', signal)
        return signal
    except Exception as e:
        logging.warning('FINAL LIVE SIGNAL ERROR: %s', repr(e))
        return None
...
def scan_live():
    try:
        ...
        for match in matches:
            try:
                signal = get_best_live_signal(match)
                if not signal:
                    continue
        ...
        for signal in candidates:
            try:
                ...
                logging.warning('LIVE SIGNAL ERROR: %s', repr(e))
        return sent
    except Exception as e:
        logging.warning('LIVE SCANNER ERROR: %s', repr(e))
        return 0
    ...
    try:
        ...
        for bet in bookmaker.get('bets', []) or []:
            ...
            for value in bet.get('values', []) or []:
                ...
                if odd is None or odd <= 1.01:
                    continue
                values.append({'value': clean_text(value.get('value')), 'odd': odd, 'stopped': bool(value.get('stopped', False)), 'main': value.get('main')})
            if values:
                markets.append({'name': bname, 'values': values})
        LIVE_MARKET_CACHE[fixture_id] = (now, markets)
        return markets
...
def live_market_probability(match, market, stats):
    ...
    hg = int(_safe_num(goals.get('home'), 0))
    ag = int(_safe_num(goals.get('away'), 0))
    total_goals = hg + ag
    hs = stats.get('home', {})
    aws = stats.get('away', {})
    corners = _stat_total(stats, 'corner kicks')
    ...
    if market in ('🎯 NEXT GOAL HOME', '🎯 NEXT GOAL AWAY', '⚡ FAST NEXT GOAL'):
        side_home = _safe_num(hs.get('shots on goal')) * 8 + _safe_num(hs.get('dangerous attacks')) * 0.5 + _safe_num(hs.get('corner kicks')) * 2 + _safe_num(hs.get('expected goals')) * 8
        side_away = _safe_num(aws.get('shots on goal')) * 8 + _safe_num(aws.get('dangerous attacks')) * 0.5 + _safe_num(aws.get('corner kicks')) * 2 + _safe_num(aws.get('expected goals')) * 8
        if market.endswith('HOME'):
            p = 52 + (side_home - side_away) * 0.22
        elif market.endswith('AWAY'):
            p = 52 + (side_away - side_home) * 0.22
        else:
            p = 50 + max(side_home, side_away) * 0.1
        p += max(0, activity - 45) * 0.2
        if market == '⚡ FAST NEXT GOAL' and 25 <= minute <= 45:
            p += 4
    ...
    if market == '⚽ OVER 1.5 GOALS':
        ...
        p = 35 + activity * 0.42 + remaining * 0.1 + xg * 4
        return round(max(0, min(94, p)), 1)
    if market == '⚽ LATE GOAL':
        if minute < 70:
            return 0
        remaining = max(0, 95 - minute)
        p = 30 + activity * 0.42 + remaining * 0.3 + xg * 5
        if abs(hg - ag) <= 1:
            p += 5
        return round(max(0, min(94, p)), 1)
    if market == '🚩 OVER 1.5 CORNERS':
        if corners >= 2:
            return 99.0
        remaining = max(0, 95 - minute)
        p = 45 + activity * 0.25 + remaining * 0.2
...
def live_market_risk(probability, confidence, minute, market, stats):
    ...
    if minute < 25:
        risk += 12
    if market == '⚡ FAST NEXT GOAL' and (not 25 <= minute <= 45):
        risk += 50
    if market == '⚽ LATE GOAL' and minute < 70:
        risk += 50
    if 'FIRST HALF' in market and minute > 45:
        risk += 50
    if 'CORNER' in market and _stat_total(stats, 'corner kicks') == 0:
        risk += 8
    if 'CARD' in market and _stat_total(stats, 'yellow cards') + _stat_total(stats, 'yellow card') == 0:
        risk += 8
    return min(100, risk)
...
def build_live_market_candidates(match):
    ...
    for target in ('over 1.5', 'over 1.5 goals'):
        ...
        if odd:
            markets.append(('⚽ OVER 1.5 GOALS', odd))
            break
    if minute >= 70:
        for target in ('over 0.5', 'over 0.5 goals'):
            odd = find_live_market_odd(odds, 'goals', target=target)
            if odd:
                markets.append(('⚽ LATE GOAL', odd))
                break
    odd = find_live_market_odd(odds, 'corners', target='over 1.5')
    if odd:
        markets.append(('🚩 OVER 1.5 CORNERS', odd))
    if minute <= 45:
        odd = find_live_market_odd(odds, 'corners', half=True, target='over 1.5')
        if odd:
            markets.append(('🚩 FIRST HALF OVER 1.5 CORNERS', odd))
    odd = find_live_market_odd(odds, 'cards', target='over 1.5')
    if odd:
        markets.append(('🟨 OVER 1.5 CARDS', odd))
    candidates = []
    for market, odd in markets:
        if odd is None or odd < LIVE_MIN_ODD or odd > LIVE_MAX_ODD:
            continue
        ...
        if market in ('🎯 NEXT GOAL HOME', '🎯 NEXT GOAL AWAY'):
            ...
            if other_odd and other_odd > 1.01:
                ...
                if market_probs[idx] is not None:
                    p = _blend_probability(p, market_probs[idx], 0.8)
        c = live_market_confidence(p, market, minute, stats)
        r = live_market_risk(p, c, minute, market, stats)
        edge = value_edge(p, odd)
        if p < MIN_LIVE_PROBABILITY or c < MIN_LIVE_CONFIDENCE or r > MAX_LIVE_RISK:
            continue
        fixture_data = match.get('fixture', {})
        teams = match.get('teams', {})
        league = match.get('league', {})
        candidates.append({'fixture_id': fid, 'home_team': teams.get('home', {}).get('name', 'HOME'), 'away_team': teams.get('away', {}).get('name', 'AWAY'), 'league': league.get('name', ''), 'country': league.get('country', ''), 'market': market, 'probability': p, 'confidence': c, 'risk': r, 'odd': round(odd, 2), 'edge': round(edge, 1), 'score': live_signal_score(p, c, edge, r), 'minute': minute, 'attack': round(_stat_total(stats, 'total shots') * 1.0 + _stat_total(stats, 'dangerous attacks') * 0.1, 1), 'pressure': round(_stat_total(stats, 'corner kicks') * 4 + _stat_total(stats, 'shots on goal') * 8, 1), 'xg': round(_stat_total(stats, 'expected goals'), 2), 'home_goals': int(_safe_num(match.get('goals', {}).get('home'), 0)), 'away_goals': int(_safe_num(match.get('goals', {}).get('away'), 0)), 'home_id': match.get('teams', {}).get('home', {}).get('id'), 'away_id': match.get('teams', {}).get('away', {}).get('id')})
    return candidates


# ============================================================
# FINAL BUILDER OVERRIDE — REAL MODEL PROBABILITY / SMALL LEG ODDS OK
# ============================================================
BET_BUILDER_MIN_LEGS = 2
BET_BUILDER_MAX_LEGS = 5
BET_BUILDER_DAILY_TOP3 = 2
BUILDER_MIN_LEG_PROB = 68.0
BUILDER_MIN_LEG_CONF = 70.0
BUILDER_MAX_RISK = 38.0
BUILDER_MIN_ODD = 1.25          # a leg may be below 1.40
BET_BUILDER_MAX_ODD = 6.00
...
def _final_builder_candidates(match):
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        label = str(row.get('market') or row.get('label') or '').strip()
        upper = label.upper()
        if not label:
            continue
        if any(x in upper for x in ('TOUCH', 'THROW', 'UNDER 4.5')):
            continue
        if not any(x in upper for x in (
            'GOAL', 'BTTS', 'CORNER', 'CARD', 'HOME WIN', 'AWAY WIN'
        )):
            continue
        ...
        # IMPORTANT: odds do not create model probability.
        if odd is None or odd < BUILDER_MIN_LEG_ODD or odd > 3.50:
            continue
        if prob is None or prob < BUILDER_MIN_LEG_PROB or prob > 95.0:
            continue
        if conf is None or conf < BUILDER_MIN_LEG_CONF:
            continue
        if risk is None or risk > BUILDER_MAX_RISK:
        ...
        if not family:
            family = (
                'goals' if ('GOAL' in upper or 'BTTS' in upper) else
                'corners' if 'CORNER' in upper else
                'cards' if 'CARD' in upper else
                'result'
            )

        key = (upper, round(float(odd), 3))
        if key in seen:
            continue
        seen.add(key)
...
def build_best_bet_builder(match, detailed=True):
    ...
    for n in range(2, max_n + 1):
        for combo in combinations(candidates, n):
            families = [x['family'] for x in combo]
            # One market family per leg; prevents stacking near-duplicate goals/corners.
            if len(set(families)) != len(families):
                continue
            ...
            if combined_odd < 1.50 or combined_odd > BET_BUILDER_MAX_ODD:
                continue

            # Real model probabilities only. Never derive probability from odds.
            joint = 1.0
            for leg in combo:
                joint *= leg['probability'] / 100.0

            # Conservative dependence discounts.
            ...
            joint_pct = min(92.0, joint * 100.0)
            implied_pct = 100.0 / combined_odd
            edge = joint_pct - implied_pct
            ...
            # Prefer strong joint probability, then value and confidence.
            score = (
                joint_pct * 0.60
                + edge * 0.25
                + confidence * 0.15
            ...
            if best is None or score > best[0]:
                best = (score, combo, combined_odd, joint_pct, confidence, risk, edge)

    if best is None:
        return None

    score, combo, combined_odd, joint_pct, confidence, risk, edge = best
    fixture = match.get('fixture', {}) or {}
    teams = match.get('teams', {}) or {}
    league = match.get('league', {}) or {}
    ...
    return {
        ...
        'edge': round(edge, 1),
        'ev': round(joint_pct / 100.0 * combined_odd - 1.0, 3),
        'score': round(score, 2),
        'match_date': fixture.get('date'),
        'builder_legs': [
            {
...
def _final_builder_sanity(signal):
    ...
    for leg in legs:
        label = str(leg.get('market', '')).upper()
        if any(x in label for x in ('UNDER 4.5', 'TOUCH', 'THROW')):
            return False
        p = safe_float(leg.get('probability'))
        o = safe_float(leg.get('odd'))
        if p is None or o is None or p <= 0 or o < BUILDER_MIN_LEG_ODD:
            return False
    return True


# ============================================================
# FINAL PREMATCH SCAN OVERRIDE — TOP 3 + TOP 2 BUILDERS
# ============================================================
def _final_prematch_scan():
    try:
        matches = remove_started_matches(get_prematch_matches()) or []
        ...
        try:
            ...
            logging.warning("PREMATCH SELECTOR ERROR: %s", repr(e))
            selected = []

        # Hard cap: best 3 normal prematch signals.
        selected = sorted(
            selected,
            key=lambda x: (safe_float(x.get('score'), 0), safe_float(x.get('confidence'), 0), -safe_float(x.get('risk'), 100)),
            reverse=True,
        )[:3]
        ...
        for m in matches:
            try:
                b = build_best_bet_builder(m, True)
                if b and _final_builder_sanity(b):
                    builders.append(b)
            except Exception as e:
                logging.warning("BUILDER ERROR: %s", repr(e))

        # One builder per fixture, then rank the best two for the day.
        unique = {}
        for b in builders:
            fid = b.get('fixture_id')
            if fid is None:
                continue
            if fid not in unique or b.get('score', 0) > unique[fid].get('score', 0):
                unique[fid] = b
        builders = list(unique.values())
        builders.sort(
            key=lambda x: (safe_float(x.get('score'), 0), safe_float(x.get('probability'), 0), safe_float(x.get('confidence'), 0)),
            reverse=True,
        )
        builders = builders[:2]
        ...
        print(
            f"PREMATCH FINAL | fixtures={len(matches)} | selected={len(selected)} | "
            f"builder={len(builders)} | sent={sent + builder_sent}"
        )
        return sent + builder_sent
    except Exception as e:
        logging.exception("PREMATCH FINAL ERROR: %s", repr(e))
...
# ============================================================
# FINAL SINGLE SCHEDULER — ONE LIVE ENGINE
# ============================================================
LIVE_INTERVAL = 60
...
def main_loop():
    init_database()
    print("=" * 64)
    print("🤖 AI FOOTBALL BOT V8 — FINAL SINGLE ENGINE")
    print("STATUS: ONLINE")
    print("LIVE ENGINE: PROVEN MAIN_V4 LIVE MARKET ENGINE")
    print("PREMATCH: TOP 3")
    print("BUILDER: TOP 2 / 2–5 LEGS / LEG ODDS >= 1.25")
    print("LIVE: 60 sec")
    print("PREMATCH: 300 sec")
    print("=" * 64)
    ...
    while True:
        ...
        try:
            ...
            logging.exception("LIVE LOOP ERROR: %s", repr(e))
...
# ============================================================
# FINAL V10 PATCH — PROVEN LIVE ENGINE FROM MAIN_V4_FIXED
# ============================================================
# This section intentionally overrides duplicate LIVE definitions above.
# Only one LIVE engine is active after this point.

# Proven LIVE thresholds from MAIN_V4_FIXED.
LIVE_MINUTE = 25
LIVE_SCAN_INTERVAL = 30
MAX_LIVE_SIGNALS_PER_SCAN = 5
...
# Builder: small legs are allowed because they are combined with another leg.
# Standalone PREMATCH signals still use the separate >=1.40 filter.
BUILDER_MIN_LEG_ODD = 1.25
BET_BUILDER_MIN_LEGS = 2
BET_BUILDER_MAX_LEGS = 5
BET_BUILDER_DAILY_TOP3 = 2

# These two reporting hooks were referenced by the previous scheduler but are
# not required for signal generation. Safe no-op fallbacks prevent a missing
# helper from breaking PREMATCH.
...
# --- proven LIVE: analyze_live_match ---
...
# --- proven LIVE: parse_live_next_goal_odds ---
def parse_live_next_goal_odds(
    fixture_id,
    home_team,
    away_team
...
# --- proven LIVE: adjust_live_probability ---
def adjust_live_probability(
    probability,
    odd
):
...
# --- proven LIVE: build_final_live_signal ---
def build_final_live_signal(
    match,
    market,
    probability,
...
# --- proven LIVE: get_best_live_signal ---
def get_best_live_signal(match):
    try:
        candidates=build_live_market_candidates(match)
        if not candidates:
...
# --- proven LIVE: live_signal_allowed ---
def live_signal_allowed(
    fixture_id,
    market,
    minute
...
# --- proven LIVE: process_live_matches ---
def process_live_matches(
    matches
):
...
# --- proven LIVE: live_signal_quality_filter ---
def live_signal_quality_filter(
    signal
):
...
# ============================================================
# FINAL BUILDER / PREMATCH QUALITY PATCH
# ============================================================
def _final_builder_candidates(match):
    try:
        rows = _v7_prematch_candidates(match, detailed=True) or []
    ...
    for row in rows:
        ...
        label = str(row.get('market') or row.get('label') or '').strip()
        upper = label.upper()
        if not label or any(x in upper for x in ('TOUCH', 'THROW', 'UNDER 4.5')):
            continue
        if not any(x in upper for x in ('GOAL', 'BTTS', 'CORNER', 'CARD', 'HOME WIN', 'AWAY WIN')):
            continue
        ...
        if odd is None or odd < BUILDER_MIN_LEG_ODD or odd > 3.50:
            continue
        # A 97–99% model probability is treated as an unreliable/model-artifact
        # value for Builder purposes and is rejected rather than displayed.
        if prob is None or prob < BUILDER_MIN_LEG_PROB or prob > 92.0:
            continue
        if conf is None or conf < BUILDER_MIN_LEG_CONF:
            continue
        if risk is None or risk > BUILDER_MAX_RISK:
        ...
        family = row.get('family')
        if not family:
            family = ('goals' if ('GOAL' in upper or 'BTTS' in upper) else
                      'corners' if 'CORNER' in upper else
                      'cards' if 'CARD' in upper else 'result')
        key = (upper, round(float(odd), 3))
        if key in seen:
            continue
        seen.add(key)
        x = dict(row)
        x.update({'market': label, 'odd': float(odd), 'probability': float(prob),
                  'confidence': float(conf), 'risk': float(risk), 'family': family})
        out.append(x)
    return out
...
def _final_prematch_scan():
    try:
        ...
        # Standalone PREMATCH: hard minimum 1.40 and TOP 3 only.
        filtered = []
        for s in selected:
            odd = safe_float(s.get('odd'))
            if odd is None or odd < 1.40:
                continue
            filtered.append(s)
        filtered.sort(key=lambda x: (safe_float(x.get('score'), 0),
                                     safe_float(x.get('confidence'), 0),
                                     -safe_float(x.get('risk'), 100)), reverse=True)
        selected = filtered[:3]
        ...
        # Builder: TOP 2 for the whole day, not 2 new ones every scan.
        builders = []
        if day not in DAILY_BUILDER_SENT:
            for m in matches:
                try:
            ...
            for b in builders:
                fid = b.get('fixture_id')
                if fid is not None and (fid not in unique or b.get('score', 0) > unique[fid].get('score', 0)):
                    unique[fid] = b
            builders = list(unique.values())
            builders.sort(key=lambda x: (safe_float(x.get('score'), 0),
                                         safe_float(x.get('probability'), 0),
                                         safe_float(x.get('confidence'), 0)), reverse=True)
            builders = builders[:2]
        ...
        print(f"PREMATCH FINAL | fixtures={len(matches)} | selected={len(selected)} | "
              f"builder={len(builders)} | sent={sent + builder_sent}")
        return sent + builder_sent
    except Exception as e:
        logging.exception("PREMATCH FINAL ERROR: %s", repr(e))
...
# ============================================================
# FINAL V10 SCHEDULER — PREMATCH + ONE PROVEN LIVE ENGINE
# ============================================================
PREMATCH_INTERVAL = 300
...
def main_loop():
    init_database()
    print("=" * 64)
    print("🤖 AI FOOTBALL BOT V10 — FINAL")
    print("STATUS: ONLINE")
    print("LIVE ENGINE: MAIN_V4_FIXED PROVEN ENGINE")
    print("PREMATCH: TOP 3")
    print("BUILDER: TOP 2 / 2–5 LEGS / LEG ODDS >= 1.25")
    print("LIVE: 30 sec")
    print("PREMATCH: 300 sec")
