...
def _final_builder_candidates(match):
    """Build Builder legs from real Betano lines and existing PREMATCH model.

    Never derive model probability from odds.  Goal markets can be combined when
    they are different selections; corner/card lines are used only when the
    PREMATCH engine actually provides a matching model signal.
    """
    fid = match.get('fixture', {}).get('id')
    if not fid:
        return []
    markets = get_prematch_betano_builder_markets(fid) or []
    model_signals = analyze_prematch(match) or []
    if isinstance(model_signals, dict):
        model_signals = [model_signals]

    def norm(x):
        return clean_text(x).replace('🧩', '').strip().lower()
    ...
    def model_for(label):
        ...
        for sig in model_signals:
            ...
            if sm in keys or any(k in sm for k in keys):
                ...
                if pp is None or cc is None or cc < 70:
                    continue
    ...
    out = []
    for m in markets:
        name = clean_text(m.get('name')).lower()
        if any(x in name for x in ('1st half', 'first half', '1h')):
            continue
        if 'corner' in name:
            family, prefix = 'corner', 'CORNER '
        elif 'card' in name or 'booking' in name or 'yellow' in name:
            family, prefix = 'card', 'CARD '
        ...
        for v in m.get('values', []) or []:
            value = clean_text(v.get('value')).lower()
            odd = safe_float(v.get('odd'))
            if odd is None or odd < BUILDER_LEG_MIN_ODD_FINAL or odd > BUILDER_LEG_MAX_ODD_FINAL:
                continue
            if family in ('corner', 'card') and not value.startswith(('over ', 'under ')):
                continue
            if family == 'goal' and not (
                value.startswith(('over 1.5', 'under 2.5', 'under 3.5'))
                or value in ('btts yes', 'btts no', 'yes', 'no')
            ):
                continue
            label = f'{prefix}{value}' if prefix else value.upper()
            pp = model_for(label)
            if pp is None:
                continue
            pp = min(BUILDER_MAX_LEG_PROB_FINAL, max(5.0, pp))
            if pp < BUILDER_MIN_PROB_FINAL:
                continue
            out.append({'label': label, 'odd': float(odd), 'probability': round(pp, 1), 'family': family})
...
def build_best_bet_builder(match):
    ...
    for n in (2, 3):
        ...
        for combo in combinations(candidates, n):
            ...
            if joint_probability < 45.0 or joint_probability > 88.0:
                continue
            fair_probability = 100.0 / combined_odd
            edge = joint_probability - fair_probability
            ev = (joint_probability / 100.0) * combined_odd - 1.0
            if edge < 1.5 or ev < 0.015:
                continue
            # Penalise same-family combinations because they are more correlated.
            families = [x['family'] for x in combo]
            correlation_penalty = 3.0 if len(set(families)) == 1 else 0.0
            score = joint_probability * 0.65 + edge * 2.5 + min(20.0, (combined_odd - 1.40) * 10.0) - correlation_penalty - (n - 2) * 2.0
            if best is None or score > best[0]:
                best = (score, combo, combined_odd, joint_probability, edge, ev)

    if best is None:
        return None
    score, combo, combined_odd, joint_probability, edge, ev = best
    confidence = min(88.0, round(sum(x['probability'] for x in combo) / len(combo) - (len(combo) - 2) * 2.0, 1))
    ...
    return {
        ...
        'ev': round(ev, 3),
        'score': round(score, 2),
        'builder_legs': [{'market': x['label'], 'odd': round(x['odd'], 2), 'probability': round(x['probability'], 1)} for x in combo],
    }
...
def _final_standalone_allowed(signal):
    ...
    if p >= 64.0 and c >= 75.0 and r <= 34.0 and edge >= 3.0:
        return True
    # Secondary gate prevents an empty board while retaining a meaningful edge.
    return p >= 60.0 and c >= 70.0 and r <= 36.0 and edge >= 1.5
...
def get_best_prematch_signals(matches):
    ...
    for s in standalone:
        ...
        key = (_safe_num(s.get('edge'), -999), _safe_num(s.get('probability'), 0), _safe_num(s.get('confidence'), 0), -_safe_num(s.get('risk'), 100))
    ...
    top3 = sorted(by_fixture.values(), key=lambda s: (_safe_num(s.get('edge'), -999), _safe_num(s.get('probability'), 0), _safe_num(s.get('confidence'), 0), -_safe_num(s.get('risk'), 100)), reverse=True)[:PREMATCH_TOP3_FINAL]
    builders.sort(key=lambda s: (_safe_num(s.get('edge'), -999), _safe_num(s.get('ev'), -999), _safe_num(s.get('probability'), 0), _safe_num(s.get('confidence'), 0)), reverse=True)
