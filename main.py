...
def calculate_dynamic_stake(confidence):
    if confidence >= 80: return "🔥 СИЛЕН ЗАЛОГ: 3.5% от банката"
    elif confidence >= 65: return "💰 СРЕДЕН ЗАЛОГ: 2.0% от банката"
    return "⚠️ КОНСЕРВАТИВЕН ЗАЛОГ: 1.0% от банката"

def calculate_radar_pressure(team_stats_list, goals_scored, elapsed_minute):
    """ ИСТИНСКИ БЕТ365 РАДАР: Изчислява точен индекс на офанзивен натиск в минута """
