...
# RAPIDAPI COMPATIBLE STANDARD EDITION - BULLETPROOF FIXED
...
def safe_api_get(endpoint, params=None):
    try:
        # Изчистване на наклонени черти от началото на ендпойнта, за да няма дублиране
        clean_endpoint = endpoint.lstrip('/')
        
        # БАВНО ПРОВЕРЕН И ФИКСИРАН ОФИЦИАЛЕН АДРЕС ЗА RAPIDAPI АБОНАМЕНТ
