...
def build_live_market_candidates(match):
    ...
    # LIVE odds are optional. API-Football /odds/live frequently returns
    # no bookmaker data or is rate-limited. Do not kill the statistical
    # live engine just because live odds are unavailable.
    try:
        odds = get_live_betano_markets(fid) or []
    except Exception as e:
        logging.warning('LIVE ODDS ERROR | fixture=%s | %s', fid, repr(e))
        odds = []
