...
def _blend_probability(model_probability, market_probability, model_weight=0.75):
    ...
    return round(max(0.0, min(100.0,
        model_probability * model_weight + market_probability * (1.0 - model_weight)
    )), 1)
...
def get_best_prematch_signals(matches):
    ...
    for match in matches:
        try:
            ...
            if builder and builder['confidence']>=78 and builder['risk']<=30 and builder['odd']>=BET_BUILDER_MIN_ODD:
            ...
            for s in list(candidates):
                ...
                # ODDS DROP DISABLED: a first-seen in-memory price is not a
                # reliable opening price. This prevents false falling-odds signals.
        except Exception as e:
...
def get_best_prematch_signals(matches):
    ...
    for match in matches:
        try:
            ...
            for raw in signals:
                ...
                fid = signal.get('fixture_id') or match.get('fixture', {}).get('id')
                signal['fixture_id'] = fid
                key = (fid, signal.get('market'))
                if key in seen:
                    continue
                seen.add(key)
            ...
            # Builder is evaluated separately. If it is good enough, its own
            # probability is used in the same final ranking.
            builder = build_best_bet_builder(match)
            if builder:
                builder = dict(builder)
                if prematch_signal_allowed(builder):
                    builder['score'] = prematch_signal_score(builder)
                    candidates.append(builder)

        except Exception as e:
            logging.warning('PREMATCH PROBABILITY RANK ERROR: %s', repr(e))

    # Highest probability first. Confidence/risk are tie-breakers only.
    candidates.sort(
        key=lambda x: (
            _safe_num(x.get('probability'), 0),
            _safe_num(x.get('confidence'), 0),
            _safe_num(x.get('edge'), 0),
    ...
    selected = []
    for signal in candidates:
        # One signal per exact market/fixture. Different markets may still
        # compete normally for the top-5 places.
...
# ============================================================
# FINAL ENTRY POINT — ONE LOOP ONLY
# LIVE + PREMATCH 11:00 + PREMATCH NIGHT 21:00
# ============================================================

PREMATCH_DAY_HOUR = 11
PREMATCH_NIGHT_HOUR = 21
PREMATCH_SCHEDULE_MINUTE = 0
PREMATCH_TOP_NORMAL = 3
PREMATCH_TOP_BUILDERS = 2


def _run_prematch_package(label, matches, include_builders=True):
    """Run one scheduled prematch package without killing the main loop."""
    try:
        matches = remove_started_matches(matches or []) or []
        print(f"{label} FIXTURES: {len(matches)}")
        if not matches:
            print(f"{label} COMPLETE | no upcoming fixtures")
            return 0

        # The final get_best_prematch_signals() already contains the quality
        # engine and the Bet Builder candidate.  We split the result here so
        # the Builder cannot be pushed out by ordinary markets.
        candidates = get_best_prematch_signals(matches) or []

        builders = [
            s for s in candidates
            if str(s.get('market', '')).strip() == '🧩 BET BUILDER'
        ]
        normal = [
            s for s in candidates
            if str(s.get('market', '')).strip() != '🧩 BET BUILDER'
        ...
        normal = normal[:PREMATCH_TOP_NORMAL]
        builders = builders[:PREMATCH_TOP_BUILDERS] if include_builders else []
        ...
        for signal in normal:
            try:
                ...
                logging.warning("PREMATCH NORMAL SEND ERROR: %s", repr(exc))
        ...
        for signal in builders:
            try:
                ...
                logging.warning("PREMATCH BUILDER SEND ERROR: %s", repr(exc))

        print(
            f"{label} COMPLETE | "
            f"normal={len(normal)} | builders={len(builders)} | "
            f"sent_normal={sent_normal} | sent_builder={sent_builder}"
        )
        return sent_normal + sent_builder

    except Exception as exc:
        logging.exception("%s ERROR: %s", label, repr(exc))
        return 0


def _run_prematch_day_package():
    try:
        matches = get_prematch_matches() or []
        return _run_prematch_package(
            "☀️ PREMATCH 11:00",
            matches,
            include_builders=True,
        )
    except Exception as exc:
        logging.exception("PREMATCH DAY ERROR: %s", repr(exc))
        return 0


def _run_prematch_night_package():
    """21:00 BG package: only tomorrow 00:00–09:59 BG, top 3 normal."""
    try:
        now_dt = datetime.now(TIMEZONE)
        tomorrow = (now_dt + timedelta(days=1)).strftime("%Y-%m-%d")
        data = api_get(
            "fixtures",
            {"date": tomorrow, "timezone": str(TIMEZONE)},
        ) or {}
        all_matches = data.get("response", []) or []

        night_matches = []
        for match in all_matches:
            try:
                dt_text = match.get("fixture", {}).get("date")
                if not dt_text:
                    continue
                ...
                logging.warning("NIGHT TIME FILTER ERROR: %s", repr(exc))

        # Night package intentionally sends top 3 normal prematch signals.
        return _run_prematch_package(
            "🌙 PREMATCH 21:00 — 00:00-09:59",
            night_matches,
            include_builders=False,
        )
    except Exception as exc:
        logging.exception("PREMATCH NIGHT ERROR: %s", repr(exc))
        return 0
...
def main_loop():
    ...
    if not api_health_check():
        print("❌ API NOT AVAILABLE — retrying from loop")
    else:
        print("✅ API CONNECTION OK")

    # Start live immediately after deployment. Scheduled prematch packages
    # remain tied to Bulgarian local time and cannot be blocked by LIVE errors.
    LAST_LIVE_SCAN = 0
    LAST_PREMATCH_SCAN = time.time()
    LAST_RESULT_SCAN = 0

    day_slot_sent = None
    night_slot_sent = None

    print("============================================================")
    print("🤖 FINAL MAIN — LIVE + PREMATCH + BUILDER")
    print("LIVE: 60 sec")
    print("PREMATCH: 11:00 BG + 21:00 BG")
    print("11:00: TOP 3 + TOP 2 BET BUILDERS")
    print("21:00: TOP 3 for tomorrow 00:00-09:59 BG")
    print("============================================================")
    ...
    while True:
        ...
        try:
            ...
            logging.warning("MEMORY CLEANUP ERROR: %s", repr(exc))
        ...
        try:
            ...
            if now - LAST_LIVE_SCAN >= LIVE_SCAN_INTERVAL:
                ...
                try:
                    ...
                    logging.exception("LIVE SCAN ERROR: %s", repr(exc))
                LAST_LIVE_SCAN = now
        except Exception as exc:
            logging.exception("LIVE LOOP ERROR: %s", repr(exc))
        ...
        try:
            ...
            if (
                now_dt.hour == PREMATCH_DAY_HOUR
                and now_dt.minute == PREMATCH_SCHEDULE_MINUTE
                and day_slot_sent != slot
            ):
                _run_prematch_day_package()
                day_slot_sent = slot
                LAST_PREMATCH_SCAN = time.time()

            if (
                now_dt.hour == PREMATCH_NIGHT_HOUR
                and now_dt.minute == PREMATCH_SCHEDULE_MINUTE
                and night_slot_sent != slot
            ):
                _run_prematch_night_package()
                night_slot_sent = slot
                LAST_PREMATCH_SCAN = time.time()
        except Exception as exc:
            logging.exception("PREMATCH SCHEDULE ERROR: %s", repr(exc))
        ...
        try:
            ...
            if now - LAST_RESULT_SCAN >= RESULT_SCAN_INTERVAL:
                try:
                    ...
                    logging.warning("RESULT SCAN ERROR: %s", repr(exc))
                LAST_RESULT_SCAN = now
        except Exception as exc:
            logging.warning("RESULT LOOP ERROR: %s", repr(exc))

        # Short sleep keeps the exact 11:00/21:00 minute from being missed,
        # while LIVE remains protected by its own interval.
        elapsed = time.time() - loop_started
        time.sleep(max(1, min(5, 5 - elapsed)))


if __name__ == "__main__":
    print("MAIN V6 STARTED")
    try:
        main_loop()
    except KeyboardInterrupt:
        print("BOT STOPPED")
    except Exception as exc:
        logging.exception("FATAL MAIN ERROR: %s", repr(exc))
        raise
