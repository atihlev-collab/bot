...
# ============================================================
# CLEAN V3 LIVE ENGINE — ISOLATED
# Everything above this marker is the original PREMATCH/BUILDER base.
# Only the LIVE entry point is redirected below.
# ============================================================
def v3_live_send_telegram(message):
    try:
        r = requests.post(f'https://api.telegram.org/bot{BOT_TOKEN}/sendMessage', json={'chat_id': CHAT_ID, 'text': message}, timeout=20)
...
def v3_live_get_live_matches():
    try:
        r = requests.get(f'{BASE_URL}/fixtures', headers=HEADERS, params={'live': 'all'}, timeout=20).json()
        return r.get('response', [])
    except:
        return []

def v3_live_get_statistics(fixture_id):
    try:
        r = requests.get(f'{BASE_URL}/fixtures/statistics', headers=HEADERS, params={'fixture': fixture_id}, timeout=20).json()
        print('STATS RAW:', fixture_id, r)
        return r.get('response', [])
    except:
        return []

def v3_live_get_odds(fixture_id):
    try:
        r = requests.get(f'{BASE_URL}/odds', headers=HEADERS, params={'fixture': fixture_id}, timeout=20).json()
        return r.get('response', [])
    except:
        return []

def v3_live_get_match_odds(fixture_id):
    if fixture_id in odds_cache:
        cache_time, data = odds_cache[fixture_id]
    ...
    try:
        ...
        odds = v3_live_get_odds(fixture_id)
        if not odds:
            return None
        ...
        betano = None
        for bookmaker in bookmakers:
            if bookmaker.get('id') == 32 or str(bookmaker.get('name', '')).lower() == 'betano':
                betano = bookmaker
                break
        if betano is None:
            ...
            if bet_name in ('match winner', '1x2', 'winner'):
                for value in values:
                    ...
                    if value_name == 'home':
                        ...
                        away_odd = odd
            if 'both teams to score' in bet_name or bet_name == 'btts':
                for value in values:
                    if clean_text(value.get('value')) in ('yes', 'btts yes'):
                        btts_odd = safe_float(value.get('odd'))
            is_full_goal_total = bet_name in ('goals over/under', 'over/under', 'total goals') or ('goal' in bet_name and 'over/under' in bet_name and ('home' not in bet_name) and ('away' not in bet_name) and ('team' not in bet_name) and ('first half' not in bet_name) and ('second half' not in bet_name))
            if is_full_goal_total:
                for value in values:
                    ...
                    if value_name in ('over 2.5', 'over 2.5 goals'):
                        ...
                        over35_odd = odd
            is_home_total = 'home' in bet_name and 'total' in bet_name and ('goal' in bet_name) or bet_name in ('home team over/under', 'home team total', 'home goals over/under')
            is_away_total = 'away' in bet_name and 'total' in bet_name and ('goal' in bet_name) or bet_name in ('away team over/under', 'away team total', 'away goals over/under')
            if is_home_total:
                for value in values:
                    value_name = clean_text(value.get('value'))
...
def v3_live_extract(team, stat_name):
    try:
...
def v3_live_calculate_pressure(team):
    pressure = 0
    possession = v3_live_extract(team, 'Ball Possession')
    shots_on = v3_live_extract(team, 'Shots on Goal')
    total_shots = v3_live_extract(team, 'Total Shots')
    corners = v3_live_extract(team, 'Corner Kicks')
    attacks = v3_live_extract(team, 'Dangerous Attacks')
...
def v3_live__norm_market_text(value):
    return ' '.join(str(value or '').strip().lower().replace('_', ' ').split())

def v3_live_get_betano_market_catalog(fixture_id):
    ...
    try:
        ...
        if odds:
            bookmakers = odds[0].get('bookmakers', [])
            betano = next((b for b in bookmakers if b.get('id') == 32 or v3_live__norm_market_text(b.get('name')) == 'betano'), None)
            if betano:
                for bet in betano.get('bets', []):
                    catalog.append({'name': v3_live__norm_market_text(bet.get('name')), 'values': [v3_live__norm_market_text(v.get('value')) for v in bet.get('values', [])]})
    except Exception as e:
        print('BETANO MARKET CATALOG ERROR:', fixture_id, repr(e))
    betano_market_cache[fixture_id] = (time.time(), catalog)
    return catalog

def v3_live_get_live_market_catalog(fixture_id):
    ...
    catalog = []
    try:
        data = requests.get(f'{BASE_URL}/odds/live', headers=HEADERS, params={'fixture': fixture_id}, timeout=15).json()
        for event in data.get('response', []):
            ...
            for bet in groups:
                ...
                catalog.append({'name': v3_live__norm_market_text(bet.get('name')), 'values': [v3_live__norm_market_text(v.get('value')) for v in values]})
    except Exception as e:
        print('LIVE MARKET CATALOG ERROR:', fixture_id, repr(e))
    live_market_cache[fixture_id] = (time.time(), catalog)
    return catalog

def v3_live__catalog_has_market(catalog, market_type):
    for bet in catalog:
        name = bet.get('name', '')
        values = bet.get('values', [])
...
def v3_live_betano_live_market_available(fixture_id, market_type):
    ...
    """
    betano_catalog = v3_live_get_betano_market_catalog(fixture_id)
    live_catalog = v3_live_get_live_market_catalog(fixture_id)
    betano_ok = v3_live__catalog_has_market(betano_catalog, market_type)
    live_ok = v3_live__catalog_has_market(live_catalog, market_type)
    print('BETANO LIVE MARKET CHECK:', fixture_id, market_type, 'BETANO=', betano_ok, 'LIVE=', live_ok)
    return betano_ok and live_ok

def v3_live_calculate_card_pressure(minute, home_fouls, away_fouls, home_yellow, away_yellow, home_red, away_red, home_danger, away_danger):
    pressure = 50
...
def v3_live_analyze_live_match(fixture):
    try:
        ...
        blocked_live = ['reserve', 'reserves', 'women', 'woman', 'female', 'ladies', 'femenina', 'feminine', 'feminin', 'femminile', 'frauen', 'damen', 'feminino', 'u17', 'u18', 'u19', 'u20', 'u21', 'u22', 'u23', 'russia', 'belarus']
        ...
        if away_team.strip().lower().endswith((' w', ' women', ' ladies')):
            return None
        stats = v3_live_get_statistics(fixture_id)
        ...
        home_red = v3_live_extract(home_stats, 'Red Cards')
        away_red = v3_live_extract(away_stats, 'Red Cards')
        home_shots_on = v3_live_extract(home_stats, 'Shots on Goal')
        away_shots_on = v3_live_extract(away_stats, 'Shots on Goal')
        home_total_shots = v3_live_extract(home_stats, 'Total Shots')
        away_total_shots = v3_live_extract(away_stats, 'Total Shots')
        home_corners = v3_live_extract(home_stats, 'Corner Kicks')
        away_corners = v3_live_extract(away_stats, 'Corner Kicks')
        home_fouls = v3_live_extract(home_stats, 'Fouls')
        away_fouls = v3_live_extract(away_stats, 'Fouls')
        home_yellow = v3_live_extract(home_stats, 'Yellow Cards')
        away_yellow = v3_live_extract(away_stats, 'Yellow Cards')
        home_xg = v3_live_extract(home_stats, 'Expected Goals')
        away_xg = v3_live_extract(away_stats, 'Expected Goals')
        home_pressure = v3_live_calculate_pressure(home_stats)
        away_pressure = v3_live_calculate_pressure(away_stats)
        home_form = v3_live_get_team_form(fixture['teams']['home']['id'], venue='home')
        away_form = v3_live_get_team_form(fixture['teams']['away']['id'], venue='away')
        if home_form:
        ...
        print('LIVE ENGINE:', home_team, away_team, 'MIN=', minute, 'SCORE=', home, away, 'PRESS=', home_pressure, away_pressure, 'SOT=', home_shots_on, away_shots_on, 'SHOTS=', home_total_shots, away_total_shots, 'CORNERS=', home_corners, away_corners, 'XG=', home_xg, away_xg, 'CARDS=', total_cards, 'FOULS=', total_fouls)
        if 20 <= minute <= 40:
            ...
            if best_pressure >= 76 and total_shots >= 6 and (total_corners >= 3) and (fh_corner_probability >= 66):
                if not v3_live_betano_live_market_available(fixture_id, 'FH_CORNERS'):
                    print('SKIP FH CORNERS - MARKET NOT AVAILABLE:', fixture_id)
                else:
                    return ('🚩 FIRST HALF OVER 1.5 CORNERS', fh_corner_probability, minute, fh_corner_probability)
        card_probability = v3_live_calculate_card_pressure(minute, home_fouls, away_fouls, home_yellow, away_yellow, home_red, away_red, home_pressure, away_pressure)
        if goal_diff <= 1:
            card_probability += 2
        if home == away:
            card_probability += 1
        ...
        if 65 <= minute <= 78 and card_probability >= 76 and (total_cards >= 4) and (total_fouls >= 18) and (goal_diff <= 2):
            if not v3_live_betano_live_market_available(fixture_id, 'NEXT_CARDS'):
                print('SKIP NEXT CARDS - MARKET NOT AVAILABLE:', fixture_id)
            else:
                return ('🟨 OVER 1.5 NEXT CARDS', card_probability, minute, card_probability)
        if 25 <= minute <= 40 and total >= 1:
            ...
            if best_pressure >= 52 and total_shots_on >= 3 and (pressure_diff >= 4) and (fast_goal_probability >= 54):
                if home_pressure >= away_pressure + 6 and home_shots_on >= away_shots_on:
                    return ('🎯 NEXT GOAL HOME', fast_goal_probability, minute, fast_goal_probability)
                if away_pressure >= home_pressure + 6 and away_shots_on >= home_shots_on:
                    return ('🎯 NEXT GOAL AWAY', fast_goal_probability, minute, fast_goal_probability)
        if 41 <= minute <= 76:
            normal_goal_probability = 78
            normal_goal_probability += max(0, best_pressure - 67) * 0.7
            ...
            if best_pressure >= 86 and total_shots_on >= 12 and (pressure_diff >= 25) and (normal_goal_probability >= 85):
                if home_pressure >= away_pressure + 14 and home_shots_on >= away_shots_on:
                    return ('🎯 NEXT GOAL HOME', normal_goal_probability, minute, normal_goal_probability)
                if away_pressure >= home_pressure + 14 and away_shots_on >= home_shots_on:
                    return ('🎯 NEXT GOAL AWAY', normal_goal_probability, minute, normal_goal_probability)
        ...
        if 25 <= minute <= 65 and best_pressure >= 58 and (total_shots_on >= 4) and (total_shots >= 7) and (home_xg + away_xg >= 1.4) and (remaining_probability >= 60) and (total <= 1):
            return ('🚀 OVER 1.5 REMAINING GOALS', remaining_probability, minute, remaining_probability)
        ...
        if 65 <= minute <= 76 and total_corners >= 10 and (total_shots >= 16) and (best_pressure >= 70) and (corner_probability >= 77) and (total_corners <= 11):
            if not v3_live_betano_live_market_available(fixture_id, 'NEXT_CORNERS'):
        ...
        if 75 <= minute <= 88:
            ...
            if best_pressure >= 54 and total_shots_on >= 3 and (total_shots >= 7) and (total_corners >= 4) and (home_xg + away_xg >= 1.2) and (late_goal_probability >= 60):
                return ('🔥 LATE GOAL', late_goal_probability, minute, late_goal_probability)
        return None
    except Exception as e:
        print('LIVE ANALYSIS ERROR:', repr(e))
        return None

def v3_live_calculate_rest_fatigue(games):
    try:
        if not games:
            return 0
        ...
        for game in games[:5]:
            try:
                game_date = datetime.fromisoformat(game['fixture']['date'].replace('Z', '+00:00'))
                match_dates.append(game_date)
            except Exception:
                continue
        if not match_dates:
            return 0
        ...
        rest_days = (now - last_match).total_seconds() / 86400
        matches_last_8 = 0
        for game_date in match_dates:
            days_ago = (now - game_date).total_seconds() / 86400
            if 0 <= days_ago <= 8:
                matches_last_8 += 1
        ...
        elif rest_days >= 6 and rest_days <= 9:
        ...
        if matches_last_8 >= 4:
            ...
            fatigue_adjustment -= 2
        fatigue_adjustment = max(-4, min(4, fatigue_adjustment))
        print('REST FATIGUE:', 'REST=', round(rest_days, 1), 'MATCHES_8D=', matches_last_8, 'ADJ=', fatigue_adjustment)
        return fatigue_adjustment
    except Exception as e:
        print('REST FATIGUE ERROR:', repr(e))
        return 0

def v3_live_get_team_form(team_id, venue=None):
    cache_key = f'{team_id}_{venue}'
    if cache_key in team_form_cache:
        cache_time, data = team_form_cache[cache_key]
        if time.time() - cache_time < 21600:
            return data
    try:
        r = requests.get(f'{BASE_URL}/fixtures', headers=HEADERS, params={'team': team_id, 'last': 10}, timeout=20).json()
        ...
        if len(games) < 3:
            return None
        fatigue_adjustment = v3_live_calculate_rest_fatigue(games)
        print('TEAM FATIGUE:', team_id, 'ADJ=', fatigue_adjustment)
        scored = 0
        conceded = 0
        wins = 0
        losses = 0
        ...
        for i, g in enumerate(recent_games):
            ...
            recent_goal_diff += team_goals - opp_goals
            weight = weights[i]
            max_weight_points += weight * 3
            if team_goals > opp_goals:
            ...
            if gh + ga >= 3:
                recent_over25 += 1
        recent_form_pct = round(recent_points / max_weight_points * 100, 2) if max_weight_points > 0 else 0
        print('RECENT FORM:', recent_points, max_weight_points, recent_form_pct)
        recent_avg_scored = round(recent_scored / len(recent_games), 2)
        recent_avg_conceded = round(recent_conceded / len(recent_games), 2)
        total = len(games)
        points = wins * 3
        form_pct = round(points / (total * 3) * 100, 2)
        momentum = round(recent_form_pct - form_pct, 2)
        unbeaten = wins + draws
        unbeaten_pct = round(unbeaten / total * 100, 2)
        clean_sheet_pct = round(clean_sheets / total * 100, 2)
        scored_pct = round(scored_games / total * 100, 2)
        goal_diff = scored - conceded
        result = {'fatigue_adjustment': fatigue_adjustment, 'home_wins': home_wins, 'away_wins': away_wins, 'home_games': home_games, 'away_games': away_games, 'scored_pct': scored_pct, 'clean_sheet_pct': clean_sheet_pct, 'momentum': momentum, 'avg_scored': round(scored / total, 2), 'total_scored': scored, 'goal_diff': goal_diff, 'recent_goal_diff': recent_goal_diff, 'avg_conceded': round(conceded / total, 2), 'wins': wins, 'losses': losses, 'draws': draws, 'unbeaten': unbeaten, 'unbeaten_pct': unbeaten_pct, 'over25': over25, 'over25_pct': round(over25 / total * 100, 2), 'btts': btts, 'played': total, 'form_pct': form_pct, 'recent_form_pct': recent_form_pct, 'recent_avg_scored': recent_avg_scored, 'recent_avg_conceded': recent_avg_conceded, 'recent_over25': recent_over25}
        team_form_cache[cache_key] = (time.time(), result)
        return result
    except:
        return None

def v3_live_live_loop():
    matches = v3_live_get_live_matches()
    print(f'Live matches: {len(matches)}')
    print('LIVE SCAN START')
    for match in matches:
        signal = v3_live_analyze_live_match(match)
        if not signal:
            continue
        ...
        away_xg = 0
        if len(stats) >= 2:
            home_pressure = v3_live_calculate_pressure(stats[0])
            away_pressure = v3_live_calculate_pressure(stats[1])
            home_form = v3_live_get_team_form(match['teams']['home']['id'], venue='home')
            away_form = v3_live_get_team_form(match['teams']['away']['id'], venue='away')
            home_shots = v3_live_extract(stats[0], 'Shots on Goal')
            away_shots = v3_live_extract(stats[1], 'Shots on Goal')
            home_corners = v3_live_extract(stats[0], 'Corner Kicks')
            away_corners = v3_live_extract(stats[1], 'Corner Kicks')
        country = match['league']['country']
        league = match['league']['name']
        odds_text = '-'
        match_odds = v3_live_get_match_odds(fixture_id)
        home_odd = None
        away_odd = None
        if match_odds:
        ...
        v3_live_send_telegram(f"                                \n🔥 LIVE SIGNAL\n\n🏆 {home} vs {away}\n\n🌍 {country}\n🏟 {league}\n\n📊 Score:\n{match['goals']['home'] or 0} - {match['goals']['away'] or 0}\n\n⏱ Minute: {minute}\n\n🔥{signal[0]}🔥\n\n💰 Odds:\n{odds_text}\n\n💎 Confidence: {signal[1]}%\n\n🎯 Goal Probability:\n{goal_probability}%\n")

# Route the application's Live entry point to the isolated V3 Live engine.
live_loop = v3_live_live_loop
