        ...
        try:
            ...
            if minutes >= 600 and last_day_sport != day:
                ...
                if hasattr(daily_scanner, "_quota_locked") and daily_scanner._quota_locked("sport"):
                    ...
                    try:
                        ...
                        last_day_sport = day
            # Football morning flow: exactly once at/after 10:00 BG.
            # The statistical scanner and the legacy PREMATCH/Builder engine
            # are deliberately independent: a quota/error in one must NOT
            # silently prevent the other from running.
            if minutes >= 600 and last_day_football != day:
                try:
                    ...
                    # Never retry the statistical daily scanner during the same BG day.
                    last_day_football = day

                # IMPORTANT: PREMATCH + BET BUILDER must still be attempted even
                # when the statistical scanner above hit a quota/error. scan_prematch()
                # has its own API safety checks and will stop cleanly if quota is locked.
                try:
            ...
            # LIVE is active only 17:00-00:00 BG, every 5 minutes.
            # Outside this window it makes ZERO live API calls. A failed scan
            # still advances the 5-minute scheduler slot so it cannot hammer
            # the provider on every 30-second scheduler tick.
