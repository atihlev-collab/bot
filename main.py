...
def home_win_score(
    ...
    score += (
        home_form.get("unbeaten_pct", 0)
        -
        away_form.get("unbeaten_pct", 0)
    ) * 0.1
...
def team_strength(                    
    ...
    score += form.get("unbeaten_pct", 0) * 0.10     
...
def analyze_prematch_match(match):
    ...
    try:
        ...
        if (
            ...
            home_odds_ok
            and
            home_form.get("unbeaten_pct", 0) >= 70
            and
            home_form["wins"] >= 5 
            and                         
            home_form["losses"] <= 1     
        ...
        if (
            ...
            away_odds_ok
            and
            away_form.get("unbeaten_pct", 0) >=66
            and
            away_form["wins"] >= 3
            and                          
            away_form["losses"] <= 1   
...
def prematch_loop():
    ...
    # Keep the already-fetched prematch pool available to the builder.
    globals()["_LAST_PREMATCH_SIGNALS"] = [m for m in matches if isinstance(m, dict)]
    ...
    for match in matches:
        ...
        if signals:
            # Normalize the strongest existing signal for the builder without
            # triggering any additional API call.
            try:
                best_signal = max(
                    [x for x in signals if isinstance(x, dict)],
                    key=lambda x: float(x.get("confidence", x.get("conf", 0)) or 0),
                    default=None,
                )
                if best_signal:
                    match["signal"] = best_signal
            except Exception:
                pass
    ...
    while True:
        ...
        if now - last_prematch_scan >= PREMATCH_INTERVAL:
            ...
            try:
                prematch_loop()
                try:
                    # Build from the same prematch candidate pool, then send only
                    # validated 2-3 leg builders. Never make extra fixture scans.
                    builder_candidates = []
                    for item in globals().get("_LAST_PREMATCH_SIGNALS", []):
                        builder_candidates.append(item)
                    builders = select_quality_bet_builders(builder_candidates, limit=2)
                    for builder in builders:
                        legs_text = "\n".join(
                            f"• {leg.get('market', leg.get('name', 'Market'))} @ {leg.get('odds', leg.get('odd'))}"
                            for leg in builder["legs"]
                        )
                        send_telegram(
                            "🎯 BET BUILDER\n"
                            f"Probability: {builder['probability']:.1f}%\n"
                            f"Confidence: {builder['confidence']:.1f}%\n"
                            f"Combined odds: {builder['combined_odds']:.2f}\n"
                            f"{legs_text}"
                        )
                except Exception as builder_exc:
                    logging.exception("BET BUILDER ERROR: %s", repr(builder_exc))
