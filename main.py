...
def process_prematch_matches(
    ...
    # IMPORTANT: PREMATCH is a DAILY selection, not a 5-minute feed.
    # Once today's Top-5 have been sent, every later prematch scan is analysis-only.
    ...
    # One best market per fixture, then global Top-5 by probability.
    best_by_fixture={}
    for signal in candidates:
        fid=signal.get('fixture_id')
        if fid is None: continue
        old=best_by_fixture.get(fid)
        if old is None or (_safe_num(signal.get('probability'),0),_safe_num(signal.get('confidence'),0)) > (_safe_num(old.get('probability'),0),_safe_num(old.get('confidence'),0)):
...
BUILDER_MIN_PROB_FINAL = 64.0
BUILDER_MAX_LEG_PROB_FINAL = 90.0
BUILDER_MAX_JOINT_PROB_FINAL = 78.0
...
def _final_builder_candidates(match):
    """Build conservative Betano Builder legs.

    A low-priced leg is allowed because it is meant to be combined with
    another market.  Its probability is NOT inflated to 97-99%: the market
    implied probability is used only as a conservative baseline and capped.
    """
    markets = get_prematch_betano_builder_markets(
    ...
    for m in markets:
        ...
        for v in m.get('values', []) or []:
            ...
            if odd is None:
                continue
            if odd < BUILDER_LEG_MIN_ODD_FINAL or odd > BUILDER_LEG_MAX_ODD_FINAL:
                continue

            # Only total-match over/under selections for corners/cards/goals.
            ...
            # Never present a low price as near-certainty.
            model_probability = min(BUILDER_MAX_LEG_PROB_FINAL, implied)
    ...
    # Remove duplicate market labels, keeping the strongest probability.
    best = {}
    for c in candidates:
        old = best.get(c['label'])
        if old is None or c['probability'] > old['probability']:
            best[c['label']] = c
    return sorted(
        best.values(),
        key=lambda x: (x['probability'], x['odd']),
        reverse=True,
    )[:20]
...
def build_best_bet_builder(match):
    ...
    for n in range(2, min(4, len(candidates)) + 1):
        for combo in combinations(candidates, n):
            ...
            if joint_probability < 40.0 or joint_probability > BUILDER_MAX_JOINT_PROB_FINAL:
                continue
...
def _final_standalone_allowed(signal):
    ...
    if p < MIN_PREMATCH_PROBABILITY or c < MIN_PREMATCH_CONFIDENCE or r > MAX_PREMATCH_RISK:
        return False
...
def get_best_prematch_signals(matches):
    ...
    # One best standalone signal per fixture.
    by_fixture = {}
    for s in standalone:
        fid = s.get('fixture_id')
    ...
    # Builder ranking is independent of standalone ranking: the user gets
    # up to two genuinely strong Builders in addition to the top three.
    builders.sort(
        key=lambda s: (
            _safe_num(s.get('probability'), 0),
