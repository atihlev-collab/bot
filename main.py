...
# =========================================================
# LEGACY LIVE COMPATIBILITY HELPERS
# =========================================================

def betano_live_market_available(fixture_id, market_type):
    # Legacy LIVE did not require the separate /odds/live feed to produce
    # a signal. Keep the old signal path alive; the wrapper below attaches
    # the existing match price when one is available.
    return True
...
def legacy_analyze_live_match(fixture):
    try:
...
def build_live_market_candidates(match):
    """Compatibility LIVE builder based on the last known working LIVE model.

    IMPORTANT: this deliberately does NOT require /odds/live. The old working
    LIVE path used the live statistics engine and existing match odds, which
    is why it could actually emit signals when /odds/live returned no Betano
    market. Prematch/statistics code is untouched.
    """
    try:
        raw = legacy_analyze_live_match(match)
        if not raw:
            return []

        market, probability, minute, model_probability = raw
        fid = (match.get("fixture") or {}).get("id")
        if not fid:
            return []

        # Use the same existing match-price source as the old LIVE system.
        odds = get_match_odds(fid)
        if not odds:
            return []

        home_odd, draw_odd, away_odd, over25_odd, under25_odd, over35_odd, under35_odd, btts_odd, home15_odd, away15_odd = odds

        odd = None
        if "NEXT GOAL HOME" in market:
            odd = home_odd
        elif "NEXT GOAL AWAY" in market:
            odd = away_odd
        elif "OVER 1.5 REMAINING GOALS" in market or "LATE GOAL" in market:
            odd = over25_odd
        elif "FIRST HALF OVER 1.5 CORNERS" in market or "NEXT CORNERS" in market or "NEXT CARDS" in market:
            # No exact price in the old 1X2/goal odds tuple. Do not invent one.
            return []

        if odd is None:
            return []
        odd = _safe_num(odd, 0.0)
        if odd <= 1.0 or odd > 4.50:
            return []
        ...
        return [{
            ...
            "odd": round(odd, 2),
            "edge": round(edge, 1),
            "quality": round(float(probability), 1),
        ...
        logging.warning("LEGACY LIVE BUILDER ERROR: %s", repr(e))
        return []
