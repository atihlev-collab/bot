...
def get_prematch_matches():
    ...
    try:
        ...
        # EXACT DAILY WINDOW: today's 12:00 BG -> next day's 12:00 BG.
        # The daily PREMATCH/BUILDER run happens at 10:00, so never use yesterday's window.
        ...
        # Fallback only when the scanner cache is genuinely unavailable.
        dates = {start.strftime('%Y-%m-%d'), end.strftime('%Y-%m-%d')}
        all_matches = []
        for date_string in sorted(dates):
            data = api_get('fixtures', {'date': date_string, 'timezone': 'Europe/Sofia'})
            all_matches.extend(data.get('response', []) if isinstance(data, dict) else [])
        ...
        for m in all_matches:
            ...
            try:
                dt = datetime.fromisoformat(str(dt_raw).replace('Z', '+00:00')).astimezone(TIMEZONE)
            except Exception:
                continue
            if start <= dt < end:
                seen.add(fid)
...
def build_best_bet_builder(match, detailed=True):
    ...
    for c in candidates:
        ...
        if p < BUILDER_MIN_LEG_PROB or conf < BUILDER_MIN_CONFIDENCE:
            continue
        if odd < 1.50 or odd > BUILDER_MAX_ODD:
            continue
...
def send_live_signal(signal):
    ...
    try:
        if float(signal.get('odd', 0) or 0) < 1.50:
            logging.warning('LIVE HARD REJECT odd below 1.50: %s @ %s', market, signal.get('odd'))
            return False
    except (TypeError, ValueError):
        return False
    key=f'{fid}_{market}'
...
def send_prematch_signal(signal):
    ...
    # FINAL HARD GATE: no prematch market or builder leg below 1.50.
    try:
        if float(signal.get('odd', 0) or 0) < 1.50:
            logging.warning('PREMATCH HARD REJECT odd below 1.50: %s @ %s', market, signal.get('odd'))
            return False
    except (TypeError, ValueError):
        return False
    ...
    if market != '🧩 BET BUILDER':
        ...
        if any(not f for f in families) or len(set(families)) != len(families):
            return False
        if any(float(x.get('odd', 0) or 0) < 1.50 for x in legs):
            return False
...
def print_system_status():
    ...
    print('STATUS: ONLINE')
    print('LIVE: 300 sec (17:00-00:00 BG)')
    print('PREMATCH: once daily at 10:00 BG (12:00-12:00 window)')
...
def main_loop():
    ...
    while True:
        ...
        try:
            ...
            if minutes >= 600 and last_day_sport != day:
                ...
                if hasattr(daily_scanner, "_quota_locked") and daily_scanner._quota_locked("sport"):
                    ...
                    try:
                        ...
                        last_day_sport = day
            # Football daily: exactly once at/after 10:00.
            ...
            # Live: ONLY 17:00-00:00 BG, every 5 minutes.
            # Outside this window the live scanner is completely OFF and makes no API requests.
