...
# BLOCK: MAIN_LOOP
# ============================================================
# FINAL PREMATCH SEND GUARD
# ============================================================

_PREMATCH_ORIGINAL_SEND = send_prematch_signal

def _final_betano_exact_odd(fixture_id, market):
    """Return the exact Betano/API-Football odd for a full-match market.

    No other bookmaker is accepted. No stale signal odd is used.
    """
    try:
        ...
        bookmaker = _betano_bookmaker(
            odds[0].get("bookmakers", []) or []
        )
        if not bookmaker:
            return None

        text = clean_text(_base_market_from_label(market))
        ...
        for i, part in enumerate(parts):
            if part in ("over", "under") and i + 1 < len(parts):
                target = f"{part} {parts[i + 1]}"
                break
        if family == "btts" and target is None:
            target = "yes"
        ...
        for bet in bookmaker.get("bets", []) or []:
            ...
            if family == "cards" and not ("card" in name or "booking" in name):
                continue
            ...
            for value in bet.get("values", []) or []:
                ...
                if label == target and odd is not None and odd > 1.01:
                    return odd
        ...
        logging.warning(
            "FINAL BETANO ODDS ERROR | fixture=%s | market=%s | error=%s",
            fixture_id, market, repr(e)
        )
        return None
...
def send_prematch_signal(signal):
    ...
    market = _base_market_from_label(signal.get("market"))

    if not fid or not market:
        return False

    # A fresh Betano snapshot is mandatory immediately before sending.
    try:
        odds = get_odds(fid) or []
        bookmaker = _betano_bookmaker(
            (odds[0].get("bookmakers", []) if odds else [])
        )
        if not bookmaker:
            logging.info(
                "PREMATCH BLOCKED | Betano bookmaker unavailable | fixture=%s",
                fid
            )
            return False
        ...
        if "card" in market_text or "booking" in market_text:
            family = "cards"
        elif "corner" in market_text:
            family = "corners"
        elif "goal" in market_text:
            family = "goals"
        elif "btts" in market_text or "both teams" in market_text:
            family = "btts"
        ...
        for i, part in enumerate(parts):
            if part in ("over", "under") and i + 1 < len(parts):
                target = f"{part} {parts[i + 1]}"
                break
        if target is None and family == "btts":
            target = "yes"
        ...
        for bet in bookmaker.get("bets", []) or []:
            ...
            if family == "cards" and not (
                "card" in name or "booking" in name
            ):
                continue
            if family == "corners" and "corner" not in name:
                continue
        ...
        if found_odd is None:
            logging.info(
                "PREMATCH BLOCKED | exact line absent in Betano | "
        ...
        signal["odd"] = found_odd

    except Exception as e:
        logging.warning(
            "PREMATCH FINAL BETANO CHECK ERROR: %s",
            repr(e)
        )
        return False

    return _PREMATCH_ORIGINAL_SEND(signal)
