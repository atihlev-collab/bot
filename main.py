...
def _build_bet_builders(matches):
    ...
    for match in matches:
        try:
            ...
            if not odds:
                continue
            home_odd, draw_odd, away_odd, over25_odd, under25_odd, btts_odd = odds[:6]
            home_over15_odd = odds[6] if len(odds) > 6 else None
            away_over15_odd = odds[7] if len(odds) > 7 else None
            over35_odd = odds[8] if len(odds) > 8 else None
            odd_map = {
                "HOME WIN": home_odd, "AWAY WIN": away_odd, "OVER 2.5": over25_odd,
                "UNDER 2.5": under25_odd, "BTTS": btts_odd, "HOME OVER 1.5": home_over15_odd,
                "AWAY OVER 1.5": away_over15_odd, "OVER 3.5": over35_odd
            }
            legs = []
            for market, confidence, probability in signals:
                ...
                if odd is None or float(odd) <= 1.0:
                    continue
                ok, edge, ev, reason = tipster_final_gate(market, probability, confidence, float(odd))
                if not ok:
                    continue
                if float(probability) < 72 or float(confidence) < 78:
                    continue
                legs.append({"market": market, "odd": float(odd), "probability": float(probability), "confidence": float(confidence), "family": key})
            # Same-family combinations are avoided; two legs from one match only.
            for a, b in itertools.combinations(legs, 2):
                if a["family"] == b["family"]:
                    continue
                combined_odd = a["odd"] * b["odd"]
                joint_probability = (a["probability"] / 100.0) * (b["probability"] / 100.0) * 100.0
                confidence = (a["confidence"] + b["confidence"]) / 2.0
                if not (1.70 <= combined_odd <= 4.50 and joint_probability >= 52 and confidence >= 78):
                    continue
                fixture_time = datetime.fromisoformat(match["fixture"]["date"].replace("Z", "+00:00")).astimezone(TZ)
                builders.append({
                    "fixture_id": match["fixture"]["id"], "home": match["teams"]["home"]["name"], "away": match["teams"]["away"]["name"],
                    "country": match["league"]["country"], "league": match["league"]["name"],
                    "match_date": fixture_time.strftime("%d.%m.%Y"), "kickoff_time": fixture_time.strftime("%H:%M"),
                    "legs": [a, b], "odd": combined_odd, "probability": joint_probability, "confidence": confidence
                })
        except Exception as e:
            print("BUILDER ERROR:", repr(e))
    builders.sort(key=lambda x: (x["confidence"] * 0.55 + x["probability"] * 0.45), reverse=True)
    return builders[:2]


def _run_prematch_package(window):
    print("PREMATCH PACKAGE:", window)
    ...
    for match in filtered:
        try:
            ...
            if not odds:
                continue
            odd_map = {"HOME WIN": odds[0], "AWAY WIN": odds[2], "OVER 2.5": odds[3], "UNDER 2.5": odds[4], "BTTS": odds[5], "HOME OVER 1.5": odds[6] if len(odds)>6 else None, "AWAY OVER 1.5": odds[7] if len(odds)>7 else None, "OVER 3.5": odds[8] if len(odds)>8 else None}
            ft = datetime.fromisoformat(match["fixture"]["date"].replace("Z", "+00:00")).astimezone(TZ)
            for market, confidence, probability in signals:
                odd = odd_map.get(_prematch_market_key(market))
                if odd is None:
                    continue
                ok, edge, ev, reason = tipster_final_gate(market, probability, confidence, float(odd))
                if not ok:
                    continue
                all_signals.append((float(probability), match, market, float(confidence), float(odd), float(edge), float(ev)))
        except Exception as e:
            print("PREMATCH PACKAGE ERROR:", repr(e))
    all_signals.sort(key=lambda x: tipster_selector_score(x[2], x[0], x[3], x[4]), reverse=True)
    sent = 0
    used = set()
    for probability, match, market, confidence, odd, edge, ev in all_signals:
        fixture_id = match["fixture"]["id"]
        ...
        if key in sent_prematch and time.time() - sent_prematch[key] < 86400:
            continue
        if send_prematch_signal(fixture_id, ft.strftime("%d.%m.%Y"), ft.strftime("%H:%M"), match["league"]["country"], match["league"]["name"], match["teams"]["home"]["name"], match["teams"]["away"]["name"], market, confidence, probability, str(odd)):
            sent_prematch[key] = time.time(); used.add(fixture_id); sent += 1
        if sent >= 3:
            break
    builders = _build_bet_builders(filtered)
    for builder in builders:
        _send_bet_builder(builder)
    print("PREMATCH SENT:", sent, "BUILDERS:", len(builders))
