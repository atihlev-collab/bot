...
def main_loop():
    ...
    # DAILY SCANNER runs independently so it can never break LIVE/PREMATCH.
    if run_due_scans is not None:
        def _daily_scanner_worker():
            while True:
                try:
                    run_due_scans(send_telegram)
                except Exception as exc:
                    logging.exception("DAILY SCANNER LOOP ERROR: %r", exc)
                time.sleep(60)

        threading.Thread(
            target=_daily_scanner_worker,
            name="daily-scanner",
            daemon=True,
        ).start()
