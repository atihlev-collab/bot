...
def blocked_league(league_name):
    """
    Shared league filter used by the prematch scanner.
    """
    ...
    return any(
        re.search(
            rf"(?<![a-z0-9]){re.escape(word.lower())}(?![a-z0-9])",
            name,
        )
...
def api_health_check():
    """
    Highlightly /football/matches requires at least one PRIMARY query
    parameter. `limit` is secondary, so the old health check generated
    the 400: "At least one param has to be defined".
    """
    try:
        today = datetime.now(TIMEZONE).strftime("%Y-%m-%d")
