...
# RESTORED LEGACY LIVE PRESSURE ENGINE
# The restored LIVE analyze_live_match() depends on this helper.
...
def main_loop():
    ...
    try:
        init_database()
        logging.info('HIGHLIGHTLY INIT DATABASE OK')
    except Exception as exc:
        logging.exception('HIGHLIGHTLY INIT DATABASE ERROR: %s', exc)
        # Keep the scheduler alive; API scanning can still run even if the
        # optional signal-history database is temporarily locked.
    try:
        _quota_init()
        logging.info('HIGHLIGHTLY QUOTA INIT OK')
    except Exception as exc:
        logging.exception('HIGHLIGHTLY QUOTA INIT ERROR: %s', exc)
        # Do not kill the process before the scheduler starts.
    logging.info('HIGHLIGHTLY SCHEDULER READY')
    while True:
        now = datetime.now(TIMEZONE)
