...
def main_loop():
    """Unified scheduler: PREMATCH+Builder once daily, LIVE every 5 min, daily scanner once."""
    import scanner as daily_scanner

    last_live = 0.0
    last_day_football = None
    last_day_sport = None
    football_daily_done = None

    logging.info('HIGHLIGHTLY SYSTEM START | PREMATCH + BUILDER + LIVE ENABLED')
    init_database()
    _quota_init()
    logging.info('HIGHLIGHTLY SCHEDULER READY | waiting for scheduled flows')
    ...
    while True:
        ...
        try:
            # ------------------------------------------------------------
            # 1) PREMATCH + BET BUILDER — once per BG calendar day, >=10:00
            # ------------------------------------------------------------
            if minutes >= 600 and last_day_football != day:
                ...
                try:
                    ...
                    logging.info('PREMATCH FIXTURE CACHE READY | %s fixtures', len(cached_for_today or []))
                    logging.info('PREMATCH ENGINE START')
                    prematch_sent = scan_prematch()
                    logging.info('PREMATCH + BUILDER ENGINE END | sent=%s', prematch_sent)
                    # scan_prematch() includes the V7 separate TOP-3 Builder
                    # selection in process_prematch_matches().
                    logging.info('BET BUILDER DAILY TOP-3 | included in PREMATCH engine')
                except Exception as exc:
                    logging.exception('PREMATCH/BUILDER FLOW ERROR: %s', exc)
                finally:
                    # Prevent repeated morning processing after the first attempt.
            ...
            # ------------------------------------------------------------
            # 3) FOOTBALL DAILY STATISTICAL SCANNER — once daily, >=10:00
            # ------------------------------------------------------------
            if minutes >= 600 and football_daily_done != day:
                logging.info('FOOTBALL DAILY STATISTICAL FLOW START | %s', day)
                try:
                    if hasattr(daily_scanner, '_quota_locked') and daily_scanner._quota_locked('football'):
                        logging.warning('FOOTBALL DAILY: quota locked for %s; no retry', day)
                    else:
                        start_bg, end_bg = _main_morning_fixture_window()
                        shared_fixtures = _main_get_morning_fixtures(start_bg, end_bg)
                        original_loader = getattr(daily_scanner, 'get_fixtures_for_window', None)
                        if shared_fixtures and callable(original_loader):
                            daily_scanner.get_fixtures_for_window = (
                                lambda _start, _end, _fixtures=shared_fixtures: list(_fixtures)
                            )
                            try:
                                daily_scanner.run_daily_scanner('day', send_func=send_telegram)
                            finally:
                                daily_scanner.get_fixtures_for_window = original_loader
                        else:
                            daily_scanner.run_daily_scanner('day', send_func=send_telegram)
                except getattr(daily_scanner, 'APIQuotaExceeded', Exception) as exc:
                    logging.warning('FOOTBALL DAILY STATISTICAL STOPPED: %s', exc)
                except Exception as exc:
                    logging.exception('FOOTBALL DAILY STATISTICAL ERROR: %s', exc)
                finally:
                    football_daily_done = day
                    logging.info('FOOTBALL DAILY STATISTICAL FLOW END | %s', day)

            # ------------------------------------------------------------
            # 4) SPORT DAILY — preserve existing scanner/quota behavior
            # ------------------------------------------------------------
            if minutes >= 600 and last_day_sport != day:
                try:
                    if hasattr(daily_scanner, '_quota_locked') and daily_scanner._quota_locked('sport'):
                        logging.warning('SPORT DAILY: quota locked for %s', day)
                    else:
