...
# BLOCK: GET_PREMATCH_MATCHES
def get_prematch_matches():
    """Load the exact 12:00 BG -> next-day 12:00 BG prematch window, reusing daily scanner cache."""
    try:
        ...
        try:
            ...
            if cached:
                return [_hl_match_to_legacy(x) if isinstance(x, dict) and "fixture" not in x else x for x in cached]
        except Exception:
            pass
...
def get_v7_raw_odds(fixture_id):
    ...
    # Reuse a real odds payload cached by the daily scanner when available.
    data=None
    try:
        import scanner as daily_scanner
        data=daily_scanner.get_cached_v7_betano_odds(fixture_id)
    except Exception:
        data=None
    if data is None:
