...
def main_loop():
    """Unified resilient runner for PREMATCH, LIVE and daily statistics."""
    ...
    try:
        ...
        logging.warning("SYSTEM STATUS ERROR: %s", repr(exc))

    try:
        if not api_health_check():
            logging.warning("API NOT AVAILABLE — engines will retry")
    except Exception as exc:
        logging.warning("API HEALTH ERROR: %s", repr(exc))
    ...
    while True:
        ...
        try:
            stat_slot = now_dt.strftime("%Y-%m-%d-%H-%M")
            if stat_slot != last_stat_slot:
                run_due_scans(send_telegram)
                last_stat_slot = stat_slot
        except Exception as exc:
            logging.exception("DAILY SCANNER ERROR: %s", repr(exc))

        # ---------------- PREMATCH ----------------
        try:
            if now - LAST_PREMATCH_SCAN >= PREMATCH_SCAN_INTERVAL:
                signals = scan_prematch_matches() or []
                print("PREMATCH SIGNALS FOUND:", len(signals))

                for signal in signals:
                    try:
                        send_prematch_signal(signal)
                    except Exception as exc:
                        logging.warning(
                            "PREMATCH SEND ERROR: %s", repr(exc)
                        )
        ...
        try:
            if now - LAST_LIVE_SCAN >= LIVE_SCAN_INTERVAL:
                ...
                signals = run_live_scan() or []
                print("LIVE SIGNALS SENT:", len(signals))
                LAST_LIVE_SCAN = now
        except Exception as exc:
