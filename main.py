...
def main_loop():
    """Single resilient runner for PREMATCH, LIVE and daily statistics.

    Uses only functions that actually exist in this file:
    - init_database()
    - prematch_loop()
    - live_loop()
    ...
    print("============================================================")
    print("🤖 AI FOOTBALL BOT — LIVE + PREMATCH + STATISTICS")
    print("STATUS: ONLINE")
    print(f"LIVE: {LIVE_INTERVAL} sec")
    print("PREMATCH: 11:00 BG + 21:00 BG")
    print("STATISTICS: 10:00 BG + 20:00 BG")
    print("RESULTS: 300 sec")
    print("============================================================")
    ...
    last_prematch_thread = None
    last_stat_thread = None

    def run_statistics_safe():
        try:
            print("STATISTICS ENGINE START")
            run_due_scans(send_telegram)
            print("STATISTICS ENGINE COMPLETE")
        except Exception as exc:
            logging.exception("DAILY STATISTICAL SCANNER ERROR: %s", repr(exc))

    def run_prematch_safe(slot_name):
        try:
            print(f"PREMATCH ENGINE START: {slot_name}")
            prematch_loop()
            print(f"PREMATCH ENGINE COMPLETE: {slot_name}")
        except Exception as exc:
            logging.exception("PREMATCH ENGINE ERROR (%s): %s", slot_name, repr(exc))
    ...
    while True:
        ...
        # =====================================================
        # DAILY STATISTICAL SCANNER — 10:00 + 20:00 BG
        # Runs in its own thread so a long scan cannot block LIVE.
        # =====================================================
        stat_slot = None
        if now_dt.hour in (10, 20):
            stat_slot = now_dt.strftime("%Y-%m-%d-%H")

        if stat_slot and stat_slot != last_stat_slot:
            if last_stat_thread is None or not last_stat_thread.is_alive():
                last_stat_slot = stat_slot
                last_stat_thread = threading.Thread(
                    target=run_statistics_safe,
                    name=f"statistics-{stat_slot}",
                    daemon=True
                )
                last_stat_thread.start()
            else:
                print("STATISTICS STILL RUNNING — SKIP DUPLICATE START")

        # =====================================================
        # PREMATCH — 11:00 BG and 21:00 BG
        # The existing prematch engine is used unchanged.
        # It already performs its own selection, Betano odds checks
        # and Telegram sending.
        # =====================================================
        prematch_slot = None
        if now_dt.hour == PREMATCH_DAY_HOUR and now_dt.minute == 0:
            prematch_slot = now_dt.strftime("%Y-%m-%d-11")
        elif now_dt.hour == PREMATCH_NIGHT_HOUR and now_dt.minute == 0:
            prematch_slot = now_dt.strftime("%Y-%m-%d-21")

        if prematch_slot and prematch_slot != last_prematch_slot:
            if last_prematch_thread is None or not last_prematch_thread.is_alive():
                last_prematch_slot = prematch_slot
                slot_name = "DAY 11:00" if now_dt.hour == 11 else "NIGHT 21:00"
                last_prematch_thread = threading.Thread(
                    target=run_prematch_safe,
                    args=(slot_name,),
                    name=f"prematch-{prematch_slot}",
                    daemon=True
                )
                last_prematch_thread.start()
                LAST_PREMATCH_SCAN = now
            else:
                print("PREMATCH STILL RUNNING — SKIP DUPLICATE START")

        # =====================================================
        # LIVE — every 60 seconds
        # Runs independently of the statistical scanner and prematch.
        # =====================================================
        ...
        if now - LAST_RESULT_SCAN >= RESULT_INTERVAL:
            ...
            try:
                ...
                logging.exception("RESULT CHECK ERROR: %s", repr(exc))

        time.sleep(5)


if __name__ == "__main__":
    print("MAIN V5 STARTED")
    try:
        main_loop()
