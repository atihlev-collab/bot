...
def main_loop():
    ...
    PREMATCH/Builder and the daily statistical scanner have separate daily
    guards so a slow PREMATCH analysis can never prevent the statistical scan
    from running. The statistical scan runs first at the morning boundary
    because it also populates the shared fixture/history cache used by
    PREMATCH and Builder.
    """
    import scanner as daily_scanner
    last_live = 0.0
    last_day_sport = None
    last_day_football_daily = None
    last_day_prematch = None
    ...
    try:
        ...
        logging.exception('HIGHLIGHTLY STARTUP INIT ERROR: %s', exc)
        raise
    while True:
        now = datetime.now(TIMEZONE)
        ...
        try:
            # 1) LIVE — 17:00-00:00 BG, every 5 minutes.
            ...
            # 2) FOOTBALL STATISTICAL DAILY — independent daily guard.
            # This cannot be blocked by PREMATCH/Builder.
            if minutes >= 600 and last_day_football_daily != day:
                try:
                    logging.info("FOOTBALL DAILY STATISTICAL START | %s", day)
                    ...
                    logging.info("FOOTBALL DAILY STATISTICAL END | %s", day)
            ...
            # 3) PREMATCH + BET BUILDER — existing V7 logic unchanged.
            # It runs after the statistical fixture/history cache is ready.
            if minutes >= 600 and last_day_prematch != day:
                try:
                    ...
                    last_day_prematch = day

            # 4) SPORT DAILY — existing daily guard/quota protection.
