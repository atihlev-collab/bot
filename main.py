...
# Highlightly direct API authentication.
HEADERS = {"x-rapidapi-key": HIGHLIGHTLY_API_KEY}

# Shared daily budget with scanner.py. Keep a 500-request reserve.
...
bot = None  # Telegram is sent through HTTPS; no telegram package required.
...
def _quota_init():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS api_usage (day TEXT NOT NULL, provider TEXT NOT NULL, used INTEGER NOT NULL DEFAULT 0, PRIMARY KEY(day,provider))")
    conn.execute("CREATE TABLE IF NOT EXISTS api_quota_locks (provider TEXT PRIMARY KEY, locked_day TEXT NOT NULL, reason TEXT NOT NULL)")
    conn.commit()
    conn.close()
...
def _quota_used(provider="football"):
    _quota_init()
    conn = sqlite3.connect(QUOTA_DB)
    row = conn.execute("SELECT used FROM api_usage WHERE day=? AND provider=?", (_quota_today(), provider)).fetchone()
    conn.close()
...
def _quota_lock(provider, reason):
    _quota_init()
    conn = sqlite3.connect(QUOTA_DB)
    conn.execute("INSERT OR REPLACE INTO api_quota_locks(provider,locked_day,reason) VALUES(?,?,?)", (provider, _quota_today(), str(reason)[:300]))
    conn.commit()
    conn.close()
...
def _quota_add(provider="football"):
    ...
    today = _quota_today()
    conn = sqlite3.connect(QUOTA_DB)
    conn.execute("INSERT OR IGNORE INTO api_usage(day,provider,used) VALUES(?,?,0)", (today, provider))
    conn.execute("UPDATE api_usage SET used=used+1 WHERE day=? AND provider=?", (today, provider))
    conn.commit()
    conn.close()


def _hl_status(description):
    d = str(description or '').casefold()
    mapping = {
        'not started': 'NS', 'to be announced': 'TBD', 'first half': '1H',
        'second half': '2H', 'half time': 'HT', 'extra time': 'ET',
        'penalties': 'P', 'finished': 'FT', 'finished after penalties': 'PEN',
        'finished after extra time': 'AET', 'postponed': 'PST', 'cancelled': 'CANC',
        'suspended': 'SUSP', 'interrupted': 'INT', 'abandoned': 'ABD',
        'in progress': 'LIVE', 'break time': 'BT', 'awarded': 'AWD',
    }
    return mapping.get(d, 'NS' if not d else 'UNK')


def _team_display_name(team, fallback=""):
    """Use the most complete team name supplied by the API without inventing one."""
...
def _hl_match_to_legacy(m):
    ...
    if '-' in current:
        try:
            a,b = [x.strip() for x in current.split('-',1)]
            hg, ag = int(float(a)), int(float(b))
        except Exception:
            pass
    return {
        'fixture': {
            ...
            'timestamp': None,
            'status': {'short': _hl_status(state.get('description')), 'elapsed': state.get('clock')},
        },
        'teams': {
            'home': {'id': home.get('id'), 'name': _team_display_name(home, 'HOME')},
            'away': {'id': away.get('id'), 'name': _team_display_name(away, 'AWAY')},
        },
        'league': {'id': league.get('id'), 'name': league.get('name'), 'country': country.get('name') if isinstance(country,dict) else country},
        'goals': {'home': hg, 'away': ag},
        '_highlightly': m,
    }


def _hl_odds_to_legacy(payload):
    ...
    for item in data:
        ...
        if not mid:
            continue
        bookmakers = grouped.setdefault(mid, {})
        for market in item.get('odds', []) or []:
            if not isinstance(market, dict):
                continue
            bid = market.get('bookmakerId'); bname = market.get('bookmakerName') or ''
            b = bookmakers.setdefault((bid,bname), {'id': bid, 'name': bname, 'bets': []})
            name = market.get('market') or ''
            # Normalize Highlightly market names to the legacy engine's names.
            _mn = str(name).casefold()
            if _mn == 'full time result': name = 'Match Winner'
            elif _mn == 'both teams to score': name = 'Both Teams To Score'
            values=[]
            for v in market.get('values',[]) or []:
                if not isinstance(v,dict): continue
                values.append({'value': v.get('value'), 'odd': v.get('odd'), 'stopped': False, 'main': v.get('main')})
            if values:
                b['bets'].append({'id': None, 'name': name, 'values': values})
    response=[]
    for mid, books in grouped.items():
        response.append({'fixture': {'id': mid}, 'bookmakers': list(books.values())})
    return {'response': response}


def _hl_stats_to_legacy(payload):
    raw = payload
    ...
    for team in raw:
        ...
        tid=(team.get('team') or {}).get('id') if isinstance(team.get('team'),dict) else team.get('teamId')
        arr=[]
        for st in team.get('statistics',[]) or []:
            if not isinstance(st,dict): continue
            name=st.get('displayName') or st.get('name') or st.get('type')
            arr.append({'type': name, 'value': st.get('value')})
        out.append({'team': {'id': tid}, 'statistics': arr})
    return {'response': out}


def api_get(endpoint, params=None):
    """Highlightly compatibility adapter for the legacy 23k-line engine.
    The rest of the old signal/model code stays intact; only the provider layer changes.
    """
    params = dict(params or {})
    provider = 'football'
    if _quota_locked(provider) or _quota_used(provider) >= API_HARD_STOP:
        _quota_lock(provider, 'local safety limit/previous 429')
        logging.warning('HIGHLIGHTLY QUOTA LOCKED | %s', endpoint)
        return {}

    # Translate legacy API-Football endpoint names to Highlightly.
    path = endpoint.strip('/')
    hl_path = path
    ...
    if path == 'fixtures':
        ...
        fid=hl_params.pop('fixture', None)
        if not fid: return {'response': []}
        hl_path=f'statistics/{fid}'; transform='stats'
    elif path == 'fixtures/events':
        fid=hl_params.pop('fixture', None)
        if not fid: return {'response': []}
        hl_path=f'matches/{fid}'; transform='events'
    elif path == 'odds':
        fid=hl_params.pop('fixture', None)
        if fid: hl_params['matchId']=fid
        hl_params.setdefault('oddsType','prematch')
        hl_params.setdefault('bookmakerName','Betano')
        hl_path='odds'; transform='odds'
    elif path == 'odds/live':
        fid=hl_params.pop('fixture', None)
        if fid: hl_params['matchId']=fid
        hl_params['oddsType']='live'; hl_params.setdefault('bookmakerName','Betano')
        hl_path='odds'; transform='odds'
    elif path == 'standings':
        if 'league' in hl_params: hl_params['leagueId']=hl_params.pop('league')
        if 'season' not in hl_params: hl_params['season']=datetime.now(TIMEZONE).year
        hl_path='standings'; transform='standings'

    _quota_add(provider)
    try:
        r=requests.get(f'{BASE_URL}/{hl_path}', headers=HEADERS, params=hl_params, timeout=REQUEST_TIMEOUT)
        remaining=r.headers.get('x-ratelimit-requests-remaining')
        limit=r.headers.get('x-ratelimit-requests-limit')
        logging.info('HIGHLIGHTLY %s | %s | remaining=%s/%s', hl_path, r.status_code, remaining, limit)
        if r.status_code==429:
            _quota_lock(provider,'HTTP 429'); return {}
        if remaining is not None:
            try:
                if int(float(remaining)) <= API_SAFETY_RESERVE:
                    _quota_lock(provider,f'provider remaining={remaining}')
            except ValueError: pass
        if r.status_code != 200:
            return {}
        data=r.json()
    except Exception as e:
        logging.warning('HIGHLIGHTLY REQUEST ERROR | %s | %s', hl_path, repr(e)); return {}

    if transform in ('matches','live_filter'):
        arr=data.get('data',[]) if isinstance(data,dict) else []
        legacy=[_hl_match_to_legacy(x) for x in arr]
        legacy=[x for x in legacy if x]
        if transform=='live_filter':
            live_codes={'1H','2H','HT','ET','P','LIVE','BT','INT'}
            legacy=[x for x in legacy if x.get('fixture',{}).get('status',{}).get('short') in live_codes]
        return {'response':legacy}
    if transform=='stats': return _hl_stats_to_legacy(data)
    if transform=='odds': return _hl_odds_to_legacy(data)
    if transform=='events':
        # Match details include events; adapt them to the old event schema.
        items=data.get('data',[]) if isinstance(data,dict) else data
        if isinstance(items,list): items=items[0] if items else {}
        events=[]
        for ev in (items.get('events',[]) if isinstance(items,dict) else []):
            if not isinstance(ev,dict): continue
            events.append({'team':ev.get('team') or {}, 'type':ev.get('type'), 'detail':ev.get('type'), 'time':{'elapsed':safe_float(str(ev.get('time','')).split('+')[0],0)}, 'player':{'name':ev.get('player')}})
        return {'response':events}
    if transform=='standings':
        arr=data.get('data',[]) if isinstance(data,dict) else []
        return {'response':[{'league':{'standings':[arr]}}]}
...
def get_match_odds(fixture_id):
    ...
    if fixture_id in odds_cache:
        ...
        if time.time() - cache_time < CACHE_TIME_ODDS:
            # The V7 prematch engine reads the raw bookmaker payload from
            # RAW_ODDS_CACHE.  The old cache-hit path returned before filling
            # that cache, so Builder could silently see no markets.
            raw = RAW_ODDS_CACHE.get(fixture_id)
            if raw is not None:
                return data
            # If raw odds were lost/restarted, refresh exactly once.
    ...
    # Share the exact raw response with the V7 prematch/builder reader.
    # This prevents a second odds request for the same fixture.
    RAW_ODDS_CACHE[fixture_id] = odds
...
def _get_cached_scanner_team_matches(team_id):
    """Read the daily scanner's persistent season history without an API call."""
    try:
        conn = sqlite3.connect(QUOTA_DB, timeout=5)
        row = conn.execute(
            "SELECT data FROM scanner_team_season_history WHERE team_id=? AND season=?",
            (int(team_id), int(datetime.now(TIMEZONE).year)),
        ).fetchone()
        conn.close()
        if not row:
            return None
        ...
        if isinstance(obj, dict):
            # scanner.py stores league-specific histories as {league_id: [matches]}
            # (and some older versions used {"matches": [...]}). Merge both forms.
            if isinstance(obj.get('matches'), list):
                ...
                for value in obj.values():
                    ...
                    for game in value:
                        ...
                        key = fid if fid is not None else id(game)
                        if key in seen:
                            continue
                        seen.add(key)
                        games.append(game)
                games.sort(key=lambda g: (g.get('fixture') or {}).get('date', ''))
        else:
            games = None
...
def get_team_form(team_id, venue=None):
    ...
    cached_games = _get_cached_scanner_team_matches(team_id)
    if cached_games is not None:
        games = cached_games
    else:
        # The morning statistical scanner is the single source of truth for
        # season history. Do not start another football fixture-history scan
        # from PREMATCH/Builder when that cache is unavailable.
        logging.warning("PREMATCH HISTORY CACHE MISS | team=%s | venue=%s", team_id, venue)
        return None
    ...
    # -----------------------------------------------------
    # CURRENT-SEASON HISTORY RULE
    # Use every available official completed match from the scanner cache.
    # A minimum of 3 observations is required; never silently truncate to 5.
    # -----------------------------------------------------
    ...
    for i, game in enumerate(games):
        ...
        if gf > gc:
            ...
            wins += 1
            recent_points += 3 * weights[min(i, len(weights)-1)]

        elif gf == gc:

            draws += 1
            recent_points += weights[min(i, len(weights)-1)]
...
def analyze_live_match(
    ...
    try:
        ...
        home_name = _team_display_name(home_team, "HOME")

        away_name = _team_display_name(away_team, "AWAY")
...
def get_best_live_signal(
    ...
    try:
        ...
        home_team = _team_display_name(teams.get("home", {}), "HOME")

        away_team = _team_display_name(teams.get("away", {}), "AWAY")
...
# BLOCK: GET_PREMATCH_MATCHES
def get_prematch_matches():
    """Load the daily 12:00 BG -> next-day 12:00 BG window.

    The Football daily scanner is the single source for the fixture feed.
    If its persistent cache is populated, prematch/builder make ZERO new
    fixture-list requests. Only an empty cache falls back to Highlightly.
    """
    global PREMATCH_DATA_READY
    PREMATCH_DATA_READY = False
    try:
        now = datetime.now(TIMEZONE)
        # The morning scanner always publishes TODAY 12:00 BG -> TOMORROW
        # 12:00 BG, even though PREMATCH runs before 12:00.  Do not shift the
        # window to yesterday when the clock is still before noon.
        start = now.replace(hour=12, minute=0, second=0, microsecond=0)
        end = start + timedelta(days=1)

        try:
            import scanner as daily_scanner
            cached = daily_scanner.get_cached_upcoming_matches(start, end)
            print(f"PREMATCH CACHE DEBUG | window={start.isoformat()} -> {end.isoformat()} | fixtures={len(cached or [])}")
            if cached:
                PREMATCH_DATA_READY = True
                return cached
        except Exception as cache_exc:
            logging.warning('PREMATCH SCANNER CACHE ERROR: %s', repr(cache_exc))

        # IMPORTANT: do not make a second fixture-list request. The daily
        # scanner is the single morning source of truth. If its cache is
        # unavailable, PREMATCH/Builder stop rather than silently starting a
        # second football fixture scan later in the day.
        logging.warning('PREMATCH: morning scanner fixture cache unavailable; no fallback football fixture API call')
        print("PREMATCH CACHE DEBUG | READY=False | fixtures=0")
        return []
    except Exception as e:
        logging.warning('PREMATCH FIXTURES ERROR: %s', repr(e))
        return []
...
# PROFIT MODE: only send selections with a meaningful model-vs-price advantage.
# This targets positive expected value; it does NOT guarantee profit.
PROFIT_MODE = True
PROFIT_MIN_EDGE = 8.0
PROFIT_MIN_EV = 0.08
PROFIT_MIN_SAMPLE = 5
MARKET_LEARNING_MIN_SETTLED = 30
MARKET_LEARNING_MIN_ROI = 0.0
...
LIVE_MARKET_CACHE = {}
LIVE_ODDS_CACHE_TTL = 600  # Highlightly live odds refresh interval is ~10 min.
...
def _betano_bookmaker(bookmakers):
    """Find Betano without depending on a hard-coded bookmaker id.

    Highlightly documents bookmakerName/bookmakerId as the supported odds
    filters, so the provider id can change while the bookmaker name remains
    the stable identifier we care about.
    """
    for bookmaker in bookmakers or []:
        if not isinstance(bookmaker, dict):
            continue
        name = clean_text(bookmaker.get('name') or bookmaker.get('bookmakerName'))
        if name == 'betano' or 'betano' in name:
            return bookmaker
    return None
...
def get_live_betano_markets(fixture_id):
    ...
    if cached and now-cached[0] < LIVE_ODDS_CACHE_TTL:
        return cached[1]
    try:
        ...
        for item in response:
            ...
            if item.get('bookmakers'):
                ...
                if bookmaker:
                    break
            # Accept a direct bookmaker object too.
            if clean_text(item.get('name') or item.get('bookmakerName')) == 'betano':
                bookmaker=item
                break
        if not bookmaker:
...
def build_live_market_candidates(match):
    ...
    for market,odd in markets:
        ...
        if p<MIN_LIVE_PROBABILITY or c<MIN_LIVE_CONFIDENCE or r>MAX_LIVE_RISK or edge<0: continue
        # Extra quality gate: do not emit a market merely because odds exist.
        if edge < 3 and p < 84: continue
...
def get_best_live_signal(match):
    try:
        ...
        candidates.sort(key=lambda x:(x['score'],x['confidence'],x['probability'],-x['risk']),reverse=True)
        return candidates[0]
    except Exception as e:
...
# -------------------------
def calibrate_probability(raw_probability, sample_strength=1.0,
                          market_probability=None, min_prob=35.0,
                          max_prob=84.0):
    """
    Conservative calibration:
    - never trusts a raw model output blindly;
    - shrinks weak-sample estimates toward 50%;
    - optionally blends a market prior;
    - keeps displayed probabilities inside a realistic range.
    """
    try:
    ...
    strength = max(0.0, min(1.0, float(sample_strength)))

    # Shrink toward neutral when the sample is weak.
    p = 50.0 + (p - 50.0) * (0.55 + 0.45 * strength)

    if market_probability is not None:
        try:
            mp = float(market_probability)
            if math.isfinite(mp) and 1.0 <= mp <= 99.0:
                # Market is a sanity anchor, never the sole prediction.
                p = 0.70 * p + 0.30 * mp
        except (TypeError, ValueError):
            pass

    return round(max(min_prob, min(max_prob, p)), 1)


def probability_from_rate(rate_pct, sample_size=5):
    """Turn an empirical percentage into a conservatively calibrated probability."""
    try:
        rate = float(rate_pct)
    except (TypeError, ValueError):
        return None
    strength = min(1.0, max(0.0, float(sample_size) / 10.0))
    return calibrate_probability(rate, strength)


def fair_probability_from_odds(odd):
    try:
        odd = float(odd)
...
def realistic_edge(probability, odd):
    """Return edge percentage points relative to the fair probability implied by odds."""
    p = fair_probability_from_odds(odd)
    if probability is None or p is None:
        return None
    return round(float(probability) - p, 1)


def realistic_ev(probability, odd):
    """Expected value per 1 unit stake using the calibrated probability."""
    try:
        p = float(probability) / 100.0
        o = float(odd)
        if o <= 1.0:
            return None
        return round((p * o) - 1.0, 3)
    except (TypeError, ValueError):
        return None
...
PREMATCH_MIN_PROBABILITY = 60.0
PREMATCH_MIN_CONFIDENCE = 66.0
...
LIVE_MINUTE = 25
LIVE_MAX_MINUTE = 88
LIVE_MIN_PROBABILITY = 66.0
LIVE_MIN_CONFIDENCE = 72.0
LIVE_MAX_RISK = 35.0
LIVE_MIN_ODD = 1.50
LIVE_MAX_ODD = 4.00
LIVE_COOLDOWN = 600
...
BUILDER_MIN_LEG_PROB = 62.0
BUILDER_MAX_LEG_PROB = 84.0
BUILDER_MIN_CONFIDENCE = 72.0
BUILDER_MAX_RISK = 32.0
BUILDER_MIN_ODD = 1.50
...
def get_v7_raw_odds(fixture_id):
    if not fixture_id:
        return []
    cached = RAW_ODDS_CACHE.get(fixture_id)
    if cached is not None:
        return cached

    # get_match_odds() is the shared request/cache path. It also stores the
    # raw Betano response in RAW_ODDS_CACHE, so V7 and the legacy engine share
    # one Highlightly odds request per fixture.
    get_match_odds(fixture_id)
    return RAW_ODDS_CACHE.get(fixture_id) or []
...
def _v7_betano_from_odds(data):
    ...
    for item in data:
        if not isinstance(item, dict):
            continue
        b=_betano_bookmaker(item.get('bookmakers',[]) or [])
        if b:
            return b
        if clean_text(item.get('name') or item.get('bookmakerName')) == 'betano':
            return item
...
# ============================================================
# V8 MARKET HISTORY — REAL MATCH TOTALS
# ============================================================

V8_STAT_CACHE = {}

def _v8_fixture_stats(fixture_id):
    if not fixture_id:
        return {}
    if fixture_id in V8_STAT_CACHE:
        return V8_STAT_CACHE[fixture_id]
    try:
        # The daily scanner already persists parsed corners/shots/cards/goals.
        # Reuse it before spending another statistics request.
        cached = None
        try:
            conn = sqlite3.connect(QUOTA_DB, timeout=5)
            row = conn.execute(
            ...
            if row:
                ...
                if isinstance(cached, dict):
                    cached.pop('_parser_version', None)
        except Exception:
            cached = None

        if isinstance(cached, dict) and cached:
            V8_STAT_CACHE[fixture_id] = cached
            return cached

        rows = get_statistics(fixture_id) or []
        result = {}
        for row in rows:
            ...
            vals = {}
            for st in row.get('statistics', []) or []:
                typ = clean_text(st.get('type'))
                val = _v7_num(st.get('value'), None)
                if val is not None:
                    vals[typ] = val
            result[tid] = vals
        V8_STAT_CACHE[fixture_id] = result
        return result
    except Exception:
        V8_STAT_CACHE[fixture_id] = {}
        return {}

def _v8_team_match_history(team_id, venue):
    key = ('v8', team_id, venue)
    cached = DETAIL_HISTORY_CACHE.get(key)
    if cached and time.time() - cached[0] < DETAIL_HISTORY_CACHE_TTL:
        return cached[1]
    try:
        ...
        if cached_games is not None:
            ...
            data = api_get('fixtures', {'team': team_id, 'last': 8}) or {}
            games = data.get('response', []) or []
        selected = []
        for g in games:
            h = g.get('teams', {}).get('home', {}).get('id')
            ...
            if len(selected) >= 5:
                break
        DETAIL_HISTORY_CACHE[key] = (time.time(), selected)
        return selected
    except Exception:
        return []

def _v8_total_market_history(home_id, away_id, market):
    games = _v8_team_match_history(home_id, 'home') + _v8_team_match_history(away_id, 'away')
    values = []
    seen = set()
    for g in games:
        ...
        if not fid or fid in seen:
            continue
        seen.add(fid)
        rows = _v8_fixture_stats(fid)
        totals = []
        for tid, vals in rows.items():
            key = 'corner kicks' if market == 'corners' else 'yellow cards'
            if key in vals:
                totals.append(vals[key])
        if len(totals) >= 2:
            values.append(sum(totals))
    if len(values) < 4:
        return None
...
def _v7_prematch_candidates(match, detailed=False):
    ...
    if not fid or not home.get('id') or not away.get('id'):
        return []
    if blocked_league(league.get('name', '')) or bad_country(league.get('country', '')):
        return []

    hf = get_team_form(home['id'], 'home')
    af = get_team_form(away['id'], 'away')
    if not hf or not af:
        return []
    ...
    # Recent-match empirical evidence. This is deliberately used to
    # pull Poisson away from extreme five-match samples.
    h_over25 = _v7_num(hf.get('over25_pct'), 50.0)
    a_over25 = _v7_num(af.get('over25_pct'), 50.0)
    h_btts = _v7_num(hf.get('btts_pct'), 50.0)
    a_btts = _v7_num(af.get('btts_pct'), 50.0)

    recent_over25 = (h_over25 + a_over25) / 2.0
    recent_btts = (h_btts + a_btts) / 2.0

    raw['GOAL over 2.5'] = 0.60 * raw['GOAL over 2.5'] + 0.40 * recent_over25
    raw['GOAL under 2.5'] = 100.0 - raw['GOAL over 2.5']
    raw['BTTS yes'] = 0.60 * raw['BTTS yes'] + 0.40 * recent_btts

    # ------------------------------------------------------------
    # MARKET CONSENSUS CALIBRATION
    # ------------------------------------------------------------
    # The previous engine could produce 75-80% BTTS against a 2.90+
    # price. That is a classic overconfident-model warning.
    # Market price is not truth, but it is a strong sanity prior.
    odds = get_match_odds(fid)
    market_result = [None, None, None]
    if odds:
        market_result = _v7_norm_implied(odds[:3])

    def market_implied_for(label):
        if label == 'BTTS yes':
            val, odd = _v7_find_market(markets, 'btts', 'yes')
        elif label.startswith('GOAL '):
            target = label.replace('GOAL ', '').lower()
    ...
    def calibrate(label, stat_p):
        mi = market_implied_for(label)
        if mi is None:
            return _v7_clamp(stat_p, 5, 92)

        # 70% independent statistics / 30% market prior.
        p = 0.70 * stat_p + 0.30 * mi

        # Hard sanity cap: without a large independent sample, the
        # model cannot claim >15 percentage points above the market.
        # This removes fake 100%+ EV caused by model misspecification.
        p = min(p, mi + 15.0)

        return round(_v7_clamp(p, 5, 88), 1)

    probs = {
        'GOAL over 2.5': calibrate('GOAL over 2.5', raw['GOAL over 2.5']),
        'GOAL under 2.5': calibrate('GOAL under 2.5', raw['GOAL under 2.5']),
        'GOAL over 3.5': calibrate('GOAL over 3.5', raw['GOAL over 3.5']),
        'GOAL under 3.5': calibrate('GOAL under 3.5', raw['GOAL under 3.5']),
        'BTTS yes': calibrate('BTTS yes', raw['BTTS yes']),
    }

    # Result markets: Poisson is primary, market is a sanity prior.
    try:
        rh, rd, ra = _poisson_result_probabilities(lh, la)
        if market_result[0] is not None:
            rh = 0.72 * rh * 100 + 0.28 * market_result[0]
            rd = 0.72 * rd * 100 + 0.28 * market_result[1]
            ra = 0.72 * ra * 100 + 0.28 * market_result[2]
        probs['HOME win'] = round(_v7_clamp(rh, 28, 82), 1)
        probs['DRAW'] = round(_v7_clamp(rd, 18, 48), 1)
        probs['AWAY win'] = round(_v7_clamp(ra, 22, 82), 1)
    except Exception:
        pass

    out = []

    def add(label, odd, prob, family=None, empirical_support=0):
        if odd is None:
            return
        odd = _v7_num(odd, 0)
        prob = _v7_clamp(prob, 5, 88)
        ...
        # Never send a selection with a clearly negative model edge.
        if prob < PREMATCH_MIN_PROBABILITY or edge < 2.0:
            return

        # Very large positive edge is treated as a data-quality warning,
        # not as a reason to bet more aggressively.
        if edge > 35.0:
            return

        ev = (prob / 100.0) * odd - 1.0
        if PROFIT_MODE and (edge < PROFIT_MIN_EDGE or ev < PROFIT_MIN_EV):
            return

        agreement = empirical_support - implied
        conf = _v7_market_confidence(
            prob,
            5,
            agreement,
            hf.get('recent_form_pct', 0) + af.get('recent_form_pct', 0)
        )

        # Reduce confidence when model and market disagree strongly.
        if abs(prob - implied) > 20:
            conf -= 7

        risk = _v7_risk(prob, conf, edge, 5)
        if conf < PREMATCH_MIN_CONFIDENCE or risk > PREMATCH_MAX_RISK:
            return
        ...
        out.append({
            ...
            'edge': round(edge, 1),
            'ev': round(ev, 3),
            'family': family or _v7_family(label),
            'score': round(
                prob * 0.58 +
                conf * 0.25 +
                max(0, edge) * 0.08 -
    ...
    for label, target in [
        ...
        val, odd = _v7_find_market(markets, 'goals', target)
        key = 'GOAL ' + target
        add(label, odd, probs.get(key, 0), 'goals',
            recent_over25 if '2.5' in target else recent_over25 * 0.8 if target.startswith('over') else 100-recent_over25)

    # BTTS
    val, odd = _v7_find_market(markets, 'btts', 'yes')
    add('💎 BTTS YES', odd, probs['BTTS yes'], 'btts', recent_btts)

    # Result markets
    if odds:
        h, d, a = odds[:3]
        add('🏆 HOME WIN', h, probs.get('HOME win', 0), 'result')
        add('✈️ AWAY WIN', a, probs.get('AWAY win', 0), 'result')

    # ------------------------------------------------------------
    # CORNERS / CARDS — only with real historical match totals.
    # ------------------------------------------------------------
    if detailed:
        total_c = _v8_total_market_history(home['id'], away['id'], 'corners')
        total_y = _v8_total_market_history(home['id'], away['id'], 'cards')

        if total_c is not None:
            for target in ('under 12.5', 'under 11.5', 'under 10.5',
                           'over 7.5', 'over 8.5', 'over 9.5'):
                val, odd = _v7_find_market(markets, 'corners', target)
                if not odd:
                    continue
                line = _v7_num(target.split()[-1], 0)
                sd = max(2.0, math.sqrt(max(1.0, total_c)) * 0.70)
                if target.startswith('under'):
                    p = 0.5 * (1 + math.erf((line + 0.5 - total_c) /
                                             (sd * math.sqrt(2)))) * 100
                else:
                    p = (1 - 0.5 * (1 + math.erf((line - 0.5 - total_c) /
                                                   (sd * math.sqrt(2)))))*100
                # Shrink detailed-market estimate toward 50%; tiny samples
                # should not create 95-99% claims.
                p = 0.78 * p + 0.22 * 50
                add('🚩 CORNER ' + target.upper(), odd, p, 'corners', p)

        if total_y is not None:
            for target in ('over 2.5', 'over 3.5', 'over 4.5',
                           'over 5.5', 'under 5.5', 'under 6.5'):
                val, odd = _v7_find_market(markets, 'cards', target)
                if not odd:
                    continue
                line = _v7_num(target.split()[-1], 0)
                sd = max(1.5, math.sqrt(max(1.0, total_y)) * 0.65)
                if target.startswith('under'):
                    p = 0.5 * (1 + math.erf((line + 0.5 - total_y) /
                                             (sd * math.sqrt(2)))) * 100
                else:
                    p = (1 - 0.5 * (1 + math.erf((line - 0.5 - total_y) /
                                                   (sd * math.sqrt(2)))))*100
                p = 0.78 * p + 0.22 * 50
                add('🟨 CARD ' + target.upper(), odd, p, 'cards', p)

    # One market per family per fixture.
    best = {}
    for c in out:
        key = c['family']
...
def build_best_bet_builder(match, detailed=True):
    """Build a conservative 2-4 leg builder from independent market families.

    Every leg must already be a positive-edge V7 candidate.  Never combine
    two selections from the same family or redundant/contradictory goal lines.
    """
    candidates = _v7_prematch_candidates(match, detailed=True)
    clean = []
    seen = set()
    for c in candidates:
        try:
            p = float(c.get('probability', 0) or 0)
            conf = float(c.get('confidence', 0) or 0)
            risk = float(c.get('risk', 100) or 100)
            odd = float(c.get('odd', 0) or 0)
            edge = float(c.get('edge', 0) or 0)
        except (TypeError, ValueError):
            continue
        if p < BUILDER_MIN_LEG_PROB or conf < BUILDER_MIN_CONFIDENCE:
            continue
        if odd <= 1.10 or odd > BUILDER_MAX_ODD:
            continue
        # A builder leg with negative/zero value is never acceptable.
        if edge < 2.0 or edge > 35.0:
            continue
        if risk > BUILDER_MAX_RISK:
            continue
        family = c.get('family') or _v7_family(c.get('market', ''))
        market = str(c.get('market', ''))
        # Exact market cannot appear twice.
        key = (family, market)
        if key in seen:
            continue
        seen.add(key)
    ...
    # A single match should not produce several almost identical bets.
    def redundant(a, b):
        ma, mb = a['market'].upper(), b['market'].upper()
        # Same family is never combined.
        if a['family'] == b['family']:
            return True
        # Goal-line overlap/contradiction: UNDER/OVER on the same total-goal
        # market, including 1H goal markets, is not a useful builder pair.
        goal_tokens = ('GOAL', 'GOALS', '1H')
        if any(t in ma for t in goal_tokens) and any(t in mb for t in goal_tokens):
            if ('UNDER' in ma and 'UNDER' in mb) or ('OVER' in ma and 'OVER' in mb):
                return True
            if ('UNDER' in ma and 'OVER' in mb) or ('OVER' in ma and 'UNDER' in mb):
                # Opposite goal directions are only acceptable when they are
                # genuinely different families; this engine does not model
                # their joint distribution, so reject them.
                return True
        return False

    best = None
    # Try 2, 3 and 4 legs; prefer quality first, then joint probability.
    for n in (2, 3, 4):
        if len(clean) < n:
            continue
        for combo in combinations(clean, n):
            if len({x['family'] for x in combo}) != n:
                continue
            if any(redundant(combo[i], combo[j]) for i in range(n) for j in range(i + 1, n)):
                continue
            ...
            for leg in combo:
                odd *= float(leg['odd'])
                joint *= float(leg['probability']) / 100.0
                conf = min(conf, float(leg['confidence']))
                risk = max(risk, int(leg['risk']))
            if odd < BUILDER_MIN_ODD or odd > BUILDER_MAX_ODD:
                continue

            # Correlation penalty.  We deliberately do not pretend legs are
            # fully independent when they concern the same match dynamics.
            joint *= (0.97 ** (n - 2))
            joint_pct = joint * 100.0
            implied = 100.0 / odd
            edge = joint_pct - implied

            min_joint = {2: 55.0, 3: 48.0, 4: 42.0}[n]
            if joint_pct < min_joint:
                continue
            if edge < 3.0 or edge > 35.0:
                continue
            builder_ev = (joint_pct / 100.0) * odd - 1.0
            if PROFIT_MODE and (edge < PROFIT_MIN_EDGE or builder_ev < PROFIT_MIN_EV):
                continue

            # More legs require extra safety margin.
            risk = int(_v7_clamp(risk + max(0, 75 - joint_pct) * 0.25 + (n - 2) * 3, 10, 38))
            if risk > BUILDER_MAX_RISK:
                continue

            score = joint_pct * 0.62 + conf * 0.24 + edge * 0.10 - risk * 0.25 - (n - 2) * 1.5
            if best is None or score > best[0]:
    ...
    return {
        ...
        'league': league.get('name', ''),
        'market': '🧩 BET BUILDER',
        'probability': round(joint, 1),
        'confidence': round(conf, 1),
        'risk': int(risk),
        'odd': round(odd, 2),
        'edge': round(edge, 1),
        'ev': round((joint / 100.0) * odd - 1.0, 3),
        'score': round(score, 2),
        'builder_legs': [
            {'market': x['market'], 'odd': x['odd'], 'probability': x['probability'], 'edge': x.get('edge', 0), 'family': x['family']}
            for x in combo
        ],
        'match_date': fixture.get('date')
    }
...
# ============================================================
# MARKET LEARNING GUARD
# ============================================================

def _market_learning_ok(market):
    """Disable a market only after enough settled historical signals exist.
    This prevents a tiny sample from controlling today's scanner."""
    if not PROFIT_MODE or not market:
        return True
    try:
        conn = sqlite3.connect(DB_NAME)
        row = conn.execute(
            "SELECT COUNT(*), SUM(CASE WHEN result='WIN' THEN 1 ELSE 0 END),"
            " SUM(CASE WHEN result='WIN' THEN odd-1 ELSE -1 END)"
            " FROM signals WHERE market=? AND result IN ('WIN','LOSS') AND odd > 1.01",
            (market,)
        ).fetchone()
        conn.close()
        settled = int(row[0] or 0)
        if settled < MARKET_LEARNING_MIN_SETTLED:
            return True
        roi = (float(row[2] or 0.0) / settled) * 100.0
        return roi >= MARKET_LEARNING_MIN_ROI
    except Exception:
        return True
...
def _v7_select_normal(matches):
    ...
    logging.info('PREMATCH FILTER | raw_candidates=%s | family_candidates=%s', len(allc), len(pool))
    print(f"PREMATCH DEBUG | raw_candidates={len(allc)} | enriched_pool={len(pool)}")
    result=[]; used=set()
    for c in pool:
        if c['fixture_id'] in used: continue
        if not _market_learning_ok(c.get('market')): continue
...
def _v7_select_builders(matches):
    ...
    for m in matches:
        try:
            attempted += 1
            # This uses the same cached odds path as the normal prematch model;
            # it does not create an extra odds request.
            if get_v7_prematch_markets(m.get('fixture', {}).get('id')):
                market_ready += 1
            b = build_best_bet_builder(m, True)
            if (b and b.get('risk', 99) <= BUILDER_MAX_RISK and
                b.get('confidence', 0) >= BUILDER_MIN_CONFIDENCE and
                _market_learning_ok('🧩 BET BUILDER')):
                builders.append(b)
        except Exception as e:
            logging.warning('BUILDER MODEL ERROR: %s', repr(e))

    logging.info('BET BUILDER FILTER | fixtures=%s | odds_ready=%s | raw_candidates=%s', attempted, market_ready, len(builders))
    print(f"BUILDER DEBUG | fixtures={attempted} | odds_ready={market_ready} | raw_candidates={len(builders)}")
    builders.sort(
        key=lambda x: (x.get('probability', 0), x.get('confidence', 0), x.get('score', 0)),
        reverse=True
    )
    ...
    used = set()
    for b in builders:
        if b['fixture_id'] in used:
            continue
        result.append(b)
        used.add(b['fixture_id'])
        if len(result) >= BET_BUILDER_DAILY_TOP3:
            break
    logging.info('BET BUILDER DEBUG | fixtures=%s | odds_ready=%s | builders=%s', attempted, market_ready, len(result))
    return result
...
def _v7_live_signal(match):
    ...
    def n(d, k):
        return _v7_num(d.get(k), 0)

    h_xg, a_xg = n(hs,'expected goals'), n(aws,'expected goals')
    h_sog, a_sog = n(hs,'shots on goal'), n(aws,'shots on goal')
    h_sh, a_sh = n(hs,'total shots'), n(aws,'total shots')
    h_da, a_da = n(hs,'dangerous attacks'), n(aws,'dangerous attacks')
    h_c, a_c = n(hs,'corner kicks'), n(aws,'corner kicks')

    h_activity = h_xg*30 + h_sog*8 + h_sh*1.3 + h_da*0.16 + h_c*1.5
    a_activity = a_xg*30 + a_sog*8 + a_sh*1.3 + a_da*0.16 + a_c*1.5

    if h_sog + a_sog == 0 and h_sh + a_sh < 8 and h_da + a_da < 20:
        return None

    side = 'home' if h_activity >= a_activity else 'away'
    selected = max(h_activity, a_activity)
    other = min(h_activity, a_activity)
    total = selected + other
    dominance = (selected - other) / max(1.0, total)

    total_xg = h_xg + a_xg
    shots_on = h_sog + a_sog
    shots = h_sh + a_sh
    dangerous = h_da + a_da

    # Conditional probability that the selected side scores the NEXT goal.
    p = 50.0 + dominance * 30.0
    p += min(8.0, total_xg * 4.0)
    p += min(6.0, shots_on * 1.5)
    p += min(5.0, max(0.0, dangerous - 20.0) * 0.10)

    score_home = _v7_num(match.get('goals', {}).get('home'), 0)
    score_away = _v7_num(match.get('goals', {}).get('away'), 0)
    if score_home == score_away:
        p += 3
    elif abs(score_home - score_away) >= 2:
        p -= 2

    p = _v7_clamp(p, 55, 84)

    conf = 62.0
    conf += min(10, shots_on * 2.0)
    conf += min(8, total_xg * 5.0)
    conf += min(8, dominance * 25)
    if minute >= 55:
        conf += 2
    conf = _v7_clamp(conf, 64, 89)

    risk = 32.0
    risk -= min(10, total_xg * 5)
    risk -= min(8, shots_on * 1.5)
    risk -= min(7, dominance * 20)
    if minute < 35:
        risk += 4
    risk = int(_v7_clamp(risk, 12, 40))

    home = match.get('teams', {}).get('home', {}).get('name', 'HOME')
    away = match.get('teams', {}).get('away', {}).get('name', 'AWAY')
    odds = parse_live_next_goal_odds(fid, home, away) or {}
    ...
    # LIVE needs a real, but not enormous, price advantage.
    if p < LIVE_MIN_PROBABILITY or conf < LIVE_MIN_CONFIDENCE or risk > LIVE_MAX_RISK:
        return None
    if edge < 1.5 or edge > 35:
        return None
    live_ev = (p / 100.0) * odd - 1.0
    if PROFIT_MODE and (edge < PROFIT_MIN_EDGE or live_ev < PROFIT_MIN_EV):
        return None
    ...
    return {
        ...
        'odd': round(odd, 2),
        'edge': round(edge, 1),
        'score': round(p*0.48 + conf*0.32 + max(0, edge)*0.12 - risk*0.08, 2),
        'minute': minute,
        'attack': round(selected, 1),
        'pressure': round((shots_on*8 + dangerous*0.2 + (h_c+a_c)*3), 1),
        'xg': round(total_xg, 2),
        'home_goals': int(score_home),
        'away_goals': int(score_away)
    }

# LEGACY LIVE ENGINE RESTORED FROM MAIN V3
...
def send_live_signal(signal):
    ...
    # Final hard safety gates for LIVE output.
    try:
        if float(signal.get('odd', 0) or 0) < 1.50:
            return False
    except (TypeError, ValueError):
        return False
    if bad_country(signal.get('country')):
        return False
    key=f'{fid}_{market}'
...
def _main_match_country(match):
    league = match.get('league') or {} if isinstance(match, dict) else {}
    country = league.get('country') if isinstance(league, dict) else ''
    if isinstance(country, dict):
        country = country.get('name') or country.get('countryName') or ''
    if not country and isinstance(match, dict):
...
def _main_fixture_blocked(match):
    league = match.get('league') or {} if isinstance(match, dict) else {}
    league_name = league.get('name') if isinstance(league, dict) else ''
    country = _main_match_country(match)
    return bad_country(country) or blocked_league(league_name)


def scan_live():
    """FINAL V7 LIVE scanner.

    Uses the V7 live signal engine, which returns a dict.  Do not call the
    legacy tuple-based analyze_live_match() path here because it is not
    compatible with the final V7 signal schema.
    """
    matches = get_live_matches()
    matches = [m for m in (matches or []) if not _main_fixture_blocked(m)]

    candidates = []
    errors = 0

    print(f"LIVE DEBUG | matches={len(matches)}")

    for match in matches:
        try:
            signal = _v7_live_signal(match)
            if signal:
                candidates.append(signal)
        except Exception as exc:
            errors += 1
            logging.warning("LIVE V7 MATCH ERROR | fixture=%s | %s",
                            (match.get("fixture") or {}).get("id"),
                            repr(exc))

    candidates = rank_live_signals(candidates)
    ...
    for signal in candidates:
        try:
            ...
            logging.warning("LIVE V7 SEND ERROR | fixture=%s | %s",
                            signal.get("fixture_id"),
                            repr(exc))

    print(
        f"LIVE DEBUG | matches={len(matches)} | "
        f"candidates={len(candidates)} | sent={sent_count} | errors={errors}"
    )
    return sent_count
...
def send_prematch_signal(signal):
    ...
    # Final hard gates: blocked countries never send; every output market
    # must have odds >= 1.50. For Builder this is the combined price.
    if bad_country(signal.get('country')):
        return False
    try:
        if float(signal.get('odd', 0) or 0) < 1.50:
        ...
        return False
    # FINAL HARD GATE: never send a negative-edge prematch selection.
    if market != '🧩 BET BUILDER':
        try:
            if float(signal.get('edge', -999)) < 0:
                logging.warning('PREMATCH HARD REJECT negative edge: %s edge=%s', market, signal.get('edge'))
                return False
        except (TypeError, ValueError):
            return False
    else:
        # Builder quality is already validated by build_best_bet_builder()
        # and _v7_select_builders(). Do NOT require family/edge fields here:
        # builder_legs intentionally contains market + odd only.
        legs = signal.get('builder_legs') or []
        if len(legs) < 2 or len(legs) > 4:
            return False
        if any(
            not isinstance(leg, dict)
            or not str(leg.get('market', '')).strip()
            or float(leg.get('odd', 0) or 0) < 1.50
            for leg in legs
        ):
            return False
        if float(signal.get('edge', -999) or -999) < 0:
            return False
...
def process_prematch_matches(matches):
    ...
    logging.info('PREMATCH RESULT | whole_day=%s | normal_sent=%s | builder_sent=%s', len(matches), normal_sent, builder_sent)
...
def scan_prematch():
    global PREMATCH_DATA_READY
    try:
        raw_matches = get_prematch_matches()
        print(f"PREMATCH DEBUG | cache_ready={PREMATCH_DATA_READY} | fixtures_raw={len(raw_matches or [])}")
        if not raw_matches:
            return 0

        matches = remove_started_matches(raw_matches)
        print(f"PREMATCH DEBUG | fixtures_ns_tbd={len(matches)}")

        blocked = [m for m in matches if _main_fixture_blocked(m)]
        matches = [m for m in matches if not _main_fixture_blocked(m)]
        print(f"PREMATCH DEBUG | blocked={len(blocked)} | fixtures_usable={len(matches)}")

        if not matches:
            return 0

        sent = process_prematch_matches(matches)
        print(f"PREMATCH DEBUG | sent_total={sent}")
        return sent

    except Exception as e:
        logging.exception("PREMATCH SCAN V7 ERROR: %s", e)
        return 0
...
def print_system_status():
    print('='*60)
    print('🤖 AI FOOTBALL BOT V7 — CLEAN STATISTICAL ENGINE')
    print('STATUS: ONLINE')
    print('LIVE:',LIVE_SCAN_INTERVAL,'sec')
    print('PREMATCH:',PREMATCH_SCAN_INTERVAL,'sec')
    print('LIVE PROBABILITY:',LIVE_MIN_PROBABILITY)
    print('LIVE CONFIDENCE:',LIVE_MIN_CONFIDENCE)
    print('LIVE MAX RISK:',LIVE_MAX_RISK)
    print('PREMATCH PROBABILITY:',PREMATCH_MIN_PROBABILITY)
    print('PREMATCH CONFIDENCE:',PREMATCH_MIN_CONFIDENCE)
    print('PREMATCH MAX RISK:',PREMATCH_MAX_RISK)
    print('MAX LIVE SIGNALS:',MAX_LIVE_SIGNALS_PER_SCAN)
    print('NORMAL PREMATCH:',MAX_PREMATCH_SIGNALS_PER_SCAN)
    print('BET BUILDER TOP:',BET_BUILDER_DAILY_TOP3)
    print('='*60)
...
def main_loop():
    ...
    while True:
        ...
        try:
            ...
            # ============================================================
            # This MUST run first in the morning because it loads the exact
            ...
            # ============================================================
            # Run only after the morning statistical scanner has published
            # its fixture cache. This prevents PREMATCH/Builder from seeing
            # an empty cache and avoids a second football fixture-list call.
            if minutes >= 600 and last_day_football != day:
                try:
                    logging.info("PREMATCH MORNING FLOW START | %s", day)
                    prematch_sent = scan_prematch()
                    logging.info("PREMATCH MORNING FLOW END | sent=%s | cache_ready=%s", prematch_sent, PREMATCH_DATA_READY)
                    # If the in-memory morning fixture cache was not available,
                    # do NOT consume today's PREMATCH attempt. The next 30s
                    # scheduler cycle can retry without making a new fixture-list
                    # request. If Highlightly is quota-locked, stop for today.
                    if PREMATCH_DATA_READY:
                        last_day_football = day
                    elif hasattr(daily_scanner, "_quota_locked") and daily_scanner._quota_locked("football"):
                        last_day_football = day
                        logging.warning("PREMATCH: quota locked; no retry today")
                    else:
                        logging.warning("PREMATCH: fixture cache unavailable; will retry on next scheduler cycle")
                except Exception as exc:
                    logging.exception("PREMATCH MORNING FLOW ERROR: %s", exc)
                    if hasattr(daily_scanner, "_quota_locked") and daily_scanner._quota_locked("football"):
                        last_day_football = day

            # ============================================================
            # 3) LIVE — 17:00-00:00 BG, every 5 minutes
            # ============================================================
            # LIVE remains unchanged in its operating window and cadence.
            if 17 <= now.hour < 24 and time.time() - last_live >= 300:
                try:
                    logging.info("LIVE FLOW START | %s", now.strftime('%Y-%m-%d %H:%M:%S %Z'))
                    live_sent = scan_live()
                    logging.info("LIVE FLOW END | sent=%s", live_sent)
                except Exception as exc:
                    logging.exception("LIVE FLOW ERROR: %s", exc)
                finally:
                    last_live = time.time()

            # ============================================================
            # 4) SPORT DAILY — once at/after 10:00 BG
            # ============================================================
...
# =========================================================
# FINAL PROBABILITY / EDGE SANITY GUARD
# =========================================================
def apply_probability_sanity(signal):
    if not isinstance(signal, dict):
        return signal

    p = signal.get("probability")
    odd = signal.get("odd") or signal.get("odds")
    ...
    # No 90-99% claims from a small-sample football model.
    p = max(35.0, min(84.0, p))
    signal["probability"] = round(p, 1)

    if odd is not None:
        try:
            odd = float(odd)
            if odd > 1.0 and math.isfinite(odd):
                signal["edge"] = realistic_edge(p, odd)
                signal["ev"] = realistic_ev(p, odd)
        except (TypeError, ValueError):
            pass
