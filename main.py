# MAIN5 FINAL — consolidated production build
# Base: Main5_CLEAN_V4_BUILDER_FIXED.py
# Consolidated from the latest Main5 sources.
# Rules: one implementation per function; no duplicate builder family in one builder;
# live specialist markets remain enabled; 3+ builder legs allowed when valid.
...
CACHE_TIME_ODDS = 60             # 60 seconds - do not reuse stale bookmaker prices
...
BUILDER_MIN_LEG_PROB = 65.0
BUILDER_MIN_COMBINED_PROB = 0.0
BUILDER_MIN_CONFIDENCE = 65.0
BUILDER_MAX_RISK = 40.0
...
def _betano_bookmaker(bookmakers):
    # IMPORTANT: never identify Betano by a hard-coded numeric bookmaker id.
    # API-Football explicitly exposes bookmaker ids through /odds/bookmakers,
    # so ids can change/are not safe to assume.  Match the bookmaker by name.
...
def get_prematch_betano_builder_markets(fixture_id):
    """Return ONLY real Betano prematch markets/lines/odds.

    Rules:
      - bookmaker must be Betano by name;
      - stopped values are discarded;
      - if a Betano market contains a primary (main) value, only that
        primary value is exposed to the Builder;
      - no synthetic market/line/odd is ever created here.
    """
    odds = get_odds(fixture_id)
    if not odds:
        logging.info("BUILDER NO ODDS fixture=%s", fixture_id)
        return []
    ...
    for item in odds:
        if not isinstance(item, dict):
            continue
        bookmaker = _betano_bookmaker(item.get("bookmakers", []) or [])
        if bookmaker:
            break

    if not bookmaker:
        logging.info("BUILDER NO BETANO MARKETS fixture=%s", fixture_id)
        return []
    ...
    for bet in bookmaker.get("bets", []) or []:
        ...
        for v in bet.get("values", []) or []:
            ...
            raw_values.append({
                ...
                "odd": odd,
                "stopped": False,
                "main": bool(v.get("main", False)),
            })

        if not raw_values:
            continue

        # CRITICAL: Builder is allowed to use only a value explicitly marked
        # as the bookmaker's primary/main line. Never fall back to alternate
        # lines: those are exactly what caused false markets such as Over 3.5
        # corners to appear when the visible Betano line started at 8.5.
        # API-Football does not reliably provide the "main" flag for every
        # Betano market. Do NOT discard the market just because main=False is
        # missing. We keep the actual Betano values and apply strict market
        # validation below. No synthetic value/odd is ever created.
        result.append({"name": bname, "values": raw_values})

    logging.info(
        "BUILDER BETANO MARKETS fixture=%s markets=%d",
        fixture_id, len(result)
    )
    return result
...
def _builder_goal_lambdas(match):
    ...
    # No synthetic fallback rates. If the required form sample is missing,
    # the Builder must not invent a model probability.
    if not hf or not af:
        return None
...
def _builder_statistical_candidates(match):
    """Create Betano Builder legs from real odds plus statistical estimates.

    The Builder must not return zero merely because venue-specific history or
    one optional stats feed is missing. Goals use team form with a conservative
    fallback; corners/cards use recent stats when available and neutral
    baselines otherwise. Bookmaker odds remain mandatory.
    """
    ...
    if not markets:
        logging.info("BUILDER NO BETANO MARKETS fixture=%s", fid)
        return []

    goal_lambdas = _builder_goal_lambdas(match)
    if not goal_lambdas:
        logging.info("BUILDER NO GOAL MODEL fixture=%s", fid)
        return []
    ...
    # No neutral fallback. Missing corner/card history means no statistical
    # probability for those markets, so they are not eligible.
    corner_lambda = None
    card_lambda = None
    if home_stats and away_stats:
        # Expected FULL-MATCH total = expected home contribution + expected
        # away contribution. Each team's expectation blends what it produces
        # with what the opponent concedes. Do not divide the final total by 4.
        home_corner_exp = (
            _safe_num(home_stats.get("corners_for"), 0)
            + _safe_num(away_stats.get("corners_against"), 0)
        ) / 2.0
        away_corner_exp = (
            _safe_num(away_stats.get("corners_for"), 0)
            + _safe_num(home_stats.get("corners_against"), 0)
        ) / 2.0
        corner_lambda = home_corner_exp + away_corner_exp

        home_card_exp = (
            _safe_num(home_stats.get("cards_for"), 0)
            + _safe_num(away_stats.get("cards_against"), 0)
        ) / 2.0
        away_card_exp = (
            _safe_num(away_stats.get("cards_for"), 0)
            + _safe_num(home_stats.get("cards_against"), 0)
        ) / 2.0
        card_lambda = home_card_exp + away_card_exp

    raw = []

    def add_candidate(label, odd, probability, family):
        # odd is always the exact Betano value passed from the parser.
        odd = _safe_num(odd, 0)
        p = _safe_num(probability, 0)
        if not (1.20 <= odd <= 3.00):
            return
        # Never present an overconfident raw Poisson tail as near certainty.
        p = max(0.0, min(88.0, p))
        if p < BUILDER_MIN_LEG_PROB:
            return
        implied = 100.0 / odd
        # Keep only legs where the model is at least modestly ahead of price.
        if p + 8.0 < implied:
            return
        raw.append({
            "label": label,
    ...
    for market in markets:
        name = clean_text(market.get("name")).lower()
        values = market.get("values", []) or []
        if any(x in name for x in ("1st half", "first half", "1h", "half time")):
            continue

        # BTTS / Both Teams To Score.
        if ("both teams" in name or "btts" in name or "gg/ng" in name or
                "both teams to score" in name):
            btts_p = (1.0 - math.exp(-goal_lambdas[0])) * (1.0 - math.exp(-goal_lambdas[1])) * 100.0
            for v in values:
                val = clean_text(v.get("value")).lower()
                if val in ("yes", "btts yes", "gg", "both teams to score - yes"):
                    add_candidate("BTTS YES", v.get("odd"), btts_p, "btts")
                elif val in ("no", "btts no", "ng", "both teams to score - no"):
                    add_candidate("BTTS NO",  v.get("odd"), 100.0 - btts_p, "btts")

        # Full-match goal totals.
        if "goal" in name and not any(x in name for x in ("team", "home", "away")):
            total_lam = goal_lambdas[0] + goal_lambdas[1]
            for v in values:
                val = clean_text(v.get("value"))
                m = re.match(r"^(over|under)\s+([0-9]+(?:\.[05])?)$", val, re.I)
                if not m:
                    continue
                side, line = m.group(1).lower(), float(m.group(2))
                if line not in (1.5, 2.5, 3.5):
                    continue
                p = _builder_line_probability(side, line, total_lam) * 100.0
                add_candidate(f"GOAL {val.upper()}", v.get("odd"), p, "goal")

        # Team goals.
        if "home" in name and "goal" in name:
            for v in values:
                val = clean_text(v.get("value"))
                m = re.match(r"^over\s+([0-9]+(?:\.[05])?)$", val, re.I)
                if m and float(m.group(1)) == 1.5:
                    p = _builder_line_probability("over", 1.5, goal_lambdas[0]) * 100.0
                    add_candidate("HOME OVER 1.5 GOALS", v.get("odd"), p, "home_goal")
        ...
        if "away" in name and "goal" in name:
            for v in values:
                ...
                m = re.match(r"^over\s+([0-9]+(?:\.[05])?)$", val, re.I)
                if m and float(m.group(1)) == 1.5:
                    p = _builder_line_probability("over", 1.5, goal_lambdas[1]) * 100.0
                    add_candidate("AWAY OVER 1.5 GOALS", v.get("odd"), p, "away_goal")
        ...
        if "corner" in name and corner_lambda is not None:
            for v in values:
                ...
                if not m:
                    continue
                side, line = m.group(1).lower(), float(m.group(2))
                # Full-match Betano corners in this feed start at 8.5.
                # Never manufacture lower lines such as Over 2.5/3.5/5.5.
                if line < 8.5:
                    continue
                p = _builder_line_probability(side, line, corner_lambda) * 100.0
                add_candidate(f"CORNER {val.upper()}", v.get("odd"), p, "corner")

        # Full-match cards/bookings.
        if ("card" in name or "booking" in name) and card_lambda is not None:
            for v in values:
                ...
                if not m:
                    continue
                side, line = m.group(1).lower(), float(m.group(2))
                p = _builder_line_probability(side, line, card_lambda) * 100.0
                add_candidate(f"CARD {val.upper()}", v.get("odd"), p, "card")
    ...
    logging.info("BUILDER MARKET LEGS fixture=%s markets=%d candidates=%d", fid, len(markets), len(result))
    return result
...
def build_best_bet_builder(match):
    ...
    if len(candidates) < 2:
        logging.info("BUILDER INSUFFICIENT CANDIDATES fixture=%s count=%d", match.get("fixture", {}).get("id"), len(candidates))
        return None
    ...
    for n in range(2, min(BET_BUILDER_MAX_LEGS, len(candidates)) + 1):
        for combo in combinations(candidates, n):
            ...
            min_joint_by_legs = {2: 55.0, 3: 38.0, 4: 28.0}
            min_joint = min_joint_by_legs.get(n, 30.0)
    ...
    joint_pct = round(max(0.0, min(88.0, joint_pct)), 1)
...
def get_best_bet_builder_signals(matches):
    ...
    for match in matches:
        try:
            ...
            legs_count = len(builder.get("builder_legs", []) or [])
            min_joint = {2: 55.0, 3: 38.0, 4: 28.0}.get(legs_count, 28.0)
            if (
                _safe_num(builder.get("probability"), 0) < min_joint
                or _safe_num(builder.get("confidence"), 0) < BUILDER_MIN_CONFIDENCE
                or _safe_num(builder.get("risk"), 100) > BUILDER_MAX_RISK
                or _safe_num(builder.get("edge"), -999) < 0
            ):
                continue
