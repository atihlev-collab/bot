...
def main_loop():
    ...
    while True:
        ...
        # scanner.py owns the 11:00 / 21:00 schedule and SQLite locks.
        # Run it AFTER PREMATCH so the scheduled PREMATCH package cannot
        # be blocked by the long statistical scan at the same hour.
        # =====================================================
        try:
            run_due_scans(send_telegram)
        except Exception as exc:
            logging.warning(
                "DAILY SCANNER LOOP ERROR: %s",
                repr(exc)
            )
