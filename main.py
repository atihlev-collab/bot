...
def _strict_final_betano(signal):
    """
    Final gate immediately before Telegram.
    The exact line must exist in a fresh Betano snapshot.
    """
    fid = signal.get("fixture_id")
    bookmaker = _raw_betano(fid)
    if not bookmaker:
        return False

    lines = _betano_lines(bookmaker)

    if signal.get("market") == "🧩 BET BUILDER":
        legs = signal.get("legs") or []
        if len(legs) < MIN_BUILDER_LEGS:
            return False

        for leg in legs:
            if not _exact_line(lines, leg["family"], leg["market"]):
                return False

        return True

    family = signal.get("family")
    line = signal.get("market")

    if family in ("goals", "corners", "cards", "btts"):
        return _exact_line(lines, family, line) is not None

    return False

def send_prematch_signal(signal):
    if not _strict_final_betano(signal):
        logging.info(
            "PREMATCH BLOCKED | exact Betano market absent | fixture=%s | market=%s",
...
def _final_builder_select(matches):
    logging.info("BUILDER RUNTIME CHECK | selector active | matches=%s", len(matches or []))
    ranked=[]
    for m in matches or []:
        try:
...
def main_loop():
    ...
    # Scheduled PREMATCH package locks.
    # These are intentionally separate from the existing LIVE/statistics
    # engines above and do not alter their functions or intervals.
    PREMATCH_DAY_SENT = None
    PREMATCH_NIGHT_SENT = None
    ...
    while True:
        try:
            ...
            # ========================================================
            # LIVE — ORIGINAL ENTRY POINT, UNCHANGED
            # ========================================================
            if now - LAST_LIVE_SCAN >= LIVE_SCAN_INTERVAL:
                print(
                    bg_now.strftime("%H:%M:%S"),
                    "LIVE SCAN"
                )
            ...
            # ========================================================
            # PREMATCH — SCHEDULED PACKAGES ONLY
            #
            # 11:00 BG:
            #   TOP 3 PREMATCH + TOP 2 BET BUILDER
            #   for today's remaining matches until 00:00.
            #
            # 21:00 BG:
            #   TOP 3 PREMATCH for tomorrow 00:00-08:00.
            #
            # No other PREMATCH execution is performed here.
            # The separate LIVE/statistics code elsewhere in this file
            ...
            # --------------------------------------------------------
            # 11:00 — DAY PREMATCH + TOP 2 BUILDER
            # --------------------------------------------------------
            if bg_now.hour == 11 and PREMATCH_DAY_SENT != today:
                print(
                    bg_now.strftime("%H:%M:%S"),
                    "PREMATCH DAY PACKAGE"
                )
                ...
                try:
                    ...
                    for match in matches:
                        try:
                            ...
                            # Remaining matches of today, before midnight.
                            if (
                                match_dt.date() == today
                                and match_dt > bg_now
                            ):
                    ...
                    print(
                        "PREMATCH DAY FIXTURES:",
                        len(day_matches)
                    )

                    # TOP 3 normal PREMATCH.
                    normal = _final_prematch_select(day_matches)[:3]
                    normal_sent = 0
                    ...
                    for signal in normal:
                        try:
                            ...
                            logging.warning(
                                "DAY PREMATCH SEND ERROR: %s",
                                repr(e)
                            )

                    # TOP 2 BET BUILDER.
                    builders = _final_builder_select(day_matches)[:2]
                    builder_sent = 0
                    ...
                    for builder in builders:
                        try:
                            ...
                            logging.warning(
                                "DAY BUILDER SEND ERROR: %s",
                                repr(e)
                            )

                    print(
                        "PREMATCH DAY COMPLETE | "
                        f"fixtures={len(day_matches)} | "
                        f"normal={len(normal)} | "
                        f"builders={len(builders)} | "
                        f"sent={normal_sent + builder_sent}"
                    )

                    PREMATCH_DAY_SENT = today

                except Exception as e:
                    logging.exception(
                        "DAY PREMATCH PACKAGE ERROR: %s",
                        repr(e)
                    )

            # --------------------------------------------------------
            # 21:00 — NIGHT PREMATCH
            # --------------------------------------------------------
            if bg_now.hour == 21 and PREMATCH_NIGHT_SENT != today:
                print(
                    bg_now.strftime("%H:%M:%S"),
                    "PREMATCH NIGHT PACKAGE"
                )
                ...
                try:
                    ...
                    for match in matches:
                        try:
                            ...
                            # Tomorrow 00:00 through 07:59 BG time.
                            if (
                                match_dt.date() == tomorrow
                                and 0 <= match_dt.hour < 8
                            ):
                    ...
                    print(
                        "PREMATCH NIGHT FIXTURES:",
                        len(night_matches)
                    )

                    # TOP 3 normal PREMATCH only.
                    normal = _final_prematch_select(night_matches)[:3]
                    night_sent = 0
                    ...
                    print(
                        "PREMATCH NIGHT COMPLETE | "
                        f"fixtures={len(night_matches)} | "
                        f"normal={len(normal)} | "
                        f"sent={night_sent}"
                    )

                    PREMATCH_NIGHT_SENT = today

                except Exception as e:
                    logging.exception(
                        "NIGHT PREMATCH PACKAGE ERROR: %s",
                        repr(e)
                    )

            # ========================================================
            # Existing result handling is not present in the original
            # main3 entry point, so nothing new is introduced here.
            # ========================================================
            ...
            logging.exception("MAIN LOOP ERROR: %s", repr(e))
            time.sleep(5)
