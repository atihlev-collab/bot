...
def scan_live():
    try:
        matches=get_live_matches()
        # Russia/Belarus are blocked BEFORE any live odds/statistics request.
        ...
        for m in matches or []:
            try:
                ...
                if get_live_betano_markets(fid):
                    odds_ready += 1
                if get_live_market_stats(m):
                    stats_ready += 1
        ...
        print(f'LIVE DEBUG | matches={len(matches or [])} | odds_ready={odds_ready} | stats_ready={stats_ready} | candidates={len(candidates)} | qualified={len(ranked)} | sent={sent}')
        return sent
    except Exception as e:
