...
BUILDER_MIN_COMBINED_PROB_FINAL = 50.0
BUILDER_MIN_EDGE_FINAL = -0.5
    ...
    for match in matches:
        try:
            ...
            if (
                ...
                logging.info(
                    "BUILDER REJECT | fixture=%s | prob=%.1f | conf=%.1f | edge=%.1f | risk=%.1f",
                    builder.get("fixture_id"),
                    _safe_num(builder.get("probability"), 0),
                    _safe_num(builder.get("confidence"), 0),
                    _safe_num(builder.get("edge"), -999),
                    _safe_num(builder.get("risk"), 100),
                )
                continue
