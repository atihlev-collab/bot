...
def _normalize_highlightly_match(m, timezone_name=None):
    ...
    state_text = f"{upper} {state_type}"

    if "NOT STARTED" in state_text or "SCHEDULED" in state_text or state_type in ("NS", "NOT_STARTED", "UPCOMING"):
        status_short = "NS"
    elif "FINISHED" in state_text or "AFTER PENALTIES" in state_text or state_type in ("FT", "FINISHED"):
        status_short = "FT"
    elif state_type in ("LIVE", "IN_PLAY", "INPLAY") or any(x in state_text for x in (
...
def get_statistics(fixture_id):
    ...
    logging.info("FOOTBALL STATISTICS REQUEST | fixture=%s", fixture_id)
    ...
    if not isinstance(data, dict):
        logging.warning("FOOTBALL STATISTICS EMPTY RESPONSE | fixture=%s", fixture_id)
        return []
    ...
    logging.info(
        "FOOTBALL STATISTICS RESULT | fixture=%s | teams=%s",
        fixture_id,
        len(result)
    )
...
def scan_live():
    ...
    try:
        ...
        if not matches:
            logging.info(
                "LIVE STATISTICS SKIPPED | no live matches returned by Highlightly"
            )
            return 0
