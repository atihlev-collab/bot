            ...
            for market_name, values in markets:
                ...
                for value, odd in values:
                    ...
                    if len(history) >= 6:
                        ...
                        home_mean = sum(home_history) / len(home_history) if home_history else None
                        away_mean = sum(away_history) / len(away_history) if away_history else None
                        mean = (
                            home_mean + away_mean
                            if home_mean is not None and away_mean is not None
                            else (home_mean if home_mean is not None else (away_mean if away_mean is not None else total_c))
                        )

                        variance = (
                            sum((x - mean) ** 2 for x in history)
                            / max(1, len(history) - 1)
                        )
                ...
                for value, odd in values:
                    ...
                    if len(history) >= 6:
                        ...
                        mean = (
                            home_mean + away_mean
                            if home_mean is not None and away_mean is not None
                            else (home_mean if home_mean is not None else (away_mean if away_mean is not None else total_y))
                        )
