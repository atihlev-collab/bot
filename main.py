...
def main_loop():
    ...
    while True:
        ...
        if now-LAST_LIVE_SCAN>=LIVE_SCAN_INTERVAL:
            print(datetime.now(TIMEZONE).strftime('%H:%M:%S'),'LIVE SCAN')
            sent=scan_live(); print('LIVE SIGNALS SENT:',sent); LAST_LIVE_SCAN=now
        if now-LAST_PREMATCH_SCAN>=PREMATCH_SCAN_INTERVAL:
            print(datetime.now(TIMEZONE).strftime('%H:%M:%S'),'PREMATCH SCAN')
            sent=scan_prematch(); print('PREMATCH SIGNALS SENT:',sent); LAST_PREMATCH_SCAN=now
        time.sleep(5)

# ============================================================
# AI FOOTBALL BOT V10 — FINAL CLEAN OVERRIDES
# ============================================================
# Purpose:
#   1) LIVE uses real live Betano markets + live statistics.
#   2) LIVE can select several market types, not only NEXT GOAL.
#   3) PREMATCH is re-evaluated every scan; a zero-result first scan
#      no longer disables the rest of the day.
#   4) PREMATCH uses model probability + bookmaker sanity check + form.
#   5) BET BUILDER supports 2-5 legs, but only different market families.
#   6) No synthetic odds and no signal without an actual Betano price.
# ============================================================
...
# FINAL V10 CONFIG
# -------------------------
PREMATCH_SCAN_INTERVAL = 300          # 5 min
LIVE_SCAN_INTERVAL = 30               # 30 sec

MAX_PREMATCH_SIGNALS_PER_SCAN = 5
MAX_LIVE_SIGNALS_PER_SCAN = 5

PREMATCH_MIN_PROBABILITY = 69.0
PREMATCH_MIN_CONFIDENCE = 74.0
PREMATCH_MAX_RISK = 31.0
PREMATCH_MIN_ODD = 1.45
PREMATCH_MAX_ODD = 3.50

LIVE_MINUTE = 20
LIVE_MAX_MINUTE = 90
LIVE_MIN_PROBABILITY = 70.0
LIVE_MIN_CONFIDENCE = 78.0
LIVE_MAX_RISK = 32.0
LIVE_MIN_ODD = 1.25
LIVE_MAX_ODD = 4.00
LIVE_COOLDOWN = 480
...
BET_BUILDER_DAILY_TOP3 = 3
BUILDER_MIN_LEG_PROB = 72.0
BUILDER_MIN_LEG_CONFIDENCE = 74.0
BUILDER_MAX_LEG_RISK = 31.0
BUILDER_MIN_ODD = 1.80
BUILDER_MAX_ODD = 6.00
BET_BUILDER_MIN_LEGS = 2
BET_BUILDER_MAX_LEGS = 5

V10_PREMATCH_SENT = set()
V10_BUILDER_SENT = set()
V10_LIVE_SENT = {}
...
def _v10_num(value, default=0.0):
    try:
        x = safe_float(value)
        return default if x is None else float(x)
    except Exception:
...
def _v10_clamp(value, lo, hi):
    return max(lo, min(hi, _v10_num(value, lo)))


def _v10_market_family(label):
    s = clean_text(label)
    if 'btts' in s:
        return 'btts'
    if 'home win' in s or 'away win' in s or s == 'draw':
        return 'result'
    if 'home over' in s:
        return 'home_goals'
    if 'away over' in s:
    ...
    if 'card' in s or 'booking' in s:
        return 'cards'
    if 'goal' in s or 'over 2.5' in s or 'under 2.5' in s or 'over 3.5' in s or 'under 3.5' in s:
        return 'goals'
    return s


def _v10_norm_result_prior(odds):
    vals = []
    for odd in odds:
        odd = _v10_num(odd, 0)
        vals.append(100.0 / odd if odd > 1.01 else 0.0)
    total = sum(vals)
    if total <= 0:
        return [None] * len(vals)
    return [x / total * 100.0 if x else None for x in vals]


def _v10_poisson_total(lam, line, over=True):
    lam = _v10_clamp(lam, 0.10, 5.0)
    k = int(math.floor(line))
    if over:
        p = 1.0 - poisson.cdf(k, lam)
    else:
        p = poisson.cdf(k, lam)
    return _v10_clamp(p * 100.0, 5.0, 95.0)


def _v10_team_over15(lam):
    lam = _v10_clamp(lam, 0.10, 4.0)
    return _v10_clamp((1.0 - poisson.cdf(1, lam)) * 100.0, 5.0, 95.0)


def _v10_btts(lh, la):
    lh = _v10_clamp(lh, 0.10, 4.0)
    la = _v10_clamp(la, 0.10, 4.0)
    return _v10_clamp((1 - math.exp(-lh)) * (1 - math.exp(-la)) * 100.0, 5.0, 95.0)


def _v10_goal_lambdas(home_form, away_form):
    h_att = _v10_num(home_form.get('avg_scored'), 0)
    h_def = _v10_num(home_form.get('avg_conceded'), 0)
    a_att = _v10_num(away_form.get('avg_scored'), 0)
    a_def = _v10_num(away_form.get('avg_conceded'), 0)

    # Venue form is useful, but shrink the five-match sample.
    lh = 0.60 * h_att + 0.40 * a_def
    la = 0.60 * a_att + 0.40 * h_def
    lh = 0.78 * lh + 0.22 * 1.25
    la = 0.78 * la + 0.22 * 1.05
    return _v10_clamp(lh, 0.20, 3.20), _v10_clamp(la, 0.20, 3.20)


def _v10_confidence(prob, form_a, form_b, edge, market):
    c = 62.0
    c += max(0.0, prob - 60.0) * 0.42
    c += max(0.0, min(10.0, (form_a + form_b - 110.0) * 0.10))
    c += max(0.0, min(7.0, edge * 0.20))

    if market in ('🚀 OVER 2.5 GOALS', '💎 BTTS YES'):
        if form_a >= 70 and form_b >= 70:
            c += 4
    if market in ('🏆 HOME WIN', '✈️ AWAY WIN'):
        if abs(form_a - form_b) >= 12:
            c += 3

    return round(_v10_clamp(c, 60.0, 95.0), 1)


def _v10_risk(prob, confidence, edge, form_a, form_b, sample_ok=True):
    r = 31.0
    if prob < 72:
        r += 7
    elif prob < 76:
    ...
    if edge < 2:
        r += 5
    if abs(form_a - form_b) < 5:
        r += 4
    if not sample_ok:
        r += 5
    return int(round(_v10_clamp(r, 5, 45)))


def _v10_add_prematch(out, fid, home, away, league, market, odd, prob, hf, af):
    if odd is None:
        return
    odd = _v10_num(odd, 0)
    prob = _v10_clamp(prob, 5, 95)
    if odd < PREMATCH_MIN_ODD or odd > PREMATCH_MAX_ODD:
        return

    edge = prob - 100.0 / odd

    # Avoid "model says 80%" with a market that implies 75% and no support.
    if prob < PREMATCH_MIN_PROBABILITY or edge < 1.0:
        return

    conf = _v10_confidence(
        prob,
    ...
    risk = _v10_risk(
        ...
        hf.get('form_pct', 0),
        af.get('form_pct', 0),
        hf.get('played', 0) >= 5 and af.get('played', 0) >= 5
    )
    ...
    out.append({
        ...
        'odd': round(odd, 2),
        'edge': round(edge, 1),
        'ev': round(prob / 100.0 * odd - 1.0, 3),
        'family': _v10_market_family(market),
...
def _v10_prematch_candidates(match, detailed=False):
    try:
        fixture = match.get('fixture', {})
        teams = match.get('teams', {})
        home = teams.get('home', {})
        ...
        hf = get_team_form(home['id'], venue='home')
        af = get_team_form(away['id'], venue='away')
        if not hf or not af:
            return []

        odds = get_v7_prematch_markets(fid)
        if not odds:
            return []

        lh, la = _v10_goal_lambdas(hf, af)
        total = lh + la

        probs = {
            'over 2.5': _v10_poisson_total(total, 2.5, True),
            'under 2.5': _v10_poisson_total(total, 2.5, False),
            'over 3.5': _v10_poisson_total(total, 3.5, True),
            'under 3.5': _v10_poisson_total(total, 3.5, False),
            'btts': _v10_btts(lh, la),
            'home15': _v10_team_over15(lh),
            'away15': _v10_team_over15(la),
        }

        # Result model: Poisson result + normalized Betano prior.
        try:
            # Reuse the existing Poisson result helper if present.
            rh, rd, ra = _poisson_result_probabilities(lh, la)
            base = [rh * 100.0, rd * 100.0, ra * 100.0]

            core = get_match_odds(fid)
            if core:
                prior = _v10_norm_result_prior(core[:3])
            else:
                prior = [None, None, None]

            if prior[0] is not None:
                rh = 0.72 * base[0] + 0.28 * prior[0]
                rd = 0.72 * base[1] + 0.28 * prior[1]
                ra = 0.72 * base[2] + 0.28 * prior[2]
            else:
                rh, rd, ra = base

            probs['home_win'] = _v10_clamp(rh, 25, 88)
            probs['away_win'] = _v10_clamp(ra, 22, 88)
        except Exception:
            pass

        out = []

        def find(kind, target):
            _, odd = _v7_find_market(odds, kind, target)
            return odd

        _v10_add_prematch(
            out, fid, home, away, league,
            '🚀 OVER 2.5 GOALS', find('goals', 'over 2.5'),
            probs['over 2.5'], hf, af
        )
        _v10_add_prematch(
            out, fid, home, away, league,
            '🛡 UNDER 2.5 GOALS', find('goals', 'under 2.5'),
            probs['under 2.5'], hf, af
        )
        _v10_add_prematch(
            out, fid, home, away, league,
            '🔥 OVER 3.5 GOALS', find('goals', 'over 3.5'),
            probs['over 3.5'], hf, af
        )
        _v10_add_prematch(
            out, fid, home, away, league,
            '🛡 UNDER 3.5 GOALS', find('goals', 'under 3.5'),
            probs['under 3.5'], hf, af
        )
        _v10_add_prematch(
            out, fid, home, away, league,
            '💎 BTTS YES', find('btts', 'yes'),
            probs['btts'], hf, af
        )

        # Team goals: use only the clearly team-specific bookmaker market.
        for label, team_side, p in (
            ('⚽ HOME OVER 1.5 GOALS', 'home', probs['home15']),
            ('⚽ AWAY OVER 1.5 GOALS', 'away', probs['away15']),
        ):
            odd = None
            for name, vals in odds:
                if 'goal' not in name:
                    continue
                if team_side not in name:
                    continue
                if 'half' in name:
                    continue
                for value, vo in vals:
                    if value.startswith('over 1.5'):
                        odd = vo
                        break
                if odd is not None:
                    break
            _v10_add_prematch(out, fid, home, away, league, label, odd, p, hf, af)

        core_odds = get_match_odds(fid)
        if core_odds:
            _v10_add_prematch(
                out, fid, home, away, league,
                '🏆 HOME WIN', core_odds[0],
                probs.get('home_win', 0), hf, af
            )
            _v10_add_prematch(
                out, fid, home, away, league,
                '✈️ AWAY WIN', core_odds[2],
                probs.get('away_win', 0), hf, af
            )

        # Corners/cards only on the strongest detailed candidates.
        if detailed:
            total_c = _v7_total_market_history(home['id'], away['id'], 'corners')
            total_y = _v7_total_market_history(home['id'], away['id'], 'cards')

            if total_c is not None:
                for target in ('over 7.5', 'over 8.5', 'over 9.5', 'under 12.5'):
                    odd = find('corners', target)
                    if odd is None:
                        continue
                    line = _v10_num(target.split()[-1], 0)
                    sd = max(1.8, math.sqrt(max(1.0, total_c)) * 0.65)
                    if target.startswith('under'):
                        p = 0.5 * (1 + math.erf((line + 0.5 - total_c) / (sd * math.sqrt(2)))) * 100
                    else:
                        p = (1 - 0.5 * (1 + math.erf((line - 0.5 - total_c) / (sd * math.sqrt(2)))) * 1) * 100
                    _v10_add_prematch(
                        out, fid, home, away, league,
                        '🚩 CORNER ' + target.upper(), odd, p, hf, af
                    )

            if total_y is not None:
                for target in ('over 3.5', 'over 4.5', 'under 6.5'):
                    odd = find('cards', target)
                    if odd is None:
                        continue
                    line = _v10_num(target.split()[-1], 0)
                    sd = max(1.2, math.sqrt(max(1.0, total_y)) * 0.55)
                    if target.startswith('under'):
                        p = 0.5 * (1 + math.erf((line + 0.5 - total_y) / (sd * math.sqrt(2)))) * 100
                    else:
                        p = (1 - 0.5 * (1 + math.erf((line - 0.5 - total_y) / (sd * math.sqrt(2)))) * 1) * 100
                    _v10_add_prematch(
                        out, fid, home, away, league,
                        '🟨 CARD ' + target.upper(), odd, p, hf, af
                    )

        for s in out:
            s['match_date'] = fixture.get('date')

        return out
    except Exception as e:
        logging.warning('V10 PREMATCH MODEL ERROR: %s', repr(e))
        return []


def analyze_prematch(match):
    return _v10_prematch_candidates(match, detailed=False)


# ============================================================
# V10 BET BUILDER — 2 TO 5 LEGS
# ============================================================

def build_best_bet_builder(match, detailed=True):
    candidates = _v10_prematch_candidates(match, detailed=detailed)
    if not candidates:
        return None
    ...
    candidates = [
        ...
        and c.get('confidence', 0) >= BUILDER_MIN_LEG_CONFIDENCE
        and c.get('risk', 100) <= BUILDER_MAX_LEG_RISK
        and c.get('odd', 0) >= 1.05
        and c.get('odd', 0) <= 2.20
    ]

    # One best leg per family. This prevents e.g. OVER 2.5 + UNDER 3.5
    # from being treated as independent evidence.
    by_family = {}
    for c in candidates:
        fam = c.get('family')
        if fam not in by_family or c['score'] > by_family[fam]['score']:
            by_family[fam] = c
    ...
    for n in range(BET_BUILDER_MIN_LEGS, max_n + 1):
        for combo in combinations(candidates, n):
            ...
            fams = {x['family'] for x in combo}
            if 'goals' in fams and 'btts' in fams:
                joint *= 0.90
            if 'home_goals' in fams and 'goals' in fams:
                joint *= 0.92
            if 'away_goals' in fams and 'goals' in fams:
                joint *= 0.92
            if 'corners' in fams and 'cards' in fams:
                joint *= 0.97

            joint_pct = min(92.0, joint * 100.0)
            min_required = {2: 38, 3: 28, 4: 20, 5: 14}.get(n, 14)
            if joint_pct < min_required:
                continue

            min_conf = min(x['confidence'] for x in combo)
            max_risk = max(x['risk'] for x in combo)

            # Builder score favors real value, but penalizes excessive legs.
            edge = joint_pct - 100.0 / odd
            score = (
                joint_pct * 0.58
                + min_conf * 0.22
                + max(0, edge) * 0.16
            ...
            if best is None or score > best[0]:
                best = (score, combo, odd, joint_pct, min_conf, max_risk, edge)
    ...
    return {
        ...
        'confidence': round(conf, 1),
        'risk': round(_v10_clamp(risk + max(0, 75 - joint) * 0.12, 8, 40), 1),
        'odd': round(odd, 2),
        'edge': round(edge, 1),
...
# ============================================================
# V10 PREMATCH SELECTION
# ============================================================

def _v10_send_prematch(signal):
    ...
    V10_PREMATCH_SENT.add(key)
    save_signal(signal)
    remember_signal(signal)
    return True


def _v10_select_prematch(matches):
    all_candidates = []
    for match in matches:
        try:
            all_candidates.extend(_v10_prematch_candidates(match, detailed=False))
        except Exception as e:
            logging.warning('V10 PREMATCH CANDIDATE ERROR: %s', repr(e))

    if not all_candidates:
        return []

    all_candidates.sort(
        key=lambda x: (x['score'], x['probability'], x['confidence'], x['edge']),
        reverse=True
    )

    # Enrich up to 35 different fixtures.
    fixture_ids = []
    seen = set()
    for c in all_candidates:
        if c['fixture_id'] in seen:
            continue
        seen.add(c['fixture_id'])
        fixture_ids.append(c['fixture_id'])
        if len(fixture_ids) >= DETAIL_HISTORY_TOP_N:
            break

    by_id = {m.get('fixture', {}).get('id'): m for m in matches}
    enriched = []
    for fid in fixture_ids:
        ...
        if not m:
            continue
        try:
            enriched.extend(_v10_prematch_candidates(m, detailed=True))
        except Exception as e:
            logging.warning('V10 DETAIL ERROR: %s', repr(e))

    pool = all_candidates + enriched

    # Keep the best market per fixture + family.
    best_family = {}
    for c in pool:
        key = (c['fixture_id'], c['family'])
        if key not in best_family or c['score'] > best_family[key]['score']:
            best_family[key] = c

    pool = list(best_family.values())
    pool.sort(
        key=lambda x: (x['score'], x['probability'], x['confidence'], x['edge']),
        reverse=True
    )

    # Return the best unsent opportunities, not simply the first five.
    result = []
    for c in pool:
        if f"{c['fixture_id']}|{c['market']}" in V10_PREMATCH_SENT:
            continue
        result.append(c)
        if len(result) >= MAX_PREMATCH_SIGNALS_PER_SCAN:
            break
    return result


def _v10_select_builders(matches):
    # Evaluate builders from the strongest fixtures of the day, but don't
    # permanently mark the day as done when no builder exists yet.
    quick = []
    for m in matches:
        try:
            cs = _v10_prematch_candidates(m, detailed=False)
            if cs:
                quick.append(max(cs, key=lambda x: x['score']))
        except Exception:
            continue
    ...
    for c in quick[:DETAIL_HISTORY_TOP_N]:
        fid = c['fixture_id']
        if fid in seen:
            continue
        m = next((x for x in matches if x.get('fixture', {}).get('id') == fid), None)
    ...
    for m in selected:
        try:
            b = build_best_bet_builder(m, detailed=True)
            if not b:
                continue
            if b['confidence'] < BUILDER_MIN_LEG_CONFIDENCE:
                continue
            if b['risk'] > 34:
                continue
            if len(b.get('builder_legs', [])) < 2:
                continue
            builders.append(b)
        except Exception as e:
            logging.warning('V10 BUILDER ERROR: %s', repr(e))

    builders.sort(
        key=lambda x: (x['score'], x['probability'], x['edge'], -x['risk']),
        reverse=True
    )
    ...
    used = set()
    for b in builders:
        key = f"{b['fixture_id']}|BUILDER"
        if b['fixture_id'] in used or key in V10_BUILDER_SENT:
            continue
        result.append(b)
...
def process_prematch_matches(matches):
    # IMPORTANT: no daily "done" flag. Odds and match quality change during
    # the day, so every scan can discover a new unsent opportunity.
    normals = _v10_select_prematch(matches)
    normal_sent = 0
    for signal in normals:
        if _v10_send_prematch(signal):
            normal_sent += 1

    builders = _v10_select_builders(matches)
    builder_sent = 0
    for signal in builders:
        key = f"{signal['fixture_id']}|BUILDER"
        if key in V10_BUILDER_SENT:
            continue
        if _v10_send_prematch(signal):
            V10_BUILDER_SENT.add(key)
            builder_sent += 1

    print(
        f'PREMATCH V10 | matches={len(matches)} | '
        f'normal={normal_sent} | builder={builder_sent}'
    )
    return normal_sent + builder_sent
...
def scan_prematch():
    try:
        matches = remove_started_matches(get_prematch_matches())
        if not matches:
            print('PREMATCH V10 | no upcoming matches')
            return 0
        return process_prematch_matches(matches)
    except Exception as e:
        logging.warning('PREMATCH V10 ERROR: %s', repr(e))
        return 0


# ============================================================
# V10 LIVE ENGINE — MULTI MARKET
# ============================================================

def _v10_live_stats(match):
    stats = get_live_market_stats(match) or {}
    hs = stats.get('home', {}) or {}
    aws = stats.get('away', {}) or {}
    ...
    return {
        'hs': hs,
        'aws': aws,
        'h_shots': _v10_num(hs.get('total shots'), 0),
        'a_shots': _v10_num(aws.get('total shots'), 0),
        'h_on': _v10_num(hs.get('shots on goal'), 0),
        'a_on': _v10_num(aws.get('shots on goal'), 0),
        'h_danger': _v10_num(hs.get('dangerous attacks'), 0),
        'a_danger': _v10_num(aws.get('dangerous attacks'), 0),
        'h_xg': _v10_num(hs.get('expected goals'), 0),
        'a_xg': _v10_num(aws.get('expected goals'), 0),
        'h_corners': _v10_num(hs.get('corner kicks'), 0),
        'a_corners': _v10_num(aws.get('corner kicks'), 0),
        'h_yellow': _v10_num(hs.get('yellow cards'), 0),
        'a_yellow': _v10_num(aws.get('yellow cards'), 0),
        'h_fouls': _v10_num(hs.get('fouls'), 0),
        'a_fouls': _v10_num(aws.get('fouls'), 0),
    }


def _v10_live_intensity(d):
    return (
        (d['h_shots'] + d['a_shots']) * 1.25
        + (d['h_on'] + d['a_on']) * 9.0
        + (d['h_danger'] + d['a_danger']) * 0.17
        + (d['h_xg'] + d['a_xg']) * 16.0
        + (d['h_corners'] + d['a_corners']) * 1.7
    )


def _v10_live_candidate(match, market, odd, probability, confidence, risk, score, extra=None):
    if odd is None:
        return None
    odd = _v10_num(odd, 0)
    if odd < LIVE_MIN_ODD or odd > LIVE_MAX_ODD:
    ...
    s = {
        ...
        'risk': int(round(risk)),
        'odd': round(odd, 2),
        'edge': round(value_edge(probability, odd), 1),
        'score': round(score, 2),
        'minute': int(_v10_num(fixture.get('status', {}).get('elapsed'), 0)),
        'home_goals': _v10_num(match.get('goals', {}).get('home'), 0),
        'away_goals': _v10_num(match.get('goals', {}).get('away'), 0),
    }
    if extra:
        s.update(extra)
...
def _v10_live_candidates(match):
    try:
        fixture = match.get('fixture', {})
        fid = fixture.get('id')
        minute = int(_v10_num(fixture.get('status', {}).get('elapsed'), 0))
        ...
        d = _v10_live_stats(match)
        total_shots = d['h_shots'] + d['a_shots']
        shots_on = d['h_on'] + d['a_on']
        danger = d['h_danger'] + d['a_danger']
        xg = d['h_xg'] + d['a_xg']
        corners = d['h_corners'] + d['a_corners']
        yellow = d['h_yellow'] + d['a_yellow']

        intensity = _v10_live_intensity(d)

        # If the provider is returning no useful live statistics, do not
        # invent a signal just because an odds market exists.
        if intensity < 13 and shots_on == 0 and danger < 12:
            return []

        hg = _v10_num(match.get('goals', {}).get('home'), 0)
        ag = _v10_num(match.get('goals', {}).get('away'), 0)
        total_goals = hg + ag
        ...
        # -------------------------
        h_activity = (
            d['h_shots'] * 1.2 + d['h_on'] * 9
            + d['h_danger'] * 0.18 + d['h_xg'] * 16
            + d['h_corners'] * 1.5
        )
        a_activity = (
            d['a_shots'] * 1.2 + d['a_on'] * 9
            + d['a_danger'] * 0.18 + d['a_xg'] * 16
            + d['a_corners'] * 1.5
        )
        ...
        if selected > 0:
            ...
            p = 51.0 + dominance * 34.0 + min(7.0, max(0.0, selected - 25.0) * 0.12)
            if abs(hg - ag) <= 1:
                p += 2.0
            if minute >= 70:
                p += 1.5
            if shots_on >= 4:
                p += 2.0
            p = _v10_clamp(p, 58, 90)

            odd = find_live_market_odd(markets, 'next_goal', side=side)
            conf = 68 + min(10, shots_on * 1.8) + min(7, xg * 4.0) + min(7, dominance * 20)
            conf = _v10_clamp(conf, 68, 93)
            risk = 18
            if shots_on < 2:
                risk += 6
            if xg < 0.35:
            ...
            market = '🎯 NEXT GOAL HOME' if side == 'home' else '🎯 NEXT GOAL AWAY'
            c = _v10_live_candidate(
                match, market, odd, p, conf, risk,
                p * 0.52 + conf * 0.30 + max(0, value_edge(p, odd or 0)) * 0.12 - risk * 0.08,
                {
                    'attack': round(selected, 1),
                    'pressure': round(intensity, 1),
                    'xg': round(xg, 2),
                }
            )
            if c:
        ...
        # OVER 1.5 GOALS
        # -------------------------
        odd_15 = find_live_market_odd(markets, 'goals', target='over 1.5')
        if odd_15 is not None:
            # Current total is part of the event. If already 2+, it is
            # already settled and should not be sent as a live bet.
            if total_goals < 2:
                remaining_lambda = max(
                    0.08,
                    (xg * 0.70)
                    + (total_shots * 0.018)
                    + (shots_on * 0.075)
                ...
                # Later minutes need a stronger threshold because time is short.
                time_factor = max(0.20, (95 - minute) / 70.0)
                remaining_lambda *= time_factor
                p15 = (1 - math.exp(-remaining_lambda)) * 100.0
                if total_goals == 1:
                    p15 = max(p15, 58 + min(28, intensity * 0.16))
                p15 = _v10_clamp(p15, 50, 91)

                conf15 = 65 + min(12, shots_on * 1.8) + min(8, xg * 3.5) + min(7, intensity * 0.05)
                risk15 = 17
                if shots_on < 2:
                    risk15 += 7
                if xg < 0.30:
                ...
                c = _v10_live_candidate(
                    match, '⚽ OVER 1.5 GOALS', odd_15, p15,
                    _v10_clamp(conf15, 65, 94), risk15,
                    p15 * 0.50 + conf15 * 0.31 + max(0, value_edge(p15, odd_15)) * 0.11 - risk15 * 0.08,
                    {'attack': round(intensity, 1), 'pressure': round(intensity, 1), 'xg': round(xg, 2)}
                )
                if c:
                    result.append(c)
        ...
        # CORNERS
        # -------------------------
        for target in ('over 1.5', 'over 2.5', 'over 3.5'):
            odd = find_live_market_odd(markets, 'corners', target=target)
            if odd is None:
                continue

            line = _v10_num(target.split()[-1], 0)
            remaining_need = max(0.0, line - corners)

            if remaining_need <= 0:
                continue

            corner_rate = corners / max(1.0, minute)
            pressure_rate = (d['h_danger'] + d['a_danger']) / max(1.0, minute)
            shot_rate = total_shots / max(1.0, minute)

            expected_remaining = (
                corner_rate * max(1, 92 - minute)
                + pressure_rate * 0.22 * max(1, 92 - minute)
                + shot_rate * 0.10 * max(1, 92 - minute)
            )
            p_corner = _v10_clamp(
                50 + (expected_remaining - remaining_need) * 16
                + min(12, corners * 1.8),
                45, 93
            )

            conf_corner = 67 + min(12, corners * 1.6) + min(8, intensity * 0.04)
            risk_corner = 18
            if corners < 2:
                risk_corner += 6
            if total_shots < 8:
            ...
            label = '🚩 OVER ' + target.upper() + ' CORNERS'
            c = _v10_live_candidate(
                match, label, odd, p_corner,
                _v10_clamp(conf_corner, 65, 93), risk_corner,
                p_corner * 0.48 + conf_corner * 0.32
                + max(0, value_edge(p_corner, odd)) * 0.12
                - risk_corner * 0.08,
                {'attack': round(intensity, 1), 'pressure': round(intensity, 1), 'xg': round(xg, 2)}
            )
            if c:
        ...
        # -------------------------
        for target in ('over 1.5', 'over 2.5', 'over 3.5'):
            odd = find_live_market_odd(markets, 'cards', target=target)
            if odd is None:
                continue

            line = _v10_num(target.split()[-1], 0)
            remaining_need = max(0.0, line - yellow)
            if remaining_need <= 0:
                continue

            foul_rate = (d['h_fouls'] + d['a_fouls']) / max(1.0, minute)
            expected_remaining = foul_rate * max(1, 92 - minute) * 0.22
            p_card = _v10_clamp(
                50 + (expected_remaining - remaining_need) * 15
                + min(12, yellow * 3.0),
                45, 93
            )

            conf_card = 66 + min(12, yellow * 3.0) + min(10, (d['h_fouls'] + d['a_fouls']) * 0.35)
            risk_card = 19
            if yellow == 0:
                risk_card += 8
            if d['h_fouls'] + d['a_fouls'] < 16:
                risk_card += 6
            if minute < 55:
                risk_card += 4

            label = '🟨 OVER ' + target.upper() + ' CARDS'
            c = _v10_live_candidate(
                match, label, odd, p_card,
                _v10_clamp(conf_card, 65, 93), risk_card,
                p_card * 0.48 + conf_card * 0.31
                + max(0, value_edge(p_card, odd)) * 0.12
                - risk_card * 0.09,
                {'attack': round(intensity, 1), 'pressure': round(intensity, 1), 'xg': round(xg, 2)}
            )
            if c:
        ...
        # -------------------------
        if 70 <= minute <= 88:
            odd_late = find_live_market_odd(markets, 'goals', target='over 0.5')
            if odd_late is not None:
                late_lambda = (
                    xg * 0.55
                    + shots_on * 0.09
                    + danger * 0.006
                ...
                late_lambda *= max(0.35, (95 - minute) / 25.0)
                p_late = _v10_clamp((1 - math.exp(-max(0.05, late_lambda))) * 100, 45, 92)

                conf_late = 67 + min(12, shots_on * 2.0) + min(8, xg * 4)
                risk_late = 19
                if shots_on < 3:
                    risk_late += 7
                if xg < 0.50:
                ...
                c = _v10_live_candidate(
                    match, '⚽ LATE GOAL', odd_late, p_late,
                    _v10_clamp(conf_late, 65, 94), risk_late,
                    p_late * 0.50 + conf_late * 0.30
                    + max(0, value_edge(p_late, odd_late)) * 0.12
                    - risk_late * 0.08,
                    {'attack': round(intensity, 1), 'pressure': round(intensity, 1), 'xg': round(xg, 2)}
                )
                if c:
        ...
        logging.warning('V10 LIVE CANDIDATE ERROR: %s', repr(e))
        return []


def analyze_live_match(match):
    return _v10_live_candidates(match)


def get_best_live_signal(match):
    candidates = analyze_live_match(match)
    if not candidates:
        return None
    return max(
        candidates,
...
def live_signal_quality_filter(signal):
    if not signal:
        return False
    return (
        _v10_num(signal.get('probability'), 0) >= LIVE_MIN_PROBABILITY
        and _v10_num(signal.get('confidence'), 0) >= LIVE_MIN_CONFIDENCE
        and _v10_num(signal.get('risk'), 100) <= LIVE_MAX_RISK
        and LIVE_MIN_ODD <= _v10_num(signal.get('odd'), 0) <= LIVE_MAX_ODD
        and _v10_num(signal.get('edge'), -999) >= 0
    )
...
def format_live_signal(signal):
    return (
        ...
        f"🌍 {signal.get('country','')}\n"
        f"🏆 {signal.get('league','')}\n"
        f"⏱ {signal.get('minute',0)}' | "
        f"{signal.get('home_goals',0):.0f}-{signal.get('away_goals',0):.0f}\n\n"
        f"🎯 MARKET: {signal.get('market','UNKNOWN')}\n"
        f"📈 Probability: {signal.get('probability',0):.1f}%\n"
        f"💰 Betano Odds: {signal.get('odd',0):.2f}\n"
        f"📊 Edge: {signal.get('edge',0):+.1f}%\n"
        f"🤖 Confidence: {signal.get('confidence',0):.1f}%\n"
        f"⚡ Attack: {signal.get('attack',0):.1f}\n"
        f"🔥 Pressure: {signal.get('pressure',0):.1f}\n"
        f"📊 xG: {signal.get('xg',0):.2f}\n"
        f"🛡 Risk: {signal.get('risk',0):.0f}\n"
    )
...
def send_live_signal(signal):
    fid = signal.get('fixture_id')
    market = signal.get('market')
    minute = int(_v10_num(signal.get('minute'), 0))
    if not fid or not market:
        return False

    key = f'{fid}|{market}'
    previous = V10_LIVE_SENT.get(key)
    now = time.time()

    # Do not spam the same market every 30 seconds. Re-open it only when
    # the match has moved substantially forward.
    if previous:
        age = now - previous.get('time', 0)
        old_minute = previous.get('minute', 0)
        if age < LIVE_COOLDOWN and minute < old_minute + 5:
            return False

    if not send_telegram(format_live_signal(signal)):
        return False

    V10_LIVE_SENT[key] = {'time': now, 'minute': minute}
    save_signal(signal)
    return True
...
def scan_live():
    try:
        matches = get_live_matches()
        if not matches:
            print('LIVE V10 | matches=0')
            return 0

        all_candidates = []
        for match in matches:
            try:
                all_candidates.extend(_v10_live_candidates(match))
            except Exception as e:
                logging.warning('V10 LIVE MATCH ERROR: %s', repr(e))

        ranked = rank_live_signals(all_candidates)
        sent = 0
        ...
        print(
            f'LIVE V10 | matches={len(matches)} | '
            f'candidates={len(all_candidates)} | '
            f'qualified={len(ranked)} | sent={sent}'
        )
        return sent
    except Exception as e:
        logging.warning('LIVE V10 SCAN ERROR: %s', repr(e))
        return 0


# ============================================================
# V10 FORMATTER — PREMATCH BUILDER LEGS
# ============================================================
...
def format_prematch_signal(signal):
    ...
    if market == '🧩 BET BUILDER':
        for leg in signal.get('builder_legs', []):
            text += (
                f"  • {leg.get('market')} @ {leg.get('odd',0):.2f}"
                f" | Model: {leg.get('probability',0):.1f}%"
                f" | Conf: {leg.get('confidence',0):.1f}%\n"
            )
        text += (
            f"\n🧩 Legs: {len(signal.get('builder_legs', []))}\n"
            f"💰 Combined Odds: {signal.get('odd',0):.2f}\n"
            f"📈 Joint Probability: {signal.get('probability',0):.1f}%\n"
            f"📊 Edge: {signal.get('edge',0):+.1f}%\n"
...
# ============================================================
# V10 STATUS / MAIN LOOP
# ============================================================

def print_system_status():
    print('=' * 64)
    print('🤖 AI FOOTBALL BOT V10 — CLEAN LIVE + PREMATCH ENGINE')
    print('STATUS: ONLINE')
    print('LIVE:', LIVE_SCAN_INTERVAL, 'sec')
    print('PREMATCH:', PREMATCH_SCAN_INTERVAL, 'sec')
    print('LIVE WINDOW:', LIVE_MINUTE, '-', LIVE_MAX_MINUTE)
    print('LIVE PROBABILITY:', LIVE_MIN_PROBABILITY)
    print('LIVE CONFIDENCE:', LIVE_MIN_CONFIDENCE)
    print('LIVE MAX RISK:', LIVE_MAX_RISK)
    print('PREMATCH PROBABILITY:', PREMATCH_MIN_PROBABILITY)
    print('PREMATCH CONFIDENCE:', PREMATCH_MIN_CONFIDENCE)
    print('PREMATCH MAX RISK:', PREMATCH_MAX_RISK)
    print('BUILDER LEGS:', BET_BUILDER_MIN_LEGS, '-', BET_BUILDER_MAX_LEGS)
    print('BUILDER TOP:', BET_BUILDER_DAILY_TOP3)
    print('=' * 64)
...
def main_loop():
    ...
    # Force an immediate first scan instead of waiting for the interval.
    LAST_LIVE_SCAN = 0
    LAST_PREMATCH_SCAN = 0
...
# ============================================================
# V11 HOTFIX — BULK LIVE ODDS + REAL DIAGNOSTICS
# ============================================================
# The V10 log "matches=120 | candidates=0" shows the scanner is
# reaching the live fixtures but eliminating every fixture before
# ranking. The main weakness is the per-fixture odds/statistics
# dependency and the absence of a rejection breakdown.
#
# V11:
#   - fetches /odds/live once for all live fixtures and indexes it;
#   - keeps a per-fixture fallback only when bulk data has no match;
#   - accepts Betano whether it appears as a bookmaker item or nested;
#   - parses team-name Next Goal values, not only "home"/"away";
#   - records exact rejection reasons;
#   - uses the existing live statistics when available;
#   - does NOT invent statistics or odds.
# ============================================================

V11_LIVE_ODDS_ALL_CACHE = {'time': 0.0, 'by_fixture': {}}
V11_LIVE_DIAG = {
    'fixtures': 0,
    'stats_ok': 0,
    'stats_empty': 0,
...
def _v11_reset_diag():
    for k in V11_LIVE_DIAG:
        V11_LIVE_DIAG[k] = 0

def _v11_extract_betano(item):
    if not isinstance(item, dict):
        return None

    # Direct bookmaker object.
    if item.get('id') == 32 or clean_text(item.get('name')) == 'betano':
        return item

    # Fixture object containing bookmakers.
    for b in item.get('bookmakers', []) or []:
        if isinstance(b, dict) and (b.get('id') == 32 or clean_text(b.get('name')) == 'betano'):
            return b

    return None

def _v11_normalize_bookmaker(bookmaker):
    markets = []
    for bet in bookmaker.get('bets', []) or []:
        if not isinstance(bet, dict):
            continue
        ...
        if values:
            markets.append({'name': name, 'values': values})
    return markets

def _v11_bulk_live_odds():
    now = time.time()
    if now - V11_LIVE_ODDS_ALL_CACHE['time'] < 20:
        return V11_LIVE_ODDS_ALL_CACHE['by_fixture']

    by_fixture = {}
    try:
        data = api_get('odds/live', {}) or {}
        ...
        for item in response:
            ...
            fid = (
                ...
                else item.get('fixture_id')
            )

            bookmaker = _v11_extract_betano(item)
            if bookmaker is None:
                continue

            markets = _v11_normalize_bookmaker(bookmaker)
            if markets and fid:
                by_fixture[int(fid)] = markets

        V11_LIVE_ODDS_ALL_CACHE['time'] = now
        V11_LIVE_ODDS_ALL_CACHE['by_fixture'] = by_fixture
        return by_fixture

    except Exception as e:
        logging.warning('V11 BULK LIVE ODDS ERROR: %s', repr(e))
        return by_fixture
...
def get_live_betano_markets(fixture_id):
    ...
    all_markets = _v11_bulk_live_odds()
    try:
        fid = int(fixture_id)
    except Exception:
        fid = fixture_id

    if fid in all_markets:
        return all_markets[fid]

    # Fallback for APIs/accounts where /odds/live?fixture=ID returns
    # something that is not present in the bulk response.
    try:
        data = api_get('odds/live', {'fixture': fixture_id}) or {}
        response = data.get('response', []) if isinstance(data, dict) else []
        for item in response:
        ...
        logging.warning('V11 FIXTURE LIVE ODDS ERROR %s: %s', fixture_id, repr(e))

    return []

def _v11_find_team_next_goal(markets, home_name, away_name):
    h = clean_text(home_name)
    a = clean_text(away_name)
    ...
    for market in markets or []:
        ...
        for value in market.get('values', []) or []:
            vn = clean_text(value.get('value'))
            odd = safe_float(value.get('odd'))
            if not odd or value.get('stopped'):
                continue

            if vn in ('home', '1') or vn == h or h in vn:
                return 'home', odd
            if vn in ('away', '2') or vn == a or a in vn:
                return 'away', odd

    return None, None

def _v11_stats(match):
    stats = get_live_market_stats(match) or {}
    hs = stats.get('home', {}) or {}
    aws = stats.get('away', {}) or {}

    total_keys = [
        'total shots', 'shots on goal', 'dangerous attacks',
        'corner kicks', 'expected goals'
    ]
    has_any = any(
        _v10_num(hs.get(k), 0) + _v10_num(aws.get(k), 0) > 0
        for k in total_keys
    )

    return stats, hs, aws, has_any

def _v11_candidate(match):
    fixture = match.get('fixture', {}) or {}
    fid = fixture.get('id')
    minute = int(_v10_num(fixture.get('status', {}).get('elapsed'), 0))
    ...
    markets = get_live_betano_markets(fid)
    if markets:
        V11_LIVE_DIAG['odds_ok'] += 1
    else:
        V11_LIVE_DIAG['odds_empty'] += 1
        V11_LIVE_DIAG['market_fail'] += 1
        return None

    stats, hs, aws, stats_ok = _v11_stats(match)
    if stats_ok:
        V11_LIVE_DIAG['stats_ok'] += 1
    else:
        V11_LIVE_DIAG['stats_empty'] += 1
        # No statistics = no statistical live signal. Do not fabricate it.
        return None
    ...
    V11_LIVE_DIAG['activity_ok'] += 1

    side, odd = _v11_find_team_next_goal(markets, home_name, away_name)

    # If Next Goal is not offered, let other live markets be handled below.
    if side and odd:
        hg = _v10_num((match.get('goals', {}) or {}).get('home'), 0)
        ag = _v10_num((match.get('goals', {}) or {}).get('away'), 0)

        selected = max(h_activity, a_activity)
        other = min(h_activity, a_activity)
        dominance = (selected - other) / max(1.0, selected + other)

        p = 52 + dominance * 42 + max(0, selected - 25) * 0.10
        if abs(hg - ag) <= 1:
            p += 2
        if minute >= 70:
            p += 1
        p = _v10_clamp(p, 58, 88)
        ...
        confidence = (
            ...
            + min(12, shots_on * 2.2)
            + min(8, xg * 4)
            + min(8, dangerous * 0.12)
            + min(8, dominance * 20)
        )
        if minute < 35:
            confidence -= 3
        confidence = _v10_clamp(confidence, 62, 91)
        ...
        if minute < 25:
            risk += 5
        risk = int(_v10_clamp(risk, 10, 38))

        edge = value_edge(p, odd)

        if p < LIVE_MIN_PROBABILITY:
            V11_LIVE_DIAG['prob_fail'] += 1
        elif confidence < LIVE_MIN_CONFIDENCE:
            V11_LIVE_DIAG['confidence_fail'] += 1
        elif risk > LIVE_MAX_RISK:
            V11_LIVE_DIAG['risk_fail'] += 1
        elif edge < 0:
            V11_LIVE_DIAG['edge_fail'] += 1
        else:
            market = '🎯 NEXT GOAL HOME' if side == 'home' else '🎯 NEXT GOAL AWAY'
            return {
                'fixture_id': fid,
                ...
                'country': (match.get('league', {}) or {}).get('country', ''),
                'market': market,
                'probability': round(p, 1),
                'confidence': round(confidence, 1),
                'risk': risk,
    ...
    # Next Goal is not mandatory: evaluate live total-goal market.
    hg = _v10_num((match.get('goals', {}) or {}).get('home'), 0)
    ag = _v10_num((match.get('goals', {}) or {}).get('away'), 0)
    total_goals = hg + ag
    shots_on = h_on + a_on
    xg = h_xg + a_xg
    danger = h_danger + a_danger

    if total_goals < 2:
        odd15 = find_live_market_odd(markets, 'goals', target='over 1.5')
        if odd15:
            remaining_lambda = max(
                0.08,
                xg * 0.70 + (h_shots + a_shots) * 0.018
                + shots_on * 0.075 + danger * 0.004
            )
            remaining_lambda *= max(0.20, (95 - minute) / 70)
            p15 = _v10_clamp((1 - math.exp(-remaining_lambda)) * 100, 50, 91)
            if total_goals == 1:
                p15 = max(p15, 58 + min(28, total_activity * 0.16))

            conf15 = _v10_clamp(
                65 + min(12, shots_on * 1.8)
                + min(8, xg * 3.5) + min(7, total_activity * 0.05),
                65, 94
            )
            risk15 = 17
            if shots_on < 2:
            ...
            edge15 = value_edge(p15, odd15)
            if p15 >= LIVE_MIN_PROBABILITY and conf15 >= LIVE_MIN_CONFIDENCE and risk15 <= LIVE_MAX_RISK and edge15 >= 0:
                return {
                    'fixture_id': fid,
                    'home_team': home_name,
                    'away_team': away_name,
                    ...
                    'probability': round(p15, 1),
                    'confidence': round(conf15, 1),
                    'risk': risk15,
                    'odd': round(odd15, 2),
                    'edge': round(edge15, 1),
                    'score': round(p15 * .50 + conf15 * .31 + max(0, edge15) * .11 - risk15 * .08, 2),
                    'minute': minute,
                    'attack': round(total_activity, 1),
                    'pressure': round(total_activity, 1),
                    'xg': round(xg, 2),
                    'home_goals': hg,
                    'away_goals': ag,
...
def analyze_live_match(match):
    try:
        c = _v11_candidate(match)
        return [c] if c else []
    except Exception as e:
        V11_LIVE_DIAG['errors'] += 1
        logging.warning('V11 LIVE MATCH ERROR: %s', repr(e))
        return []
...
def scan_live():
    try:
        ...
        print(
            ...
            f'eligible={d["fixtures"]} | '
            f'odds={d["odds_ok"]} | '
            f'no_odds={d["odds_empty"]} | '
            f'stats={d["stats_ok"]} | '
            f'no_stats={d["stats_empty"]} | '
            f'activity={d["activity_ok"]} | '
        ...
        print(
            'LIVE V11 REJECT | '
            f'prob={d["prob_fail"]} | '
            f'conf={d["confidence_fail"]} | '
            f'risk={d["risk_fail"]} | '
            f'edge={d["edge_fail"]} | '
            f'market={d["market_fail"]} | '
            f'errors={d["errors"]}'
        )
        return sent

    except Exception as e:
        logging.warning('LIVE V11 SCAN ERROR: %s', repr(e))
        return 0

def print_system_status():
    print('=' * 64)
    print('🤖 AI FOOTBALL BOT V11 — BULK LIVE + PREMATCH')
    print('STATUS: ONLINE')
    print('LIVE:', LIVE_SCAN_INTERVAL, 'sec')
    print('PREMATCH:', PREMATCH_SCAN_INTERVAL, 'sec')
    print('LIVE WINDOW:', LIVE_MINUTE, '-', LIVE_MAX_MINUTE)
...
# V12 PREMATCH HOTFIX
# ============================================================
# Prematch was still using the old V7 daily lock:
# DAILY_NORMAL_SENT / DAILY_BUILDER_SENT were added even when
# zero signals were actually sent.  A single empty scan could
# therefore suppress all later prematch attempts for the day.
#
# V12 removes that lock entirely.  Every prematch cycle rescans
# the available fixtures and only signal_already_sent() prevents
# duplicates.  It also adds diagnostics so a bad market/form/API
# response cannot silently look like "no signals".
# ============================================================
...
def _v12_reset_prematch_diag():
    for k in V12_PREMATCH_DIAG:
        V12_PREMATCH_DIAG[k] = 0

def _v12_candidate_wrapper(match, detailed=False):
    try:
        fixture = match.get('fixture', {}) or {}
        teams = match.get('teams', {}) or {}
        home = teams.get('home', {}) or {}
        ...
        if blocked_league(league.get('name', '')) or bad_country(league.get('country', '')):
            V12_PREMATCH_DIAG['blocked'] += 1
            return []

        result = _v7_prematch_candidates(match, detailed)
        if result:
            V12_PREMATCH_DIAG['model_candidates'] += len(result)
        return result or []

    except Exception as e:
        V12_PREMATCH_DIAG['errors'] += 1
        logging.warning('V12 PREMATCH MODEL ERROR: %s', repr(e))
        return []

def _v12_select_normal(matches):
    allc = []

    # Evaluate every available fixture every scan.
    for m in matches or []:
        allc.extend(_v12_candidate_wrapper(m, False))

    # Enrich the strongest fixtures with detailed markets/history.
    if allc:
        strongest = sorted(
            allc,
            key=lambda x: (
                float(x.get('score', 0) or 0),
                float(x.get('probability', 0) or 0),
        ...
        used = set()
        for c in strongest[:max(DETAIL_HISTORY_TOP_N, 10)]:
            ...
            m = next(
                (x for x in matches
                 if (x.get('fixture', {}) or {}).get('id') == fid),
                None
            )
            if m:
                allc.extend(_v12_candidate_wrapper(m, True))

    # Deduplicate by fixture + family.
    best = {}
    for c in allc:
        key = (c.get('fixture_id'), c.get('family', c.get('market', '')))
        if key[0] is None:
            continue
        if key not in best or float(c.get('score', 0) or 0) > float(best[key].get('score', 0) or 0):
            best[key] = c
...
def _v12_select_builders(matches):
    # Build from a broader candidate pool, not only the already-selected
    # normal signals. This prevents a normal-signal miss from killing
    # builder generation.
    allc = []
    for m in matches or []:
        cs = _v12_candidate_wrapper(m, False)
        if cs:
            allc.extend(cs)
    ...
    for c in allc:
        ...
        if fid is None:
            continue
        if fid not in best_by_fixture or float(c.get('score', 0) or 0) > float(best_by_fixture[fid].get('score', 0) or 0):
    ...
    for c in sorted(
        ...
        m = next(
            (x for x in matches
             if (x.get('fixture', {}) or {}).get('id') == c.get('fixture_id')),
            None
        )
        if m:
            ordered_matches.append(m)

    # Reuse the original builder engine, but feed it the complete
    # quality-ranked universe instead of a daily-locked normal list.
    try:
        builders = _v7_select_builders(ordered_matches)
        return builders or []
    except Exception as e:
        V12_PREMATCH_DIAG['errors'] += 1
        logging.warning('V12 BUILDER ERROR: %s', repr(e))
        return []

def process_prematch_matches(matches):
    """
    IMPORTANT: no DAILY_NORMAL_SENT / DAILY_BUILDER_SENT lock.
    A scan that sends zero signals must never disable the rest of
    today's prematch scanning.
    """
    normal_sent = 0
    builder_sent = 0

    normals = _v12_select_normal(matches)
    V12_PREMATCH_DIAG['qualified'] += len(normals)
    ...
    for s in normals:
        try:
            ...
            V12_PREMATCH_DIAG['errors'] += 1
            logging.warning('V12 NORMAL SEND ERROR: %s', repr(e))

    # Builders are independent of the normal daily lock.
    builders = _v12_select_builders(matches)
    for b in builders:
        try:
            ...
            V12_PREMATCH_DIAG['errors'] += 1
            logging.warning('V12 BUILDER SEND ERROR: %s', repr(e))

    V12_PREMATCH_DIAG['sent'] += normal_sent + builder_sent

    print(
        'PREMATCH V12 | '
        f'whole_day={len(matches or [])} | '
        f'candidates={V12_PREMATCH_DIAG["model_candidates"]} | '
        f'qualified={len(normals)} | '
        f'builders={len(builders)} | '
        f'sent={normal_sent + builder_sent}'
    )
    print(
        'PREMATCH V12 DIAG | '
        f'blocked={V12_PREMATCH_DIAG["blocked"]} | '
        f'errors={V12_PREMATCH_DIAG["errors"]}'
    )
...
def scan_prematch():
    try:
        _v12_reset_prematch_diag()
        matches = remove_started_matches(get_prematch_matches()) or []

        if not matches:
            print('PREMATCH V12 | whole_day=0 | no upcoming fixtures returned by API')
            return 0

        print(f'PREMATCH WHOLE DAY V12: {len(matches)}')
        return process_prematch_matches(matches)

    except Exception as e:
        V12_PREMATCH_DIAG['errors'] += 1
        logging.warning('PREMATCH SCAN V12 ERROR: %s', repr(e))
        return 0

def print_system_status():
    print('=' * 64)
    print('🤖 AI FOOTBALL BOT V12 — LIVE + PREMATCH HOTFIX')
    print('STATUS: ONLINE')
    print('LIVE:', LIVE_SCAN_INTERVAL, 'sec')
    print('PREMATCH:', PREMATCH_SCAN_INTERVAL, 'sec')
    print('LIVE WINDOW:', LIVE_MINUTE, '-', LIVE_MAX_MINUTE)
