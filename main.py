...
def api_get(endpoint, params=None):
    """
    Single compatibility layer for the Highlightly football API.

    Important fixes in MAIN V4.1:
      - never send an artificial `limit` to football/matches;
      - normalize Highlightly last-five-games into the legacy fixture shape;
      - normalize Highlightly standings into the legacy standings shape;
      - keep live matching local after requesting today's matches;
      - preserve the existing odds normalization used by the signal engine.
    """
    params = dict(params or {})
    original_endpoint = endpoint
    live_requested = False
    team_form_requested = False

    # -----------------------------------------------------
    # API-FOOTBALL COMPATIBILITY -> HIGHLIGHTLY
    # -----------------------------------------------------
    if endpoint == "fixtures":
        ...
        if "team" in params:
            ...
            team_form_requested = True

        if "id" in params:
            fixture_id = params.pop("id")
            endpoint = f"football/matches/{fixture_id}"

        # Do NOT inject limit here. The deployed Highlightly route used by
        # the bot has rejected this legacy parameter with HTTP 400.
        # Date/timezone or another primary query parameter is sufficient.
    ...
    elif endpoint == "odds" or endpoint == "odds/live":
        fixture_id = params.pop("fixture", None)
        endpoint = "football/odds"
        if fixture_id is not None:
            params["matchId"] = fixture_id
        if original_endpoint == "odds/live":
            params["oddsType"] = "live"
        else:
            params.setdefault("oddsType", "prematch")
        ...
        # Highlightly requires leagueId + season and returns a different
        # structure from API-Football.
        if "league" in params:
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    logging.warning("API INVALID JSON | %s", original_endpoint)
                    return {}

                # -------------------------------------------------
                # LAST FIVE GAMES: Highlightly returns a bare list.
                # Convert it to the legacy fixture structure expected
                # by the form engine.
                # -------------------------------------------------
                if original_endpoint == "fixtures" and team_form_requested:
                    ...
                    for m in rows:
                        ...
                        current_score = score.get("current", "0 - 0")
                        try:
                            hs, aw = [int(x.strip()) for x in str(current_score).split("-", 1)]
                        except Exception:
                            hs, aw = 0, 0
                        description = str(state.get("description", "Finished"))
                        converted.append({
                            "fixture": {
                                "id": m.get("id"),
                                "date": m.get("date"),
                ...
                # -------------------------------------------------
                # STANDINGS: Highlightly returns {groups, league}.
                # Normalize to API-Football-like response so the rest
                # of V4 can stay unchanged.
                # -------------------------------------------------
                    ...
                    for group in groups:
                        ...
                        for row in group.get("standings", []) or []:
                            ...
                            rows.append({
                                ...
                                "goalsDiff": (
                                    safe_float(total.get("scoredGoals"), 0)
                                    - safe_float(total.get("receivedGoals"), 0)
                                ),
                                "team": {
                                    "id": team.get("id"),
                                    "name": team.get("name"),
                                ...
                                "all": {
                                    ...
                                    "win": total.get("wins", 0),
                                    "draw": total.get("draws", 0),
                                    "lose": total.get("loses", 0),
                                    "goals": {
                                        "for": total.get("scoredGoals", 0),
                                        "against": total.get("receivedGoals", 0)
                                    }
                                }
                            })
                    return {
                ...
                # -------------------------------------------------
                # MATCHES: Highlightly returns {data:[...]}. Normalize
                # into the legacy fixture shape used everywhere else.
                # -------------------------------------------------
                if original_endpoint == "fixtures":
                    ...
                    for m in data:
                        ...
                        if "NOT STARTED" in desc_upper or "SCHEDULED" in desc_upper:
                            status_short = "NS"
                        elif "FINISHED" in desc_upper or "AWARDED" in desc_upper:
                            status_short = "FT"
                        elif any(x in desc_upper for x in live_states):
                            status_short = "LIVE"
                        ...
                        converted.append({
                            "fixture": {
                                "id": m.get("id"),
                                "date": m.get("date"),
                                "timezone": params.get("timezone", str(TIMEZONE)),
                                "status": {
                                    "short": status_short,
                                    "long": description,
                                    "elapsed": state.get("clock")
                    ...
                    if live_requested:
                        converted = [
                            ...
                            if m.get("fixture", {}).get("status", {}).get("short") == "LIVE"
                        ]
                    return {"response": converted}

                # Statistics/events already use the legacy response list.
                if original_endpoint in ("fixtures/statistics", "fixtures/events"):
                    data = payload.get("data", []) if isinstance(payload, dict) else payload
                    return {"response": data if isinstance(data, list) else []}

                # Leagues: preserve data list as legacy response.
                if original_endpoint == "leagues":
                    data = payload.get("data", []) if isinstance(payload, dict) else payload
                    return {"response": data if isinstance(data, list) else []}

                # Odds: normalize Highlightly {matchId, odds:[...]} into
                # the legacy {bookmakers:[{bets:[...]}]} structure.
            ...
            if response.status_code >= 500:
                ...
                logging.warning(
                    ...
                    response.status_code, original_endpoint,
                    attempt, API_RETRIES
                )
                time.sleep(wait_time)
                continue
            ...
            logging.warning(
                ...
                original_endpoint, attempt, API_RETRIES, repr(e)
            )
            time.sleep(attempt)
        except Exception as e:
            logging.warning(
                "API ERROR | %s | %s",
                original_endpoint, repr(e)
            )
            time.sleep(1)
