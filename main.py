...
BUILD_TAG = "HIGHLIGHTLY-NATIVE-LIVE-FIX-1"
...
print("HIGHLIGHTLY BASE URL:", BASE_URL)
print("HIGHLIGHTLY FOOTBALL API: /football/matches")
...
# =========================================================
# API ENGINE — HIGHLIGHTLY NATIVE
# =========================================================
# IMPORTANT:
# This bot talks directly to Highlightly.  There is no Highlightly
# request or Highlightly parameter sent over the network.
#
# The functions below keep the bot's existing internal "response" shape
# where convenient, but every external endpoint and parameter is native
# Highlightly.

def _highlightly_unwrap(payload):
...
def _parse_highlightly_score(state):
    score = (state or {}).get("score") or {}
    current = score.get("current")

    # Current score may be "1 - 0".
    if isinstance(current, str):
        m = re.match(r"\s*(\d+)\s*-\s*(\d+)", current)
        if m:
            return int(m.group(1)), int(m.group(2))

    # Or Highlightly may expose home/away score arrays/objects.
    def one(v):
    ...
    return (
        one(score.get("homeTeam") or score.get("home")),
        one(score.get("awayTeam") or score.get("away")),
    )


def _normalize_highlightly_match(m, timezone_name=None):
    ...
    if "NOT STARTED" in upper or "SCHEDULED" in upper:
        status_short = "NS"
    elif "FINISHED" in upper or "AFTER PENALTIES" in upper:
        status_short = "FT"
    elif any(x in upper for x in (
        "HALF", "FIRST", "SECOND", "EXTRA",
        "PENALT", "IN PROGRESS", "BREAK"
    )):
        status_short = "LIVE"
    elif "POSTPONED" in upper:
        status_short = "PST"
    elif "CANCEL" in upper:
        status_short = "CANC"
    else:
        # Highlightly also uses a state object.  A running clock is a
        # useful fallback for live-state detection.
        status_short = "LIVE" if state.get("clock") is not None and description else "NS"

    hs, aw = _parse_highlightly_score(state)

    fid = m.get("id") or m.get("matchId")
    season = league.get("season")
    if isinstance(season, dict):
        season = season.get("year") or season.get("id")
    ...
    return {
        "fixture": {
            "id": fid,
            "date": m.get("date") or m.get("startDate"),
            "timezone": timezone_name or str(TIMEZONE),
            "status": {
                "short": status_short,
                "long": description,
                "elapsed": state.get("clock"),
            },
        },
        "league": {
            "id": league.get("id"),
        ...
        "teams": {
            "home": {
                "id": home.get("id"),
                "name": home.get("name") or home.get("displayName"),
                "logo": home.get("logo"),
            },
            "away": {
                "id": away.get("id"),
                "name": away.get("name") or away.get("displayName"),
                "logo": away.get("logo"),
            },
        },
        "goals": {
...
def _normalize_odds(rows):
    ...
    for row in rows or []:
        ...
        # Highlightly returns one or more odds rows containing bookmaker
        # information inside the odds array.
        for market in row.get("odds", []) or []:
            ...
            book = bookmakers.setdefault(key, {
                "id": bid,
                "name": bname,
                "bets": [],
            })
        ...
        normalized.append({
            "fixture": {"id": row.get("matchId") or row.get("id")},
...
def api_get(endpoint, params=None):
    """
    Native Highlightly request wrapper.

    Internal callers may still use the old function names
    (fixtures / fixtures/statistics / fixtures/events / odds), but
    those names are translated here only to Highlightly endpoints.
    No Highlightly request is sent.
    """
    ...
    # ---------------------------------------------------------
    # Native Highlightly endpoint + parameter mapping
    # ---------------------------------------------------------
    if endpoint == "fixtures":
        if "live" in params:
            ...
            fixture_id = params.pop("id")
            endpoint = f"football/matches/{fixture_id}"
            live_requested = False
        else:
            endpoint = "football/matches"
            live_requested = False
            # limit is valid only together with a primary filter.
            if not any(
                params.get(k) is not None
                for k in (
                    "leagueName", "leagueId", "date", "season",
                    "homeTeamId", "awayTeamId", "homeTeamName",
                    "awayTeamName", "countryCode", "countryName"
                )
            ):
                params.setdefault(
                    "date",
        ...
        endpoint = f"football/statistics/{fixture_id}"
        live_requested = False

    elif endpoint == "fixtures/events":
        fixture_id = params.pop("fixture", None)
        endpoint = f"football/events/{fixture_id}"
        live_requested = False

    elif endpoint in ("odds", "odds/live"):
        fixture_id = params.pop("fixture", None)
        bookmaker = params.pop("bookmaker", None)
        endpoint = "football/odds"

        if fixture_id is not None:
            params["matchId"] = fixture_id
        if bookmaker is not None:
            params["bookmakerId"] = bookmaker

        params["oddsType"] = (
            "live" if original_endpoint == "odds/live" else "prematch"
        )
        params.setdefault("limit", 5)
        live_requested = original_endpoint == "odds/live"

    elif endpoint == "standings":
        league = params.pop("league", None)
    ...
    elif endpoint == "football/matches":
        # Health check must include a primary parameter.
        if not any(
            params.get(k) is not None
            for k in (
                "leagueName", "leagueId", "date", "season",
    ...
    # Never send obsolete Highlightly parameter names to Highlightly.
    params.pop("team", None)
    params.pop("last", None)
    params.pop("live", None)
    params.pop("fixture", None)
    params.pop("bookmaker", None)
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    logging.warning("API INVALID JSON | %s", endpoint)
                    return {}

                data = _highlightly_unwrap(payload)

                # Last five games
                if original_endpoint == "fixtures" and endpoint == "football/last-five-games":
                    rows = data if isinstance(data, list) else []
                    return {
                        "response": [
                            x for x in (
                                _normalize_highlightly_match(m, timezone_name)
                                for m in rows
                            ) if x
                        ]
                    }
                ...
                # Matches
                if original_endpoint in ("fixtures", "football/matches"):
                    rows = data if isinstance(data, list) else []
                    converted = []

                    for m in rows:
                        normalized = _normalize_highlightly_match(
                            m, params.get("timezone")
                        )
                        if normalized:
                ...
                if original_endpoint in ("odds", "odds/live"):
                    return {"response": _normalize_odds(data)}

                if original_endpoint == "standings":
            ...
            if response.status_code >= 500:
                ...
                logging.warning(
                    ...
                    response.status_code, endpoint, attempt, API_RETRIES
                )
                time.sleep(wait_time)
                continue

            logging.warning(
                "API HTTP ERROR %s | endpoint=%s | caller=%s | %s",
                response.status_code,
                endpoint,
                original_endpoint,
                response.text[:300],
            ...
            logging.warning(
                ...
                endpoint, attempt, API_RETRIES, repr(e)
            )
            time.sleep(attempt)

        except Exception as e:
            logging.warning("API ERROR | %s | %s", endpoint, repr(e))
            time.sleep(1)
...
def get_live_matches():
    """Fetch today's Highlightly football matches directly and keep only in-play states.

    Highlightly has no API-Football-style `live=all` filter. The documented
    matches endpoint requires a primary parameter such as `date`, so we fetch
    today's matches and filter the returned state locally.
    """
    ...
    try:
        logging.info("LIVE DIRECT HIGHLIGHTLY | endpoint=football/matches | date=%s", today)
        data = api_get("football/matches", params)
        ...
        for match in matches:
            ...
            long_status = str(status.get("long") or "").upper().strip()
            if short == "LIVE" or long_status in live_states:
                result.append(match)

        return result

    except Exception as e:
        logging.warning("LIVE HIGHLIGHTLY ERROR: %s", repr(e))
        return []
...
def get_live_betano_markets(fixture_id):
    ...
    try:
        ...
        bookmaker=None
        # Prefer Betano, but do NOT kill the live engine when Highlightly
        # does not publish Betano for a particular live fixture.  The odds
        # must still be real real Highlightly bookmaker odds; never synthesize a
...
def get_prematch_betano_builder_markets(fixture_id):
    ...
    for item in odds:
        ...
        if bookmaker:
            break
    # Highlightly does not expose Betano for every fixture/league.
    ...
    for bet in bookmaker.get("bets", []) or []:
        ...
        for v in bet.get("values", []) or []:
            ...
            if not value or odd is None or odd <= 1.01:
                continue
            # Preserve exactly what Betano/Highlightly supplied.
...
def _builder_recent_market_stats(team_id, venue=None, limit=5):
    ...
    data = api_get("fixtures", {"team": team_id})
...
def _single_market_consistency(signal, match):
    ...
    if "btts yes" in market or market.endswith("btts"):
        # BTTS YES needs evidence from both sides, not one attacking team.
        if hb < 40.0 or ab < 40.0 or (hb + ab) / 2.0 < 52.0:
            return False
...
def get_best_prematch_signals(matches):
    ...
    for match in matches:
        try:
            ...
            for raw in signals:
                ...
                if odd < PREMATCH_MIN_SINGLE_ODD or odd > PREMATCH_MAX_ODD:
                    continue
                if p < PREMATCH_MIN_SINGLE_PROB or c < PREMATCH_MIN_SINGLE_CONFIDENCE:
                    continue
                if e < PREMATCH_MIN_SINGLE_EDGE or r > PREMATCH_MAX_SINGLE_RISK:
                    continue
    ...
    return sorted(best_by_fixture.values(), key=lambda x: (_safe_num(x.get("edge"), -999), _safe_num(x.get("probability"), 0), _safe_num(x.get("confidence"), 0), -_safe_num(x.get("risk"), 100)), reverse=True)[:PREMATCH_DAILY_TOP5]
...
def _v4_stat(stats, *names):
    """Read a statistic using the normalized/cleaned Highlightly name."""
    for name in names:
