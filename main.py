# DEPLOY BUILD: MAIN V4 + scanner.py FIX 2026-09-17
# MAIN V4
# AI BETTING SYSTEM
# VERSION 4.0
# =========================================================
...
# DAILY STATISTICAL SCANNER — independent worker.
# LIVE/PREMATCH code below remains unchanged.
try:
    from scanner import run_due_scans
except Exception as _scanner_import_error:
    run_due_scans = None
    logging.warning("DAILY SCANNER IMPORT ERROR: %r", _scanner_import_error)
...
def api_health_check():
    # TEMPORARY SPORT-ONLY MODE: do not call Football API at startup.
    # Football LIVE/PREMATCH are disabled while Sport Statistics is being fixed.
    return True
...
def main_loop():
    ...
    # Football LIVE and PREMATCH scanners are intentionally NOT called.
    # The only active scanner is run_due_scans() from scanner.py.

    print_system_status()
    print("FOOTBALL LIVE: DISABLED")
    print("FOOTBALL PREMATCH: DISABLED")
    print("SPORT STATISTICS: ENABLED")
    print("\nAPI CONNECTION CHECK: SKIPPED (FOOTBALL TEMPORARILY OFF)\n")

    if run_due_scans is None:
        logging.error("SPORT SCANNER IMPORT FAILED — run_due_scans is None")
        return
    ...
    while True:
        try:
            ...
            logging.exception("SPORT SCANNER LOOP ERROR: %r", exc)
        time.sleep(60)
...
if __name__ == "__main__":
    ...
    startup()
    print("BOOT: entering main_loop", flush=True)
