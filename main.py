...
def get_prematch_betano_builder_markets(fixture_id):
    ...
    for bet in bookmaker.get("bets", []) or []:
        ...
        main_values = [v for v in raw_values if v.get("main") is True]
        if not main_values:
            continue

        result.append({"name": bname, "values": main_values})
...
def build_best_bet_builder(match):
    ...
    joint_pct = round(max(0.0, min(90.0, joint_pct)), 1)
