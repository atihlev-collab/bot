...
def get_prematch_betano_builder_markets(fixture_id):
    ...
    for bet in bookmaker.get("bets", []) or []:
        ...
        # If Betano/API-Football marks a primary value, use only that exact
        # value. This prevents the Builder from selecting an alternate line
        # that is not the main market shown by the bookmaker.
        main_values = [v for v in raw_values if v.get("main")]
        values = main_values if main_values else raw_values

        result.append({"name": bname, "values": values})
...
def _builder_goal_lambdas(match):
    ...
    # Builder must not disappear only because one venue split has fewer
    # than five completed matches. Fall back to conservative league-neutral
    # attacking rates instead of returning None.
    if not hf:
        hf = {"avg_scored": 1.20, "avg_conceded": 1.20}
    if not af:
        af = {"avg_scored": 1.10, "avg_conceded": 1.20}
...
def _builder_statistical_candidates(match):
    ...
    # Neutral fallbacks prevent the whole Builder from dying when historical
    # corner/card statistics are unavailable for a league/team.
    corner_lambda = 9.5
    card_lambda = 4.0
