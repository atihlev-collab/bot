...
# ============================================================
# FINAL PREMATCH BETANO GATE — MUST BE BEFORE main_loop()
# ============================================================

def _final_betano_bookmaker(fixture_id):
    try:
        data = get_odds(fixture_id) or []
        for item in data:
            for bookmaker in item.get("bookmakers", []) or []:
                if clean_text(bookmaker.get("name")) == "betano":
                    return bookmaker
    except Exception as e:
        logging.warning("FINAL BETANO FETCH ERROR: %s", repr(e))
    return None


def _final_betano_exact_line(fixture_id, family, target):
    bookmaker = _final_betano_bookmaker(fixture_id)
    if not bookmaker:
        return None
    ...
    for bet in bookmaker.get("bets", []) or []:
        ...
        if family == "cards" and not any(x in name for x in ("card", "booking")):
            continue
        if family == "corners" and "corner" not in name:
            continue
        if family == "goals" and "goal" not in name:
            continue
        if family == "btts" and not ("both teams to score" in name or name == "btts"):
            continue

        # Do not accept first-half markets for full-match signals.
        if any(x in name for x in ("1st half", "first half", "1h")):
            continue
...
def _final_signal_line(signal):
    market = clean_text(signal.get("market", ""))

    # Extract the exact "over/under X" line from the displayed market.
    parts = market.split()
    direction = None
    number = None
...
def _final_signal_family(signal):
...
def send_prematch_signal(signal):
    """
    FINAL and ONLY PREMATCH send gate.

    A normal PREMATCH signal is sent only if its exact line is present
    in a fresh Betano odds response at the moment of sending.

    A Builder is sent only if EVERY leg is present exactly.
    """
    if not isinstance(signal, dict):
        return False
    ...
    # ------------------------------------------------------------
    # BUILDER: every leg must exist exactly in fresh Betano data.
    # ------------------------------------------------------------
    if market == "🧩 BET BUILDER":
        legs = signal.get("builder_legs") or signal.get("legs") or signal.get("selections") or []

        if len(legs) < 2:
            logging.info(
                "PREMATCH BLOCKED | builder has <2 legs | fixture=%s",
                fixture_id
            )
            return False
        ...
        for leg in legs:
            ...
            family = _final_signal_family(leg)
            target = _final_signal_line(leg)

            if family not in ("cards", "corners", "goals", "btts") or not target:
                logging.info(
                    "PREMATCH BLOCKED | builder invalid leg | fixture=%s | leg=%s",
                    fixture_id, leg
                )
                return False

            odd = _final_betano_exact_line(
                fixture_id,
                family,
                target
            )
            ...
            if odd is None:
                logging.info(
                    "PREMATCH BLOCKED | BUILDER LEG ABSENT IN BETANO | "
                    "fixture=%s | family=%s | target=%s",
                    fixture_id, family, target
                )
                return False

            leg["odd"] = odd

        # Recalculate combined odd from the actual Betano legs.
        try:
            combined = 1.0
            for leg in legs:
    ...
    # ------------------------------------------------------------
    # NORMAL SIGNAL: exact line must exist in Betano.
    # ------------------------------------------------------------
    else:
        family = _final_signal_family(signal)
        target = _final_signal_line(signal)

        if family not in ("cards", "corners", "goals", "btts") or not target:
            logging.info(
                "PREMATCH BLOCKED | unsupported/non-bookmaker market | "
                "fixture=%s | market=%s",
                fixture_id, market
            )
            return False
        ...
        if odd is None:
            logging.info(
                "PREMATCH BLOCKED | EXACT MARKET ABSENT IN BETANO | "
                "fixture=%s | market=%s | family=%s | target=%s",
                fixture_id, market, family, target
            )
            return False

        # Always send the current Betano odd, never the stale model odd.
        signal["odd"] = odd

    # ------------------------------------------------------------
    # DUPLICATE PROTECTION — only after Betano validation.
    # ------------------------------------------------------------
    if signal_already_sent(fixture_id, market):
        return False

    try:
        message = format_prematch_signal(signal)
    except Exception as e:
        logging.warning("PREMATCH FORMAT ERROR: %s", repr(e))
        return False
