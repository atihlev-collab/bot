...
# RAPIDAPI COMPATIBLE STANDARD EDITION - FINAL BULLETPROOF VERSION
# =========================================================
...
# 📡 ФИНАЛНО БЕЗГРЕШНО СГЛОБЯВАНЕ: ЛИНКЪТ Е ТВОЙ И Е ДИРЕКТЕН БЕЗ ПРОМЕНЛИВИ
...
def analyze_poisson_over_under(fixture_id):
    lambda_home, lambda_away = 2.96, 1.92
    prob_under_2_5 = 0.0
    for x in range(3):
        for y in range(3):
            if (x + y) < 3:
    ...
    while True:
        try:
            ...
            for match in live_matches:
                ...
                ch = extract(home_stats, "Corner Kicks")
                ca = extract(away_stats, "Corner Kicks")
                total_corners = ch + ca
                ...
                score_btts = (btts_prob * 0.5 + tempo * 0.3 + act * 0.2) if btts_prob is not None else 0.0
                score_over = (over_prob * 0.6 + tempo * 0.3 + act * 0.1) if over_prob is not None else 0.0
                market = None
