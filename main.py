...
def _final_builder_2to5(match, detailed=True):
    try:
        ...
        for n in range(BET_BUILDER_MIN_LEGS, max_n+1):
            for combo in combinations(candidates, n):
                families=[x.get('family') for x in combo]
...
def _final_live_scan():
    try:
        ...
        eligible=0
        for m in matches:
            minute=int(_v7_num(m.get('fixture',{}).get('status',{}).get('elapsed'),0))
            if LIVE_MINUTE <= minute <= LIVE_MAX_MINUTE:
                eligible += 1
                try:
                    candidates.extend(build_live_market_candidates(m) or [])
                except Exception as e:
                    logging.warning('LIVE CANDIDATE ERROR: %s',repr(e))
        ranked=rank_live_signals(candidates)
        sent=0
        for s in ranked:
            try:
                if send_live_signal(s): sent += 1
            except Exception as e: logging.warning('LIVE SEND ERROR: %s',repr(e))
        print(f'LIVE FINAL | matches={len(matches)} | eligible={eligible} | candidates={len(candidates)} | qualified={len(ranked)} | sent={sent}')
        return sent
    except Exception as e:
        logging.exception('LIVE FINAL ERROR: %s',repr(e)); return 0
...
def main_loop():
    ...
    if not api_health_check():
        print('❌ API NOT AVAILABLE'); return
    print('✅ API CONNECTION OK')
    # Run immediately after startup, then on fixed intervals.
    LAST_LIVE_SCAN=0; LAST_PREMATCH_SCAN=0
    while True:
        now=time.time(); cleanup_signal_memory()
        if now-LAST_LIVE_SCAN>=LIVE_SCAN_INTERVAL:
            print(datetime.now(TIMEZONE).strftime('%H:%M:%S'),'LIVE SCAN')
            print('LIVE SIGNALS SENT:',_final_live_scan()); LAST_LIVE_SCAN=now
        if now-LAST_PREMATCH_SCAN>=PREMATCH_SCAN_INTERVAL:
            print(datetime.now(TIMEZONE).strftime('%H:%M:%S'),'PREMATCH SCAN')
            print('PREMATCH SIGNALS SENT:',_final_prematch_scan()); LAST_PREMATCH_SCAN=now
        time.sleep(5)
...
if __name__=='__main__':
    ...
    except Exception: logging.exception('FATAL MAIN ERROR')
