    ...
    try:
        ...
        for match in matches:
            ...
            try:
                ...
                # 11:00 BG: remaining fixtures today. 21:00 BG: only
                # tomorrow 00:00-10:00, keeping the two feeds separate.
                local_match_time = match_time.astimezone(TIMEZONE)
                current_local = datetime.now(TIMEZONE)
                if current_local.hour >= 20:
                    target_date = (current_local + timedelta(days=1)).date()
                    if local_match_time.date() != target_date or local_match_time.hour >= 10:
                        continue
                elif local_match_time.date() != current_local.date():
                    continue
...
def _prematch_slot_key():
    now = datetime.now(TIMEZONE)
    return f"{now.strftime('%Y-%m-%d')}:{11 if now.hour < 20 else 21}"

def _prematch_daily_table():
    conn=sqlite3.connect(DB_NAME)
    cur=conn.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS prematch_daily_slots (slot_key TEXT PRIMARY KEY, normal_count INTEGER NOT NULL DEFAULT 0, builder_count INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL)")
    conn.commit()
    return conn, cur

def prematch_daily_already_sent():
    slot = _prematch_slot_key()
    conn,cur=_prematch_daily_table()
    cur.execute("SELECT normal_count FROM prematch_daily_slots WHERE slot_key=?",(slot,))
...
def mark_prematch_daily_sent(count):
    slot = _prematch_slot_key()
    conn,cur=_prematch_daily_table()
    cur.execute("INSERT OR IGNORE INTO prematch_daily_slots(slot_key,normal_count,builder_count,created_at) VALUES(?,?,?,?)",(slot,int(count),0,datetime.now(TIMEZONE).isoformat()))
    cur.execute("UPDATE prematch_daily_slots SET normal_count=? WHERE slot_key=?",(int(count),slot))
    conn.commit(); conn.close()

def builder_daily_already_sent():
    slot = _prematch_slot_key()
    conn,cur=_prematch_daily_table()
    cur.execute("SELECT builder_count FROM prematch_daily_slots WHERE slot_key=?",(slot,))
    row=cur.fetchone(); conn.close()
    return bool(row and row[0] > 0)

def mark_builder_daily_sent(count):
    slot = _prematch_slot_key()
    conn,cur=_prematch_daily_table()
    cur.execute("INSERT OR IGNORE INTO prematch_daily_slots(slot_key,normal_count,builder_count,created_at) VALUES(?,?,?,?)",(slot,0,int(count),datetime.now(TIMEZONE).isoformat()))
    cur.execute("UPDATE prematch_daily_slots SET builder_count=? WHERE slot_key=?",(int(count),slot))
    conn.commit(); conn.close()
...
def process_prematch_matches(
    ...
    # BET BUILDER TOP-3
    # -------------------------
    if not builder_daily_already_sent():
        ...
        if builder_sent > 0:
            mark_builder_daily_sent(builder_sent)
...
def main_loop():
    ...
    while True:
        ...
        # =============================================
        # PREMATCH SCAN — ONLY TWICE DAILY
        # =============================================
        # Do NOT poll every 5 minutes.  The live loop is independent and
        # remains exactly as it was.  Prematch is evaluated once in each
        # Bulgarian-time signal window: 11:00 and 21:00.
        global PREMATCH_LAST_SLOT
        # `now` is the epoch timestamp used by the interval timers above.
        # Keep it as a float for LIVE timing; use a timezone-aware datetime
        # separately for the Bulgarian-time PREMATCH hour/slot.
        current_dt = datetime.now(TIMEZONE)
        prematch_hour = current_dt.hour
        slot = f"{current_dt.strftime('%Y-%m-%d')}:{prematch_hour}"

        if prematch_hour in PREMATCH_SIGNAL_HOURS and PREMATCH_LAST_SLOT != slot:
            print(
            ...
            print("PREMATCH SIGNALS SENT:", prematch_sent)
            PREMATCH_LAST_SLOT = slot
...
def prematch_signal_allowed(signal):
    ...
    if p < 0 or p > 100:
        return False
    # Prematch value signals must not be negative EV/value.
...
MAX_PREMATCH_SIGNALS_PER_SCAN=5
MAX_LIVE_SIGNALS_PER_SCAN=5
# PREMATCH: run only at the two daily signal windows.
# The live scanner remains untouched at 60 seconds.
PREMATCH_DAILY_TOP5 = 5
PREMATCH_SIGNAL_HOURS = (11, 21)
PREMATCH_LAST_SLOT = None
...
def startup():
    ...
    # DAILY STATISTICAL SCANNER: separate scheduler.
    # It runs football only at 11:00/21:00 BG and staggers other sports.
    # This worker never touches the live scanner.
    def _statistics_scanner_worker():
        while True:
            try:
                run_due_scans(send_telegram)
            except Exception as e:
                print("STATISTICS SCANNER LOOP ERROR:", repr(e))
            time.sleep(60)

    threading.Thread(
        target=_statistics_scanner_worker,
        daemon=True,
        name="statistics-scanner"
    ).start()
