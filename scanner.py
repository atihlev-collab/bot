...
def get_other_sport_fixtures(sport, start_bg, end_bg):
    """Return upcoming games in the exact Sofia-local 24h window.

    One date request is used for each calendar date touched by the window.
    Results are filtered locally by Sofia time so the scanner always means
    exactly 12:00 BG today -> 12:00 BG tomorrow.
    """
    days = []
    d = start_bg.date()
    while d <= end_bg.date():
    ...
    games.sort(key=lambda g: _other_game_time_bg(g) or datetime.max.replace(tzinfo=TZ))
    return games
...
def _other_stats_for_games(sport, game_ids):
    """Use the score already returned by the games endpoint.

    For this daily totals scanner we only need points/goals/runs. Calling the
    separate game-statistics endpoint for every historical game wastes the
    sport API quota and is unnecessary because the games payload already
    contains the final score.
    """
    return {int(x): {} for x in game_ids}
...
# One daily check only: 10:00 BG.
# The window itself is always 12:00 BG today -> 12:00 BG tomorrow.
OTHER_SPORT_SCAN_HOUR = 10


def run_other_sports_scanner(reference_date=None, send_func=None):
    """Run every other-sport API once in the morning and send one report."""
    ref = reference_date or datetime.now(TZ).date()

    # Fresh daily calculation. Historical caches are process-local and should
    # not silently carry yesterday's team data into today's report.
    _OTHER_HISTORY_CACHE.clear()
    _OTHER_TEAM_HISTORY_CACHE.clear()
    _OTHER_STATS_CACHE.clear()
...
def run_due_scans(send_func):
    """Run football at 11:00/21:00 and other sports once at 10:00 BG."""
    init_scanner_db()
    now = datetime.now(TZ)
    today = now.date()

    # OTHER SPORTS — exactly once per day at/after 10:00 BG.
    if now.hour == OTHER_SPORT_SCAN_HOUR:
        key = f"other_sports:{today.isoformat()}"
        if not already_ran(key):
            print(_signal_text("OTHER SPORTS DAILY SCAN 10:00 STARTED"))
            try:
                run_other_sports_scanner(today, send_func)
                mark_ran(key)
                print(_signal_text("OTHER SPORTS DAILY SCAN 10:00 FINISHED"))
            except Exception as exc:
                print(_signal_text(f"OTHER SPORTS DAILY SCAN ERROR: {exc!r}"))
