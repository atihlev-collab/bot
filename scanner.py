...
# =========================================================
# OTHER SPORTS DAILY SCANNER — API-SPORTS
# Same history logic as the working football scanner:
# 1) current season only
# 2) same competition first
# 3) if fewer than 3, fallback to other official competitions
#    from the SAME current season
# 4) no old seasons, no friendlies
# =========================================================
...
_OTHER_API_CALLS = {sport: 0 for sport in OTHER_SPORTS}
_OTHER_API_DAILY_BUDGET = 90

# Caches live only for this daily run.
_OTHER_LEAGUE_CACHE = {}
_OTHER_TEAM_CACHE = {}
...
def _other_api(sport, endpoint, params=None, timeout=25):
    ...
    if _OTHER_API_CALLS[sport] >= _OTHER_API_DAILY_BUDGET:
        print(f"OTHER SPORTS API BUDGET STOP [{sport}] calls={_OTHER_API_CALLS[sport]}")
        return None
    ...
    try:
        ...
        if r.status_code == 429:
            print(f"OTHER SPORTS API RATE LIMIT [{sport}] {endpoint}")
            return None
        if r.status_code >= 400:
            print(f"OTHER SPORTS API HTTP ERROR [{sport}] {r.status_code} {endpoint}")
            return None
...
def _other_status(g):
    ...
    if isinstance(status, dict):
        return str(status.get("short") or status.get("long") or "").upper()
    return str(status).upper()


def _other_finished(g):
    status = _other_status(g)

    if any(x in status for x in ("CANC", "POST", "ABAN", "SUSP", "DELAY")):
        return False

    if any(x in status for x in ("FT", "FINAL", "FINISHED", "END", "AOT", "AET", "OT", "PEN")):
        return True

    return (
        _other_score(g, "home") is not None
        and _other_score(g, "away") is not None
    )
...
def _other_season(g):
    ...
    return (g.get("game") or {}).get("season")
...
def _other_official(g):
    ...
    return not (
        typ == "friendly"
        or "friendly" in name
        or "exhibition" in name
        or "club friendly" in name
    )


def _clean_history(games, season):
    """Exactly the football rule: current season + official + finished."""
    out = []
    seen = set()
    ...
    for g in games or []:
        ...
        if not _other_finished(g):
            continue

        if not _other_official(g):
            continue

        actual = _other_season(g)

        # The query is already season-scoped. If API returns an explicit
        # different season, reject it. Missing season metadata is accepted.
        if actual is not None and str(actual) != str(season):
            continue

        home, away = _other_teams(g)
    ...
    out.sort(key=lambda x: _other_dt(x) or datetime.min.replace(tzinfo=TZ), reverse=True)
    return out
...
def get_other_sport_fixtures(sport, start_bg, end_bg):
    """Same idea as football get_fixtures_for_window()."""
    out = []
    seen = set()
    day = start_bg.date()
    ...
    while day <= end_bg.date():
        for g in _other_games_for_date(sport, day):
            ...
            if not gid or gid in seen or not dt:
                continue

            if dt < start_bg or dt >= end_bg or dt <= datetime.now(TZ):
                continue

            if _other_finished(g):
                continue
    ...
    out.sort(key=lambda x: _other_dt(x) or datetime.max.replace(tzinfo=TZ))
    return out


def _league_history(sport, league_id, season):
    """Primary history: same competition, same current season."""
    key = (sport, str(league_id), str(season))

    if key not in _OTHER_LEAGUE_CACHE:
        games = _other_api(
            sport,
            "games",
            {"league": int(league_id), "season": season},
        )

        clean = _clean_history(games, season)
        _OTHER_LEAGUE_CACHE[key] = clean

        print(
            f"OTHER SPORTS PRIMARY HISTORY [{sport}] "
            f"league={league_id} season={season} matches={len(clean)}"
        )

    return _OTHER_LEAGUE_CACHE[key]


def _team_history(sport, team_id, season):
    """Fallback history: same team, other official competitions, same season."""
    key = (sport, int(team_id), str(season))

    if key not in _OTHER_TEAM_CACHE:
        games = _other_api(
            sport,
            "games",
            {"team": int(team_id), "season": season},
        )

        clean = _clean_history(games, season)
        _OTHER_TEAM_CACHE[key] = clean

        print(
            f"OTHER SPORTS FALLBACK HISTORY [{sport}] "
            f"team={team_id} season={season} matches={len(clean)}"
        )

    return _OTHER_TEAM_CACHE[key]


def _history_for_team(sport, team_id, league_id, season):
    """
    Same decision as football get_team_history():

    >=3 same competition -> use same competition.
    <3 -> use same competition + official matches from other
    competitions in the same current season.
    """
    primary = _league_history(sport, league_id, season)
    ...
    for g in primary:
        home, away = _other_teams(g)
        if int(home.get("id") or -1) == home_id or int(away.get("id") or -1) == home_id:
            same.append(g)

    same.sort(key=lambda x: _other_dt(x) or datetime.min.replace(tzinfo=TZ), reverse=True)

    if len(same) >= 3:
        print(
            f"OTHER SPORTS HISTORY SELECT [{sport}] "
            f"team={team_id} competition={league_id} matches={len(same[:12])}"
        )
        return same[:12]

    # Exact football fallback.
    all_current = _team_history(sport, team_id, season)

    other = [
        g for g in all_current
        if _other_league_id(g) != str(league_id)
    ]

    other.sort(key=lambda x: _other_dt(x) or datetime.min.replace(tzinfo=TZ), reverse=True)
    ...
    merged.sort(key=lambda x: _other_dt(x) or datetime.min.replace(tzinfo=TZ), reverse=True)

    print(
        f"OTHER SPORTS HISTORY FALLBACK SELECT [{sport}] "
        f"team={team_id} same={len(same)} other={len(other)} total={len(merged[:12])}"
    )
...
def _other_expected(sport, game):
    home, away = _other_teams(game)
    season = _other_season(game)
    league_id = _other_league_id(game)
    ...
    hp = _team_average(
        home.get("id"),
        _history_for_team(sport, home.get("id"), league_id, season),
    )
    ap = _team_average(
        away.get("id"),
        _history_for_team(sport, away.get("id"), league_id, season),
    )
...
def run_other_sports_scanner(reference_date=None, send_func=None):
    ...
    The important part is deliberately identical in meaning to football:
    upcoming fixture -> current season -> same competition -> fallback
    to other official competitions in the same current season.
    """
    _OTHER_LEAGUE_CACHE.clear()
    _OTHER_TEAM_CACHE.clear()

    for sport in OTHER_SPORTS:
        _OTHER_API_CALLS[sport] = 0
    ...
    for sport in OTHER_SPORTS:
        try:
            ...
            print(
                f"OTHER SPORTS [{sport}] FIXTURES={len(games)} "
                f"API_CALLS={_OTHER_API_CALLS[sport]}"
            )

            if not games:
                lines += [
                    OTHER_SPORTS[sport]["label"],
            ...
            for game in games:
                try:
                    expected = _other_expected(sport, game)

                    if expected:
                        results.append(_other_result(game, expected))

                except Exception as exc:
                    print(
                        f"OTHER SPORTS MATCH ERROR [{sport}] "
                        f"{_other_id(game)}: {exc!r}"
                    )

            total_valid += len(results)

            lines += [
                _format_other_sport(sport, results),
                "",
            ]

            print(
                f"OTHER SPORTS [{sport}] "
                f"VALID={len(results)} API_CALLS={_OTHER_API_CALLS[sport]}"
            )

        except Exception as exc:
            print(f"OTHER SPORTS ERROR [{sport}]: {exc!r}")
            lines += [
                OTHER_SPORTS[sport]["label"],
    ...
    lines.append(
        f"Мачове: {total_fixtures} | Валидни статистически сигнали: {total_valid}"
    )
...
def run_due_scans(send_func):
    """Football schedule remains unchanged; Other Sports runs once after 10:00."""
    init_scanner_db()
    ...
    # OTHER SPORTS: exactly once daily at/after 10:00 BG.
