...
def _build_sport_section(sport_name, metric, matches, start, end):
    ...
    # Top 4. If fewer valid matches exist, show only those available.
    # Rugby is special: the same match must NEVER appear in both OVER and UNDER.
    if sport_name.startswith("🏉"):
        # Split the available rugby matches into two disjoint groups.
        # Higher totals are OVER; lower totals are UNDER.
        split = (len(valid) + 1) // 2
        top_over = valid[:split][:4]
        top_under = list(reversed(valid[split:]))[:4]
    else:
        top_over = valid[:4]
...
def _send_sport_message_chunks(send_func, message, max_chars=3700):
    """Send a long Sport report in Telegram-safe chunks."""
    if not send_func:
        return
    ...
    for i, chunk in enumerate(chunks, 1):
        header = f"📊 SPORT DAILY STATISTICAL SCANNER ({i}/{total})\n\n" if total > 1 else ""
        send_func(header + chunk)
...
def run_sport_daily_scanner(send_func=None):
    ...
    if send_func:
        _send_sport_message_chunks(send_func, message)
