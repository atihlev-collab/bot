...
class APIQuotaExceeded(RuntimeError):
    def __init__(self, provider):
        ...
        super().__init__(f"{provider} API quota exceeded")
...
def _quota_used(provider="football"):
    _quota_init(); conn=_db()
    row=conn.execute("SELECT used FROM api_usage WHERE day=? AND provider=?", (_quota_today(),provider)).fetchone(); conn.close()
    return int(row[0]) if row else 0


def _quota_locked(provider="football"):
    _quota_init(); today=_quota_today(); conn=_db()
...
def _set_quota_lock(provider="football", reason="HTTP 429"):
    _quota_init(); conn=_db()
    conn.execute("INSERT OR REPLACE INTO api_quota_locks(provider,locked_day,reason) VALUES(?,?,?)",(provider,_quota_today(),str(reason)[:300])); conn.commit(); conn.close()


def _quota_add(provider="football"):
    _quota_init(); conn=_db(); today=_quota_today()
    conn.execute("INSERT OR IGNORE INTO api_usage(day,provider,used) VALUES(?,?,0)",(today,provider))
    conn.execute("UPDATE api_usage SET used=used+1 WHERE day=? AND provider=?",(today,provider)); conn.commit(); conn.close()


def _api(endpoint, params=None, timeout=25):
    """Single Highlightly Football request.

    Important quota rule: never retry HTTP 429 and never retry 5xx here.
    Every real request is counted by endpoint so the daily report can show
    exactly where quota was spent.
    """
    global _LAST_API_CALL
    if _quota_locked("football") or _quota_used("football") >= API_HARD_STOP:
        _set_quota_lock("football", "local safety limit")
    ...
    _quota_add("football")
    _api_endpoint_counts[endpoint] = _api_endpoint_counts.get(endpoint, 0) + 1
    try:
    ...
    if r.status_code == 429:
        ...
        print("HIGHLIGHTLY FOOTBALL API LIMIT: 429 — LOCKED; no further football requests today.")
        raise APIQuotaExceeded("football")
    ...
    try:
        ...
        print("SCANNER API ERROR:", endpoint, "invalid JSON")
        return None
...
def init_scanner_db():
    ...
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS scanner_fixture_odds (
            fixture_id INTEGER PRIMARY KEY,
            data TEXT NOT NULL,
            updated_at TEXT NOT NULL
        )
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS scanner_upcoming_matches (
            fixture_id INTEGER PRIMARY KEY,
            data TEXT NOT NULL,
...
def get_team_history(team_id, season, league_id=None):
    ...
    # Reuse yesterday's completed-match list for most of the day.  At the
    # next daily run (>20h old) it refreshes, so the current season is not
    # permanently stale while repeated calls do not re-download history.
    try:
        conn = _db()
        row = conn.execute(
            "SELECT data, updated_at FROM scanner_team_season_history WHERE team_id=? AND season=?",
            (team_id, season),
        ).fetchone()
        conn.close()
        if row:
            ...
            if (datetime.now(timezone.utc) - updated).total_seconds() < 20 * 3600:
                import json
    ...
    try:
        ...
        conn.execute(
            ...
            (team_id, season, json.dumps(primary), datetime.now(timezone.utc).isoformat()),
        )
        conn.commit(); conn.close()
    except Exception as exc:
        print("HISTORY CACHE WRITE ERROR:", team_id, repr(exc))
...
def load_historical_statistics(all_histories):
    ...
    print("SCANNER STATS CACHE:", cache_hits, "hits /", len(fixtures_by_id), "fixtures")
    return loaded
...
def _read_cached_odds(fixture_id):
    try:
        import json
        conn = _db(); row = conn.execute("SELECT data FROM scanner_fixture_odds WHERE fixture_id=?", (int(fixture_id),)).fetchone(); conn.close()
        return json.loads(row[0]) if row else None
    except Exception:
        return None


def _write_cached_odds(fixture_id, data):
    try:
        import json
        conn = _db(); conn.execute("INSERT OR REPLACE INTO scanner_fixture_odds(fixture_id,data,updated_at) VALUES (?,?,?)", (int(fixture_id), json.dumps(data), datetime.now(timezone.utc).isoformat())); conn.commit(); conn.close()
    except Exception as exc:
        print("ODDS CACHE WRITE ERROR:", fixture_id, repr(exc))


def get_cached_betano_odds(fixture_id):
    """Return cached Highlightly odds adapted to the legacy V7 schema."""
    raw = _read_cached_odds(fixture_id)
    if raw is None:
        return None
    grouped = {}
    for item in raw if isinstance(raw, list) else []:
        if not isinstance(item, dict):
            continue
        mid = item.get("matchId") or fixture_id
        books = grouped.setdefault(mid, {})
        for market in item.get("odds", []) or []:
            if not isinstance(market, dict):
                continue
            bid = market.get("bookmakerId")
            bname = market.get("bookmakerName") or ""
            if bid not in (BETANO_BOOKMAKER_ID, 32) and str(bname).casefold() != "betano":
                continue
            b = books.setdefault((bid, bname), {"id": bid, "name": bname, "bets": []})
            name = market.get("market") or ""
            low = str(name).casefold()
            if low == "full time result": name = "Match Winner"
            elif low == "both teams to score": name = "Both Teams To Score"
            vals=[]
            for v in market.get("values", []) or []:
                if isinstance(v, dict):
                    vals.append({"value": v.get("value"), "odd": v.get("odd"), "stopped": False, "main": v.get("main")})
            if vals:
                b["bets"].append({"id": None, "name": name, "values": vals})
    response=[]
...
def run_daily_scanner(mode="day", reference_date=None, send_func=None):
    ...
    # Save the exact upcoming window so the main V7 engine can reuse the same
    # fixture list instead of spending two more match-list requests.
    try:
        import json
        conn = _db()
        conn.execute("DELETE FROM scanner_upcoming_matches")
        for m in matches:
            fid = (m.get("fixture") or {}).get("id")
            md = (m.get("fixture") or {}).get("date") or ""
            if fid:
                conn.execute("INSERT OR REPLACE INTO scanner_upcoming_matches(fixture_id,data,match_date,updated_at) VALUES (?,?,?,?)", (int(fid), json.dumps(m), str(md), datetime.now(timezone.utc).isoformat()))
        conn.commit(); conn.close()
    except Exception as exc:
        print("UPCOMING CACHE WRITE ERROR:", repr(exc))

    # Build the statistical result first.  Odds are checked only for matches
    # that actually have at least one statistical market.  This is the main
    # quota-saving change: we no longer spend one odds request on every match.
    # GLOBAL BLOCK: Russia + Belarus are never scanned.
    matches = [m for m in matches if not _is_blocked_fixture(m)]
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        futures = {pool.submit(get_team_history, tid, season, league_id): (tid, league_id, season) for tid, league_id, season in unique_requests}
        for fut in as_completed(futures):
            key = futures[fut]
            try:
                ...
                if isinstance(exc, APIQuotaExceeded):
                    raise
                print("SCANNER HISTORY ERROR:", key, repr(exc)); histories[key] = []
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        def analyse_one(m):
            ...
            profiles = {home_id: team_profiles.get((home_id, hs[0], hs[1]), {}) if hs else {}, away_id: team_profiles.get((away_id, aws[0], aws[1]), {}) if aws else {}}
            return analyse_fixture(m, profiles)
        futures = {pool.submit(analyse_one, m): m for m in matches}
        for fut in as_completed(futures):
            try: results.append(fut.result())
            except Exception as exc:
                if isinstance(exc, APIQuotaExceeded): raise
                print("SCANNER MATCH ERROR:", repr(exc))

    # Only now query Betano, and only for matches where the statistics engine
    # found a candidate market.  Goals do not require a separate Betano check;
    # corners/cards/shots are removed only when Betano explicitly confirms the
    # market is unavailable.
    candidates_for_odds = [r for r in results if r.get("markets")]
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        futures = {pool.submit(get_betano_prematch_markets, int(r["fixture_id"])): r for r in candidates_for_odds}
        for fut in as_completed(futures):
            r = futures[fut]
            try: r["_betano_markets"] = fut.result()
            except Exception as exc:
                if isinstance(exc, APIQuotaExceeded): raise
                r["_betano_markets"] = {}

    for r in results:
        betano = r.get("_betano_markets", {})
        r["markets"] = {k:v for k,v in r.get("markets", {}).items() if k == "goals" or betano.get(k) is not False}
    ...
    lines.append(f"\n⏱ Scan time: {time.time() - scan_started:.1f}s")
    ...
    print("FOOTBALL API REQUESTS:", sum(_api_endpoint_counts.values()))
    print("FOOTBALL API BY ENDPOINT:", dict(sorted(_api_endpoint_counts.items())))
...
def get_cached_upcoming_matches():
    """Return the latest daily scanner fixture window without an API call."""
    try:
        import json
        conn = _db()
        rows = conn.execute("SELECT data, updated_at FROM scanner_upcoming_matches ORDER BY match_date ASC").fetchall()
        conn.close()
        if not rows:
            return []
    ...
    if now.hour > 10 or (now.hour == 10 and now.minute >= 30):
        ...
        if not already_ran(football_key):
            ...
            try:
                ...
                print(_signal_text(f"FOOTBALL DAILY SCANNER STOPPED: {exc!r}"))
                mark_ran(football_key)
            except Exception as exc:
