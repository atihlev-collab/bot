...
SPORT_API_LIMIT = 100
SPORT_HISTORY_FROM = "2025-07-01"  # broad enough to cover the current season; extractor keeps newest season only
...
SPORTS_CONFIG = {
    ...
    "american-football": {"name": "🏈 NFL / NCAA", "endpoint": "american-football/matches", "stats": "american-football/teams/statistics", "metric": "points"},
...
def _extract_team_average(stats_rows, metric):
    """Return ONLY the newest/current-season team average exposed by Sport Ultra.

    Older seasons are never combined with the newest season.
    """
    ...
    for row in stats_rows:
        ...
        games = _recursive_number(
            total, {"played", "gamesplayed", "games", "matchesplayed"}
        )
        scored = _recursive_number(total, metric_names)
        if scored is None:
            scored = _recursive_number(row, metric_names)

        if not games or games <= 0 or scored is None:
            continue

        season = _recursive_number(row, {"season", "seasonid", "year"})
        if season is None:
            season = _recursive_number(total, {"season", "seasonid", "year"})

        # Do not invent a season. Rows with no season are retained only as a
        # fallback if the API returned no explicitly-seasoned rows at all.
        candidates.append((int(season) if season else None, games, scored, row))

    if not candidates:
        return None

    explicit = [x for x in candidates if x[0] is not None]
    pool = explicit if explicit else candidates

    if explicit:
        current_season = max(x[0] for x in explicit)
        pool = [x for x in explicit if x[0] == current_season]

    # If several rows exist for the same current season, prefer the row with
    # the greatest games played; never add seasons together.
    season, games, scored, raw = max(pool, key=lambda x: x[1])
...
def _volleyball_set_points_from_match(match, team_id):
    """Sum actual rally points scored by a team across completed sets."""
    home = match.get("homeTeam") or match.get("home") or {}
    away = match.get("awayTeam") or match.get("away") or {}
    ...
    score = (match.get("state") or {}).get("score") or {}
    if not isinstance(score, dict):
        return None

    total = 0
    sets = 0
    for key in ("firstSet", "secondSet", "thirdSet", "fourthSet", "fifthSet"):
        raw = score.get(key)
        if not raw:
            continue
        ...
        if not m:
            continue
        home_points, away_points = int(m.group(1)), int(m.group(2))
        total += home_points if team_id == home_id else away_points
        sets += 1

    return total if sets >= 3 else None


def _get_volleyball_points_average(match, team_id):
    """Current-season/competition average of actual points per volleyball match."""
    if not team_id:
        return None
    ...
    cache_key = ("volleyball_points", int(team_id), season, league_id)
    if cache_key in _SPORT_STATS_CACHE:
        return _SPORT_STATS_CACHE[cache_key]

    base = {"season": season, "limit": SPORT_API_LIMIT}
    if league_id:
    ...
    for side in ("homeTeamId", "awayTeamId"):
        params = dict(base)
        params[side] = int(team_id)
        data = _sport_api_get("volleyball/matches", params)
    ...
    for row in unique.values():
        points = _volleyball_set_points_from_match(row, int(team_id))
        if points is not None:
            values.append(points)

    if not values:
        print(f"SPORT HISTORY: volleyball team={team_id} NO POINT DATA")
        _SPORT_STATS_CACHE[cache_key] = None
        return None
    ...
    print(
        ...
        f"points_per_match={result['average']:.2f} games={result['games']}"
    )
    return result
...
def _american_football_competition(match):
    """Return NFL or NCAA for an American-football fixture, otherwise None."""
    league = match.get("league") or {}
    name = ""
    if isinstance(league, dict):
        name = str(league.get("name") or league.get("leagueName") or "")
    else:
        name = str(league or "")
    ...
    if "nfl" in text:
        return "NFL"
    if "ncaa" in text or "college" in text or "fbs" in text:
        return "NCAA"
    return None
...
def _build_sport_section(sport_name, candidates):
    ...
    if sport_name.startswith("🏉"):
        over_ids = {id(item) for item in top_over}
        under_pool = [
            item for item in sorted(candidates, key=lambda x: x["expected"])
            if id(item) not in over_ids
        ]
        top_under = under_pool[:4]
    else:
...
def run_due_scans(send_func):
    """Run Sport Statistics once per day; football daily scans remain disabled."""
    init_scanner_db()
    now = datetime.now(TZ)
    today = now.date()
    sport_key = f"sport_ultra_v7:{today.isoformat()}"
