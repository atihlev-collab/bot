...
def run_due_scans(send_func):
    ...
    if now.hour >= 10:
        ...
        if not already_ran(other_key):
            # Do NOT send the report with missing sports. If the service was
            # restarted after the 09:30-10:00 collection window, recover the
            # missing products one at a time every 5 minutes.
            results = _get_other_sport_results(today)
            missing = [sport for sport in OTHER_SPORTS if sport not in results]

            if missing:
                recovery_slot = (now.hour * 60 + now.minute) // 5
                recovery_key = (
                    f"other_sports_recovery:{today.isoformat()}:"
                    f"{recovery_slot}"
                )
                if not already_ran(recovery_key):
                    sport = missing[0]
                    print(_signal_text(
                        f"OTHER SPORTS RECOVERY {sport.upper()} STARTED"
                    ))
                    try:
                        report, fixtures, valid = _run_one_other_sport(sport, today)
                        _save_other_sport_result(
                            today, sport, report, fixtures, valid
                        )
                        print(_signal_text(
                            f"OTHER SPORTS RECOVERY {sport.upper()} FINISHED: "
                            f"valid={valid} fixtures={fixtures}"
                        ))
                    except Exception as exc:
                        print(_signal_text(
                            f"OTHER SPORTS RECOVERY {sport.upper()} ERROR: {exc!r}"
                        ))
                    mark_ran(recovery_key)
            else:
                ...
                try:
                    ...
                    print(_signal_text(
                        f"OTHER SPORTS DAILY REPORT ERROR: {exc!r}"
                    ))
