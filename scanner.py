...
def _build_sport_section(sport_name, metric, matches, start, end):
    ...
    if sport_name.startswith("🏉"):
        # Rugby: the same fixture must never appear in both groups.
        over_ids = {id(item) for item in top_over}
        under_pool = [item for item in sorted(valid, key=lambda x: x["value"])
                      if id(item) not in over_ids]
        top_under = under_pool[:4]
    else:
        top_under = sorted(valid, key=lambda x: x["value"])[:4]
...
_LAST_SPORT_SEND_OK = False

def _send_telegram_chunks(message, send_func):
    """Send a long Sport report in safe Telegram-sized chunks."""
    global _LAST_SPORT_SEND_OK
    _LAST_SPORT_SEND_OK = False
    if not send_func:
        _LAST_SPORT_SEND_OK = True
        return True
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        result = send_func(chunk)
        if result is False:
            ok = False
            print(f"SPORT TELEGRAM CHUNK FAILED: {i}/{total}")
        else:
            print(f"SPORT TELEGRAM CHUNK SENT: {i}/{total} chars={len(chunk)}")
    _LAST_SPORT_SEND_OK = ok
    return ok
...
def run_sport_daily_scanner(send_func=None):
    ...
    print(message)
    _send_telegram_chunks(message, send_func)
...
def run_due_scans(send_func):
    """SPORT-ONLY daily scheduler. Football is intentionally disabled."""
    init_scanner_db()

    now = datetime.now(TZ)
    today = now.date()

    # New versioned key forces one clean run after this deployment.
    # Afterwards it remains once per day.
    sport_key = f"sport_ultra_v6:{today.isoformat()}"

    if already_ran(sport_key):
        return True

    print(_signal_text("SPORT DAILY SCANNER STARTED"))
    try:
        run_sport_daily_scanner(send_func)
        if _LAST_SPORT_SEND_OK:
            mark_ran(sport_key)
            print(_signal_text("SPORT DAILY SCANNER FINISHED"))
            return True

        print(_signal_text("SPORT DAILY SCANNER NOT MARKED — TELEGRAM SEND FAILED"))
        return False
