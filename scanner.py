...
def _api(endpoint, params=None, timeout=25):
    ...
    for attempt in range(5):
        try:
            ...
            # Highlightly Football API may return either a JSON object
            # containing `data` or the data itself as a JSON list.
            # Never call .get() on a list.
            if isinstance(payload, list):
                return payload

            if not isinstance(payload, dict):
                print("SCANNER API ERROR: unexpected JSON type:", endpoint, type(payload).__name__)
                return None
...
def run_due_scans(send_func):
    ...
    if now.hour > 10 or (now.hour == 10 and now.minute >= 30):
        key = f"football_daily:{today.isoformat()}"
    ...
    # Sport Statistics stays enabled and separate.
...
def run_football_daily_now(send_func=None):
    """Run the football daily scanner immediately for testing."""
    return run_daily_scanner("day", datetime.now(TZ).date(), send_func)
