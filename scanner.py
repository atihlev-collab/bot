...
# =========================================================
# =========================================================
# OTHER SPORTS — SOFASCORE PUBLIC DATA
# One daily scan at 10:00 BG. Window: 12:00 today -> 12:00 tomorrow.
# Historical sample: CURRENT SEASON ONLY, minimum 3 official matches.
# NBA is naturally separate from general Basketball by tournament/league.
# =========================================================

SOFASCORE_BASE = "https://api.sofascore.app/api/v1"
SOFASCORE_HEADERS = {
...
# Sofascore sport slugs. NBA is separated at the tournament level from other basketball.
OTHER_SPORTS = {
    "nba": {"label": "🏀 NBA", "slug": "basketball", "kind": "points", "league_filter": "nba"},
    "basketball": {"label": "🏀 БАСКЕТБОЛ", "slug": "basketball", "kind": "points", "league_filter": None},
    "hockey": {"label": "🏒 ХОКЕЙ", "slug": "ice-hockey", "kind": "goals", "league_filter": None},
    "handball": {"label": "🤾 ХАНДБАЛ", "slug": "handball", "kind": "goals", "league_filter": None},
    "rugby": {"label": "🏉 РЪГБИ", "slug": "rugby", "kind": "points", "league_filter": None},
    "american_football": {"label": "🏈 NFL / NCAA", "slug": "american-football", "kind": "points", "league_filter": None},
    "baseball": {"label": "⚾ БЕЙЗБОЛ", "slug": "baseball", "kind": "runs", "league_filter": None},
}

_SOFASCORE_CACHE = {}
_SOFASCORE_LOCK = threading.Lock()
_SOFASCORE_LAST_CALL = 0.0
_SOFASCORE_MIN_INTERVAL = 0.35
...
def _sofa_total(event):
    h = _sofa_score(event, "home")
    a = _sofa_score(event, "away")
    return h + a if h is not None and a is not None else None


def _sofa_completed(event):
    status = event.get("status") or {}
    typ = str(status.get("type") or "").lower()
    desc = str(status.get("description") or "").lower()
    return bool(status.get("type") in {"finished", "ended"} or "final" in desc or typ in {"finished", "ended"})


def _sofa_official(event):
    t = event.get("tournament") or {}
    name = _norm(t.get("name") or t.get("slug"))
    bad = ("friendly", "friendlies", "exhibition", "preseason", "pre-season", "test match", "club friendly")
    return not any(x in name for x in bad)


def _sofa_season_id(event):
    s = event.get("season") or {}
    return s.get("id")
...
def _sofa_tournament_id(event):
    ...
    return u.get("id") or t.get("id")


def _sofa_tournament_name(event):
    t = event.get("tournament") or {}
    u = t.get("uniqueTournament") or {}
    return u.get("name") or t.get("name") or ""


def _sofa_daily_games(sport, start_bg, end_bg):
    cfg = OTHER_SPORTS[sport]
    out, seen = [], set()
    d = start_bg.date()
    while d <= end_bg.date():
        data = _sofa_get(f"sport/{cfg['slug']}/scheduled-events/{d.isoformat()}") or {}
        for event in data.get("events", []):
            eid = event.get("id")
            dt = _sofa_dt(event)
            if not eid or eid in seen or not dt:
                continue
            if start_bg <= dt < end_bg and dt > datetime.now(TZ):
                seen.add(eid)
                if cfg["league_filter"]:
                    tn = _norm(_sofa_tournament_name(event))
                    if "nba" not in tn:
                        continue
                out.append(event)
    out.sort(key=lambda e: _sofa_dt(e) or datetime.max.replace(tzinfo=TZ))
    return out


def _sofa_team_history(team_id, max_pages=2):
    events = []
    for page in range(max_pages):
        data = _sofa_get(f"team/{int(team_id)}/events/last/{page}") or {}
        page_events = data.get("events", [])
        if not page_events:
            break
        events.extend(page_events)
        # Two pages are enough for our minimum-3 current-season sample.
        if len(events) >= 40 or not data.get("hasNextPage"):
            break
    return events


def _sofa_current_history(team_id, current_event, minimum=3):
    """Current season only. Same competition first; other official competitions second."""
    season_id = _sofa_season_id(current_event)
    tournament_id = _sofa_tournament_id(current_event)
    all_events = _sofa_team_history(team_id, max_pages=2)
    candidates = []
    for e in all_events:
        if not _sofa_completed(e) or not _sofa_official(e):
            continue
        if _sofa_season_id(e) != season_id:
            continue
        if _sofa_total(e) is None:
            continue
        candidates.append(e)
    same = [e for e in candidates if _sofa_tournament_id(e) == tournament_id]
    same.sort(key=lambda e: _sofa_dt(e) or datetime.min.replace(tzinfo=TZ), reverse=True)
    if len(same) >= minimum:
        return same[:minimum]
    candidates.sort(key=lambda e: _sofa_dt(e) or datetime.min.replace(tzinfo=TZ), reverse=True)
    return candidates[:minimum]


def _sofa_team_average(team_id, history):
    vals = []
    for e in history:
        h = _sofa_team(e, "home")
        side = "home" if int(h.get("id") or -1) == int(team_id) else "away"
        score = _sofa_score(e, side)
...
def _sofa_expected(event, home_hist, away_hist):
    home_id = (_sofa_team(event, "home") or {}).get("id")
    away_id = (_sofa_team(event, "away") or {}).get("id")
    ha = _sofa_team_average(home_id, home_hist)
    aa = _sofa_team_average(away_id, away_hist)
    if not ha or not aa:
        return None
    # Simple, transparent expectation: each team's current-season scoring average.
    expected = ha[0] + aa[0]
    return {"home": ha[0], "away": aa[0], "expected": expected, "sample": min(ha[1], aa[1])}


def _format_sofa_sport(sport, rows):
    label = OTHER_SPORTS[sport]["label"]
    rows = [r for r in rows if r.get("expected") is not None]
    high = sorted(rows, key=lambda r: (r["expected"]["expected"], r["expected"]["sample"]), reverse=True)[:3]
    low = sorted(rows, key=lambda r: (r["expected"]["expected"], -r["expected"]["sample"]))[:3]
    lines = [label, "🔥 НАЙ-МНОГО"]
    if high:
        for i, r in enumerate(high, 1):
            x = r["expected"]
            lines += [
                f"{i}. {r['home']} - {r['away']}",
                f"   Средно: {x['home']:.2f} + {x['away']:.2f} = {x['expected']:.2f}",
                f"   Лига: {r['league']}",
                f"   Начало: {r['kickoff']} BG",
                f"   Извадка: {x['sample']} мача",
                "",
            ]
    else:
        lines.append("Няма поне 3 валидни мача от текущия сезон.")
    lines += ["❄️ НАЙ-МАЛКО"]
    if low:
        for i, r in enumerate(low, 1):
            x = r["expected"]
...
def _run_one_other_sport(sport, reference_date):
    ...
    games = _sofa_daily_games(sport, start, end)
    print(f"SOFASCORE [{sport}]: fixtures={len(games)}")
    rows = []
    team_hist_cache = {}
    for game in games:
        home = _sofa_team(game, "home")
        away = _sofa_team(game, "away")
        hid, aid = home.get("id"), away.get("id")
        if not hid or not aid:
            continue
        try:
            if hid not in team_hist_cache:
                team_hist_cache[hid] = _sofa_current_history(hid, game)
            if aid not in team_hist_cache:
                team_hist_cache[aid] = _sofa_current_history(aid, game)
            expected = _sofa_expected(game, team_hist_cache[hid], team_hist_cache[aid])
            if not expected:
                continue
            dt = _sofa_dt(game)
            rows.append({
                ...
                "league": _sofa_tournament_name(game),
                "kickoff": dt.strftime("%H:%M") if dt else "?",
                "expected": expected,
            })
        except Exception as exc:
            print("SOFASCORE MATCH ERROR:", sport, repr(exc))
    return _format_sofa_sport(sport, rows), len(games), len(rows)
...
def _save_other_sport_result(run_date, sport, report, fixtures, valid):
    ...
    conn.execute(
        """INSERT OR REPLACE INTO other_sports_daily_results
           ...
           VALUES (?, ?, ?, ?, ?, ?)""",
        (run_date.isoformat(), sport, report, int(fixtures), int(valid), datetime.now(timezone.utc).isoformat()),
    )
    conn.commit()
    conn.close()
...
def _get_other_sport_results(run_date):
    conn = _db()
    rows = conn.execute(
        "SELECT sport, report, fixtures, valid FROM other_sports_daily_results WHERE run_date=?",
        (run_date.isoformat(),),
    ).fetchall()
    conn.close()
...
def _send_other_sports_daily_report(reference_date, send_func):
    ...
    lines = [
        ...
        "Източник: SofaScore public data | текущ сезон само | минимум 3 официални мача",
        "",
    ]
    ...
    for sport in OTHER_SPORTS:
        ...
        if item:
            ...
            lines += [OTHER_SPORTS[sport]["label"], "Данните не бяха събрани.", ""]
    lines.append(f"Мачове: {total_fixtures} | Валидни сигнали: {total_valid}")
    message = "\n".join(lines)
    if send_func:
        send_func(message)
    return message
...
def run_other_sports_scanner(reference_date=None, send_func=None):
    ...
    for sport in OTHER_SPORTS:
        try:
            ...
            print("SOFASCORE OTHER SPORTS ERROR:", sport, repr(exc))
            _save_other_sport_result(ref, sport, f"{OTHER_SPORTS[sport]['label']}\nГрешка при зареждането на данните.", 0, 0)
    return _send_other_sports_daily_report(ref, send_func) if send_func else _get_other_sport_results(ref)


def run_due_scans(send_func):
    """Other sports once at 10:00 BG; football remains 11:00 and 21:00."""
    init_scanner_db()
    now = datetime.now(TZ)
    today = now.date()

    if now.hour == 10:
        key = f"other_sports:{today.isoformat()}"
        if not already_ran(key):
            print(_signal_text("SOFASCORE OTHER SPORTS 10:00 STARTED"))
            try:
                run_other_sports_scanner(today, send_func)
                mark_ran(key)
                print(_signal_text("SOFASCORE OTHER SPORTS 10:00 FINISHED"))
            except Exception as exc:
                print(_signal_text(f"SOFASCORE OTHER SPORTS 10:00 ERROR: {exc!r}"))
