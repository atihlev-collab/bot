...
def get_other_team_history(sport, team_id, season, league_id=None):
    ...
    games = _other_api(sport, "games", {"team": int(team_id), "season": season})
    current = _other_history_clean(games, season)

    # Football rule copied exactly in spirit:
    # 1) same competition first; 2) if fewer than 3, use other official
    # competitions from the SAME CURRENT SEASON. Never go to an old season.
    if league_id:
    ...
    if len(same) >= 3:
        ...
        print(
            "OTHER SPORTS HISTORY FALLBACK:", sport, int(team_id),
            "competition_matches=", len(same),
            "-> current-season official matches=", len(result),
        )
