...
# The daily scanner is the single source for the 12:00 BG -> next-day 12:00 BG window.
...
# Compatibility helpers used by the legacy main scheduler.
# The scanner itself does not maintain a separate quota-lock subsystem,
# so an absent lock is treated as unlocked. The main scheduler still
# guarantees that each daily Sport/Football scan is attempted only once.
class APIQuotaExceeded(Exception):
    pass


def _quota_locked(scope):
    return False
...
def _betano_market_name(market):
    ...
    value = (
        ...
        or (market.get("marketInfo") or {}).get("name")
        or ""
    )
    return str(value).strip().casefold()


def _classify_betano_market(name):
    n = str(name or "").casefold().strip()
    if not n:
        return None
    if (
        "total corners" in n
        or "total corner" in n
        or "over/under corners" in n
        or n.startswith("corners ")
        or n == "corners"
    ):
        return "corners"
    if (
        "total cards" in n
        or "total card" in n
        or "over/under cards" in n
        or "bookings" in n
        or "booking" in n
        or n.startswith("cards ")
        or n == "cards"
    ):
        return "cards"
    if (
        "total shots" in n
        or "total shot" in n
        or "over/under shots" in n
        or n.startswith("shots ")
        or n == "shots"
    ):
        return "shots"
    if (
        "total goals" in n
        or "total goal" in n
        or "over/under goals" in n
        or n in {"goals", "goal", "match goals"}
    ):
        return "goals"
    return None


def _read_betano_odds_rows(fixture_id, use_name=False):
    params = {
    ...
    if use_name:
        params["bookmakerName"] = BETANO_BOOKMAKER_NAME
    else:
        params["bookmakerId"] = BETANO_BOOKMAKER_ID
    return _api("odds", params)


def get_betano_prematch_markets(fixture_id):
    """Detect actual Betano markets exposed by Highlightly.

    Every scanner market is independently gated.  No market is inferred from
    historical statistics, and goals no longer bypass the Betano check.
    """
    result = {
        "match": False,
        "goals": False,
    ...
    rows = _read_betano_odds_rows(fixture_id, use_name=False)

    def consume(rows_to_read):
        found_betano_row = False
        for row in rows_to_read if isinstance(rows_to_read, list) else []:
            ...
            for market in row.get("odds", []) or []:
                ...
                bookmaker_name = str(market.get("bookmakerName") or "").strip().casefold()
                if bookmaker_id is not None:
                    try:
                        ...
                        continue
                elif bookmaker_name and bookmaker_name != BETANO_BOOKMAKER_NAME.casefold():
                    continue

                name = _betano_market_name(market)
                if name:
                ...
                found_betano_row = True
                result["match"] = True
                key = _classify_betano_market(name)
                if key:
                    result[key] = True
        return found_betano_row

    found = consume(rows)

    # If bookmaker ID 32 is stale/changed, use Highlightly's documented
    # bookmakerName filter as a fallback. This normally costs no extra request.
    if not found:
        try:
            rows_by_name = _read_betano_odds_rows(fixture_id, use_name=True)
            consume(rows_by_name)
        except APIQuotaExceeded:
            raise
        except Exception as exc:
            print("BETANO NAME FALLBACK ERROR:", fixture_id, repr(exc))
...
def run_daily_scanner(mode="day", reference_date=None, send_func=None):
    ...
    # One football daily scan: sent at/after 10:00 BG.
    ...
    # Later PREMATCH runs reuse this cache and make no second fixture-list call.
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        def analyse_one(m):
            ...
            # Every scanner market must be explicitly present at Betano.
            result["markets"] = {
                k: v
