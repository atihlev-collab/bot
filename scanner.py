...
# Same history flow as football. SofaScore is NOT used.
...
def _other_primary_history(sport, team_id, season, league_id):
    if not league_id:
        return []
    games = _other_api(sport, "games", {"team": int(team_id), "league": int(league_id), "season": season})
    clean = []
    for g in games or []:
        if str(_other_season(g)) != str(season) or _other_league_id(g) != str(league_id):
            continue
        if not _other_is_finished(g) or not _other_is_official(g):
            continue
        if not _other_game_id(g):
            continue
        clean.append(g)
    clean = {int(_other_game_id(g)): g for g in clean}
    out = list(clean.values())
    out.sort(key=lambda x: _other_game_time_bg(x) or datetime.min.replace(tzinfo=TZ), reverse=True)
    return out[:12]


def get_other_team_history(sport, team_id, season, league_id=None):
    """Football-style rule: same competition first; then official current-season matches."""
    key = (sport, int(team_id), str(season), str(league_id or ""))
    if key in _OTHER_HISTORY_CACHE:
        return _OTHER_HISTORY_CACHE[key]
    primary = _other_primary_history(sport, team_id, season, league_id)
    if len(primary) >= 3:
        primary.sort(key=lambda x: _other_game_time_bg(x) or datetime.min.replace(tzinfo=TZ))
        _OTHER_HISTORY_CACHE[key] = primary
        print("OTHER SPORTS HISTORY:", sport, team_id, "competition_matches=", len(primary), "-> current competition")
        return primary
    print("HISTORY FALLBACK:", team_id, "competition_matches=", len(primary), "-> current-season official matches")
    all_games = _other_api(sport, "games", {"team": int(team_id), "season": season})
    clean = _other_history_clean(all_games, season)
    print("HISTORY FALLBACK RESULT:", team_id, "matches=", len(clean))
    _OTHER_HISTORY_CACHE[key] = clean
    return clean
...
def _format_other_sport(sport, results):
    ...
    for i, r in enumerate(high, 1):
        x = r["expected"]; dt = _other_game_time_bg({"date": r["date"]})
...
def run_other_sports_scanner(reference_date=None, send_func=None):
    ...
    for sport in OTHER_SPORTS:
        try:
            ...
            histories = {}; unique = {}
            for g in games:
                season = _other_season(g); home, away = _other_game_teams(g); league_id = _other_league_id(g)
            ...
            for team_id, season, league_id in unique:
                try:
                    histories[(team_id, str(season), league_id)] = get_other_team_history(sport, team_id, season, league_id)
                except Exception as exc:
                    print("OTHER SPORTS HISTORY ERROR:", sport, team_id, repr(exc)); histories[(team_id, str(season), league_id)] = []
            all_hist_games = {int(_other_game_id(g)): g for h in histories.values() for g in h if _other_game_id(g)}
            stats_by_game = _other_stats_for_games(sport, list(all_hist_games))
            print(f"OTHER SPORTS HISTORY STATS [{sport}]: games={len(all_hist_games)} with_data={sum(1 for v in stats_by_game.values() if v)}")
            results = []
            for g in games:
                try:
                    x = _other_fixture_expected(g, histories, stats_by_game)
                    if x: results.append(_other_fixture_result(sport, g, x))
                except Exception as exc:
            ...
            print("OTHER SPORTS SCANNER ERROR:", sport, repr(exc)); lines += [OTHER_SPORTS[sport]["label"], "Грешка при зареждането на данните.", ""]
    lines.append(f"Мачове: {total_fixtures} | Валидни статистически сигнали: {total_valid}")
    message = "\n".join(lines)
    with _CONSOLE_LOCK: print(f"OTHER SPORTS MESSAGE READY — {total_valid} valid signals / {total_fixtures} fixtures")
    if send_func: send_func(message)
    return message
