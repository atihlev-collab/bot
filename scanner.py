...
def _sport_league_country(match):
    ...
    if not country_name:
        ...
        if isinstance(direct, dict):
            country_name = (
                direct.get("name")
                or direct.get("countryName")
                or direct.get("country_name")
                or ""
            )
        elif direct:
            country_name = str(direct)
    ...
    if not country:
        ...
        if isinstance(country_obj, dict):
            country = (
                ...
                or country_obj.get("countryName")
                or country_obj.get("country_name")
                or ""
            )
        elif country_obj:
...
def run_due_scans(send_func):
    """Temporary SPORT-ONLY scheduler.

    Football daily scans are disabled here.
    LIVE/PREMATCH football are disabled in main.py.
    Sport statistics can be tested immediately once per day.
    """
    init_scanner_db()
    ...
    # =====================================================
    # SPORT STATISTICS — TEST / DAILY
    # =====================================================
    # Run once on the first scheduler pass of the day.
    # This intentionally does NOT call any football endpoint.
