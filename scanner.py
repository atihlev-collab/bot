...
def _other_league_history(sport, league_id, season):
    """Fetch one competition/season once and reuse it for every team in it."""
    key = (sport, str(league_id), str(season))
    if not league_id:
        return []
    if key not in _OTHER_HISTORY_CACHE:
        games = _other_api(sport, "games", {"league": int(league_id), "season": season})
        _OTHER_HISTORY_CACHE[key] = _other_history_clean(games, season)
        print(f"OTHER SPORTS HISTORY [{sport}] league={league_id} season={season} games={len(_OTHER_HISTORY_CACHE[key])}")
    return _OTHER_HISTORY_CACHE[key]


def _build_other_histories(sport, games):
    """Build all team histories from competition-level data, not one API call per team."""
    # One request per unique competition/season. This is the critical fix for the
    # 100-request/day free API limit: a league response contains every team game.
    competitions = {}
    for g in games:
        league_id = _other_league_id(g)
        season = _other_season(g)
        if league_id and season is not None:
            competitions[(str(league_id), str(season))] = (league_id, season)

    by_team = {}
    for league_id, season in competitions.values():
        for hist_game in _other_league_history(sport, league_id, season):
            home, away = _other_game_teams(hist_game)
            for team in (home, away):
                tid = team.get("id")
                if tid:
                    by_team.setdefault(int(tid), []).append(hist_game)

    # Deduplicate and keep the same-tournament games first for each upcoming match.
    for tid, arr in list(by_team.items()):
        uniq = {}
        for g in arr:
            uniq[_other_game_id(g)] = g
        by_team[tid] = sorted(uniq.values(), key=lambda x: _other_game_time_bg(x) or datetime.min.replace(tzinfo=TZ), reverse=True)
    return by_team


def get_other_team_history(sport, team_id, season, league_id=None, all_histories=None):
    """Return current-season history, preferring the upcoming fixture's competition."""
    if all_histories is None:
        # Kept only as a safety fallback; the daily scanner uses competition caches.
        key = (sport, int(team_id), str(season))
        if key not in _OTHER_HISTORY_CACHE:
            games = _other_api(sport, "games", {"team": int(team_id), "season": season})
            _OTHER_HISTORY_CACHE[key] = _other_history_clean(games, season)
        current = _OTHER_HISTORY_CACHE[key]
    else:
        current = list(all_histories.get(int(team_id), []))
    if league_id:
    ...
    for sport in OTHER_SPORTS:
        try:
            ...
            if not games:
                ...
                continue
            try:
                histories = _build_other_histories(sport, games)
            except Exception as exc:
                print("OTHER SPORTS HISTORY ERROR:", sport, repr(exc))
                histories = {}
            results = []
            for g in games:
                try:
