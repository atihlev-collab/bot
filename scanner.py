...
def _sport_finished(m):
    # Highlightly's last-five endpoint already guarantees finished games,
    # but different sports expose the finished state slightly differently.
    state = m.get("state") or {}
    desc = _norm(state.get("description"))
    stage = _norm(m.get("stage"))
    status = _norm(state.get("status"))
    finished_words = ("finished", "final", "ended", "after penalties", "awarded")
    is_finished = any(x in desc for x in finished_words) or any(x in stage for x in finished_words) or any(x in status for x in finished_words)
    return is_finished and _sport_score(m) is not None
...
def _sport_last_five(sport, team_id, season):
    rows = _get(f"{SPORT_BASE}/{sport}/last-five-games", SPORT_HOST, {"teamId": int(team_id)})
    games = []
    for m in rows:
        if not _sport_finished(m):
            continue
