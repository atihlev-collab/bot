...
# =========================================================
# SPORT DAILY STATISTICAL SCANNER — SPORT ULTRA
# =========================================================
# Football LIVE/PREMATCH remains disabled in the scheduler.
# Sport Statistics uses the Highlightly Sport Ultra API.
# We fetch today's/tomorrow's fixtures, then real current-season
# team statistics for those teams. No fake values are generated.
# =========================================================
...
def _sport_api_get(endpoint, params=None):
    """One real Sport Ultra request. No retries or hidden pagination."""
...
def _recursive_number(obj, names):
    """Find a numeric field by name anywhere in a stats object."""
    if isinstance(obj, dict):
...
def _extract_team_average(stats_rows, metric):
    """Return current-season scored average from Highlightly team statistics."""
    if not isinstance(stats_rows, list):
        return None

    candidates = []
    metric_names = {
        "points": {"scored", "score", "scoredpoints", "points_scored"},
        "goals": {"scored", "goals_scored", "goalsscored"},
    ...
    for row in stats_rows:
        if not isinstance(row, dict):
            continue
        games = _recursive_number(row.get("total", row), {"played", "gamesplayed", "games"})
        if isinstance(games, float) and games <= 0:
            continue
        scored = _recursive_number(row.get("total", row), metric_names)
        if scored is None:
            scored = _recursive_number(row, metric_names)
        if games and scored is not None:
            season = _recursive_number(row, {"season"})
            candidates.append((season or 0, games, scored, row))

    if not candidates:
        return None

    candidates.sort(key=lambda x: x[0], reverse=True)
    _, games, scored, row = candidates[0]
    return {
        "average": scored / games,
        "games": int(games),
        "season": int(candidates[0][0]) if candidates[0][0] else None,
        "raw": row,
    }
...
def _get_team_average(sport_key, team_id, metric):
    ...
    print(
        ...
        f"average={result['average']:.2f} games={result['games']}" if result else
        f"SPORT HISTORY: {sport_key} team={team_id} NO DATA"
    )
    return result
...
def run_sport_daily_scanner(send_func=None):
    """Build Top 3 Over/Under from real current-season team statistics."""
    ...
    for sport_key, cfg in SPORTS_CONFIG.items():
        ...
        for day in (start.date(), end.date()):
            rows = _sport_api_get(
                cfg["endpoint"],
                {"date": day.isoformat(), "timezone": SPORT_API_TZ, "limit": SPORT_API_LIMIT},
            )
        ...
        for match in unique.values():
            ...
            candidates.append({
                "match": match,
                "datetime": _sport_match_datetime(match),
                "home_avg": h["average"],
                "away_avg": a["average"],
        ...
        print(
            f"SPORT RESULT: {sport_key} fixtures={len(unique)} "
...
def run_due_scans(send_func):
    """Run Sport Statistics once per day; football daily scans are disabled."""
    init_scanner_db()
    now = datetime.now(TZ)
    today = now.date()
