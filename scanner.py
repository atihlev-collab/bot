...
# =========================================================
# OTHER SPORTS DAILY SCANNER
# 10:00 BG
# Matches: 12:00 today -> 12:00 tomorrow
# FOOTBALL SCANNER IS NOT TOUCHED
# =========================================================
...
def _other_sport_api(sport, endpoint, params=None):
    global _LAST_API_CALL
...
def _other_sport_datetime(match):
    raw = (match.get("fixture") or {}).get("date")
...
def _other_sport_score(match, side):
    scores = match.get("scores") or {}
    side_data = scores.get(side) or {}
    ...
    for key in (
        ...
        value = _safe_float(side_data.get(key))
...
def _other_sport_team_name(match, side):
    return (
        (match.get("teams") or {})
        .get(side, {})
        .get("name")
...
def _other_sport_history_average(
    sport,
    team_id,
    season,
):
    matches = _other_sport_api(
        sport,
        "games",
        {
            "team": team_id,
    ...
    for match in matches:
        ...
        side = (
            "home"
            if int(home_id or 0) == int(team_id)
            else "away"
        )

        score = _other_sport_score(match, side)
    ...
    while day <= end_bg.date():
        ...
        if isinstance(response, list):
            ...
            for match in response:
                ...
                dt_bg = _other_sport_datetime(match)
...
def _analyse_other_sport_match(
    sport,
    match,
):
    home_id = (
    ...
    if season is None:
        dt = _other_sport_datetime(match)
        season = dt.year if dt else datetime.now(TZ).year

    home_avg = _other_sport_history_average(
        sport,
        home_id,
        season,
    )

    away_avg = _other_sport_history_average(
        sport,
        away_id,
        season,
    )
    ...
    return {
        "home": _other_sport_team_name(match, "home"),
        "away": _other_sport_team_name(match, "away"),
        "expected": expected,
...
def run_other_sports_scanner(
    ...
    for sport in OTHER_SPORTS:
        ...
        try:
            matches = _get_other_sport_fixtures(
                sport,
                start,
                end,
            )
            ...
            for match in matches:
                result = _analyse_other_sport_match(
                    sport,
                    match,
                )
