...
OTHER_SPORTS = {
    # API-Sports has NBA as a separate product/API (v2), distinct from API-Basketball.
    "nba": {"label": "🏀 NBA", "base": "https://v2.nba.api-sports.io", "api_version": "nba"},
    "basketball": {"label": "🏀 БАСКЕТБОЛ", "base": "https://v1.basketball.api-sports.io", "api_version": "v1"},
    "hockey": {"label": "🏒 ХОКЕЙ", "base": "https://v1.hockey.api-sports.io", "api_version": "v1"},
    "handball": {"label": "🤾 ХАНДБАЛ", "base": "https://v1.handball.api-sports.io", "api_version": "v1"},
    "rugby": {"label": "🏉 РЪГБИ", "base": "https://v1.rugby.api-sports.io", "api_version": "v1"},
    "american_football": {"label": "🏈 NFL / NCAA", "base": "https://v1.american-football.api-sports.io", "api_version": "v1"},
    "baseball": {"label": "⚾ БЕЙЗБОЛ", "base": "https://v1.baseball.api-sports.io", "api_version": "v1"},
}
...
_OTHER_TEAM_SEASON_CACHE = {}
_OTHER_LEAGUE_SEASON_CACHE = {}
...
def _other_score(g, side):
    ...
    if isinstance(value, dict):
        # API-Sports uses different score keys between sports.
...
def _other_season(g):
    ...
    # API-NBA can expose the season at the top level in some responses.
    return g.get("season")
...
def _other_is_official(g):
    ...
    # Same rule as football: never use friendlies/exhibitions.
...
def _other_history_clean(games, season):
    ...
    for g in games or []:
        ...
        if not _other_is_official(g):
            continue
        # Some API-Sports non-football game payloads omit league.season.
        # The /games?team=...&season=... request is already season-scoped,
        # so missing season metadata must NOT discard the match.
        game_season = _other_season(g)
        if game_season is not None and str(game_season) != str(season):
            continue
...
def get_other_sport_fixtures(sport, start_bg, end_bg):
    ...
    while day <= end_bg.date():
        response = _other_games_for_date(sport, day)
        for g in response:
            gid = _other_game_id(g)
            ...
            if dt < start_bg or dt >= end_bg or dt <= datetime.now(TZ):
                continue
            if _other_is_finished(g) or not home.get("id") or not away.get("id"):
                continue

            # NBA is a separate API-Sports product.  Do not let the generic
            # Basketball API duplicate NBA games; NBA games are read only from
            # API-NBA above.
            if sport == "basketball":
                league_name = str((g.get("league") or {}).get("name") or "").casefold()
                if league_name == "nba" or league_name.startswith("nba ") or "nba cup" in league_name:
                    continue
...
def get_other_team_history(sport, team_id, season, league_id=None):
    """Football-style history lookup with API-call minimization."""
    team_id = int(team_id)
    season_key = str(season)
    league_key = str(league_id or "")
    key = (sport, team_id, season_key, league_key)
    if key in _OTHER_HISTORY_CACHE:
        return _OTHER_HISTORY_CACHE[key]

    # Prefer one current-season competition request and filter locally.
    if league_id:
        lk = (sport, league_key, season_key)
        if lk not in _OTHER_LEAGUE_SEASON_CACHE:
            games = _other_api(sport, "games", {"league": league_id, "season": season})
            _OTHER_LEAGUE_SEASON_CACHE[lk] = _other_history_clean(games, season)
        competition_games = [
            g for g in _OTHER_LEAGUE_SEASON_CACHE.get(lk, [])
            if any(int(t.get("id") or -1) == team_id for t in _other_game_teams(g))
        ]
        competition_games.sort(
            key=lambda x: _other_game_time_bg(x) or datetime.min.replace(tzinfo=TZ),
            reverse=True,
        )
        if len(competition_games) >= 3:
            result = competition_games[:12]
            result.sort(key=lambda x: _other_game_time_bg(x) or datetime.min.replace(tzinfo=TZ))
            _OTHER_HISTORY_CACHE[key] = result
            return result
        print("OTHER SPORTS HISTORY FALLBACK:", sport, team_id,
              "competition_matches=", len(competition_games),
              "-> current-season official matches")

    # Fallback: one team+season request, never one request per historical game.
    base_key = (sport, team_id, season_key)
    if base_key not in _OTHER_TEAM_SEASON_CACHE:
        games = _other_api(sport, "games", {"team": team_id, "season": season})
        _OTHER_TEAM_SEASON_CACHE[base_key] = _other_history_clean(games, season)
    current = list(_OTHER_TEAM_SEASON_CACHE.get(base_key, []))
    current.sort(key=lambda x: _other_game_time_bg(x) or datetime.min.replace(tzinfo=TZ), reverse=True)
    result = current[:12]
    result.sort(key=lambda x: _other_game_time_bg(x) or datetime.min.replace(tzinfo=TZ))
    _OTHER_HISTORY_CACHE[key] = result
    return result
...
def _other_team_avg(team_id, history):
    ...
    for g in history or []:
        home, away = _other_game_teams(g)
        hs, aws = _other_score(g, "home"), _other_score(g, "away")
...
def run_other_sports_scanner(reference_date=None, send_func=None):
    ...
    lines = ["🌍 DAILY OTHER SPORTS SCANNER", ref.strftime("%d.%m.%Y"),
    ...
    for sport in OTHER_SPORTS:
        try:
            ...
            if not games:
                lines += [OTHER_SPORTS[sport]["label"], "Няма срещи в 24-часовия прозорец.", ""]
                continue
            histories = {}
            unique = {}
            for g in games:
                ...
                if season is not None:
                    for team in (home, away):
                        if team.get("id"):
                            unique[(int(team["id"]), str(season), _other_league_id(g))] = None
            for team_id, season, league_id in unique:
                try:
                    ...
                    histories[team_id] = []
            print(
                f"OTHER SPORTS HISTORY [{sport}]: teams={len(unique)} "
                f"with_history={sum(1 for h in histories.values() if len(h) >= 3)}"
            )
            results = []
            for g in games:
                try:
            ...
            lines += [OTHER_SPORTS[sport]["label"], "Грешка при зареждането на данните.", ""]
